...
std::string resolvePath_Internal(std::string const& originalPath)
{
  ...
  if (originalPath.find('[') != std::string::npos) {
    ...
    while (openPos > -1) {
      ...
      if (closePos < openPos || closePos == openPos + 1) {
        EC_LOG_ERROR("[ExocortexAlembic] Invalid token bracketing in path: "
                     << path.asChar());
        return std::string(path.asChar());
      }
      MString prefix = (openPos > 0) ? path.substring(0, openPos - 1) : "";
      MString suffix = (closePos < ((int)path.length() - 1))
                           ? path.substring(closePos + 1, path.length() - 1)
                           : "";
      MString token = path.substring(openPos + 1, closePos - 1);
      MStringArray tokens;
      token.split(' ', tokens);
      if (tokens.length() != 2) {
        EC_LOG_ERROR("[ExocortexAlembic] Invalid token '"
                     << token.asChar() << "' in path: " << path.asChar());
        return std::string(path.asChar());
      }
      if (tokens[0].toLowerCase() == "env") {
        MString value = getenv(tokens[1].asChar());
        if (value.length() == 0) {
          EC_LOG_ERROR("[ExocortexAlembic] Environment variable '"
                       << tokens[1].asChar() << "' not found!");
          return std::string(path.asChar());
        }
        path = prefix + value + suffix;
      }
      else if (tokens[0].toLowerCase() == "scene" &&
               tokens[1].toLowerCase() == "folder") {
        MFileIO io;
  ...
  if (s_originalToResolvedPath.find(expandedPath) !=
    ...
    s_originalToResolvedPath.insert(std::pair<std::string, std::string>(
        expandedPath, std::string(path.asChar())));
  }
