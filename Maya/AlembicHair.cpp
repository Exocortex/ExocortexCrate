...
MStatus AlembicHairNode::initialize()
{
   ...
   // input file name
   mFileNameAttr = tAttr.create("fileName", "fn", MFnData::kString,
emptyStringObject);
   ...
   // input identifier
   mIdentifierAttr = tAttr.create("identifier", "if", MFnData::kString,
emptyStringObject);
   status = tAttr.setStorable(true);
   status = tAttr.setKeyable(false);
...
not found in archive '"+mFileName+"'.");
      ...
      {
         MGlobal::displayWarning("[ExocortexAlembic] Identifier '"+identifier+"'
in archive '"+mFileName+"' is not a Curves.");
         return MStatus::kFailure;
      }
   ...
   if(!mCurvesData.isNull() && mLastSampleInfo.floorIndex ==
sampleInfo.floorIndex && mLastSampleInfo.ceilIndex == sampleInfo.ceilIndex)
      return MStatus::kSuccess;
   ...
   {
      MGlobal::displayWarning("[ExocortexAlembic] Identifier '"+identifier+"' in
archive '"+mFileName+"' contains more than one curve.");
      return MStatus::kFailure;
   }
   ...
   if(samplePos->size() > 0)
   {
      ...
      if(sampleInfo.alpha != 0.0)
      {
         ...
         if(points.length() == (unsigned int)samplePos2->size())
         {
            ...
            {
               points[i].x = samplePos->get()[i].x * iblend +
samplePos2->get()[i].x * blend;
               points[i].y = samplePos->get()[i].y * iblend +
samplePos2->get()[i].y * blend;
               points[i].z = samplePos->get()[i].z * iblend +
samplePos2->get()[i].z * blend;
            }
            done = true;
         }
      }
   ...
   mCurves.create(points,knots,degree,closed ? MFnNurbsCurve::kClosed :
MFnNurbsCurve::kOpen, false, false, mCurvesData);
...
MStatus AlembicHairDeformNode::deform(MDataBlock & dataBlock, MItGeometry &
iter, const MMatrix & localToWorld, unsigned int geomIndex)
{
...
iblend + samplePos2->get()[index].x * blend);
            abcPos.y = iweight * pt.y + weight * (samplePos->get()[index].y *
iblend + samplePos2->get()[index].y * blend);
            abcPos.z = iweight * pt.z + weight * (samplePos->get()[index].z *
iblend + samplePos2->get()[index].z * blend);
            done = true;
         }
      }
      if(!done)
