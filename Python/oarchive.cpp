...
static PyObject *oArchive_createTimeSampling(PyObject *self, PyObject *args)
{
  ...
  const size_t nbTimes =
      is_list ? PyList_Size(timesTuple) : PyTuple_Size(timesTuple);
  if (nbTimes == 0) {
  ...
  int ii = 0;
  for (std::list<PyObject *>::iterator beg = valid_obj.begin();
       beg != valid_obj.end(); ++beg, ++ii) {
    unsigned long tsIndex = archive->mArchive->addTimeSampling(
        *(((EA_TimeSampling *)*beg)->tsampling));
...
static PyObject *oArchive_createObject(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "ss|i", &type, &identifier, &tsIndex)) {
    PyErr_SetString(
        getError(),
        "No type, identifier and / or timeSamplingIndex specified!");
    return NULL;
  }
  ...
  if (objectPtr) {
    Py_INCREF(objectPtr);
    return (PyObject *)objectPtr;  // NEW changed because when you get an
                                   // oProperty, if it already exists, it
                                   // returns it, it doesn't say there's some
                                   // kind of error
  }
  ...
  if (typeStr.substr(0, 14) == "AbcGeom_Xform_") {
    ...
    // INFO_MSG("parent.getMetaData().get(\"schema\") = " <<
    // parent.getMetaData().get("schema"));
    // INFO_MSG("type string = " << typeStr);

    if (typeStr.substr(0, 15) == "AbcGeom_Camera_") {
      casted.mType = oObjectType_Camera;
      casted.mCamera =
          new AbcG::OCamera(parent, parts[parts.size() - 1], tsIndex);
      ...
      casted.mPolyMesh =
          new AbcG::OPolyMesh(parent, parts[parts.size() - 1], tsIndex);
      ...
      casted.mCurves =
          new AbcG::OCurves(parent, parts[parts.size() - 1], tsIndex);
      ...
      casted.mPoints =
          new AbcG::OPoints(parent, parts[parts.size() - 1], tsIndex);
    ...
    // **** NEW
      ...
      casted.mFaceSet =
          new AbcG::OFaceSet(parent, parts[parts.size() - 1], tsIndex);
      ...
      casted.mNuPatch =
          new AbcG::ONuPatch(parent, parts[parts.size() - 1], tsIndex);
      obj = Abc::OObject(*casted.mNuPatch, Abc::kWrapExisting);
    }

    // moved at the end!! let see if it works
    else if (parent.getMetaData().get("schema").substr(0, 14) !=
             "AbcGeom_Xform_") {
      PyErr_SetString(
          getError(),
          "This type of object has to be put below a xform object!");
      return NULL;
    }
    else {
...
static PyMethodDef oArchive_methods[] = {
    {"getFileName", (PyCFunction)oArchive_getFileName, METH_NOARGS,
     "Returns the filename this archive is linked to."},
    {"createTimeSampling", (PyCFunction)oArchive_createTimeSampling,
     METH_VARARGS,
     "Takes in a flat list of sample times (float) and creates a new "
     "TimeSampling. Returns True if successful, False otherwise."},
    {"createObject", (PyCFunction)oArchive_createObject, METH_VARARGS,
     "Takes in a valid type string, an identifier string as well as an "
     "optional timeSamplingIndex. Returns the created oObject or, if it "
     "already exists, a reference to this oObject. Valid object types can be "
     "found in AppendixA of the documentation."},
    {NULL, NULL}};
...
static void createArchive(oArchive *object, const char *fileName, bool useOgawa)
{
  const std::string expName =
      getExporterName("Python " EC_QUOTE(crate_Python_Version));
  const std::string expFileName = getExporterFileName("Unknown");
  if (useOgawa) {
    *object->mArchive = CreateArchiveWithInfo(
        Alembic::AbcCoreOgawa::WriteArchive(), fileName, expName.c_str(),
        expFileName.c_str(), Abc::ErrorHandler::kThrowPolicy);
  }
  else {
    *object->mArchive = CreateArchiveWithInfo(
        Alembic::AbcCoreHDF5::WriteArchive(true), fileName, expName.c_str(),
        expFileName.c_str(), Abc::ErrorHandler::kThrowPolicy);
  }
}
...
static void init_oArchiveElement(oArchiveElement &element,
...
void oArchive_registerPropElement(oArchive *archive, std::string identifier,
                                  oProperty *prop)
{
  oArchiveElement element;
  init_oArchiveElement(element, identifier);
...
void oArchive_registerCompPropElement(oArchive *archive, std::string identifier,
...
static oArchiveElement *oArchive_getArchiveElement(oArchive *archive,
                                                   std::string identifier)
{
...
oCompoundProperty *oArchive_getCompPropElement(oArchive *archive,
                                               std::string identifier)
{
  oArchiveElement *element = oArchive_getArchiveElement(archive, identifier);
...
oXformProperty *oArchive_getXformElement(oArchive *archive,
                                         std::string identifier)
{
  oArchiveElement *element = oArchive_getArchiveElement(archive, identifier);
