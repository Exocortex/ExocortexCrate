...
static PyObject *oXformProperty_setValues(PyObject *self, PyObject *args)
{
  ...
  for (size_t i = 0; i < 16; i++) {
    ...
    if (!PyArg_Parse(item, "d", &values[i])) {
      PyErr_SetString(
          getError(),
          "Some item of the sample tuple is not a floating point number!");
      return NULL;
    }
  }

  Imath::M44d matrix(values[0], values[1], values[2], values[3], values[4],
...
static PyMethodDef oXformProperty_methods[] = {
    ...
    {"isCompound", (PyCFunction)oXformProperty_isCompound, METH_NOARGS,
     "To distinguish between an oXformProperty and an oCompoundProperty, "
     "always returns false for oXformProperty."},
    {NULL, NULL}};
...
static PyTypeObject oXformProperty_Type = {
    ...
    "This is the output Xform property. It provides methods for setting data "
    "on the Xform.",        /* tp_doc */
    0,                      /* tp_traverse */
    0,                      /* tp_clear */
    0,                      /* tp_richcompare */
                             ...
                             boost::uint32_t tsIndex)
{
  ...
  prop->mMaxNbSamples =
      archive->mArchive->getTimeSampling(tsIndex)->getNumStoredTimes();
  prop->mMembers = new oXformMembers();
  prop->mMembers->mXformSchema = in_casted.mXform->getSchema();
  prop->mMembers->mXformSchema.setTimeSampling(
      archive->mArchive->getTimeSampling(tsIndex));

  // prop->mXformSchema = new
  // AbcG::OXformSchema(in_casted.mXform->getSchema().getPtr(),Abc::kWrapExisting);
