...
static PyObject *iObject_getMetaData(PyObject *self, PyObject *args)
{
  ...
  Abc::IStringArrayProperty metaDataProp =
      Abc::IStringArrayProperty(compound, ".metadata");
...
static PyObject *iObject_getPropertyNames(PyObject *self, PyObject *args)
{
  ...
  if (AbcG::IXform::matches(object->mObject->getMetaData())) {
    ...
    for (size_t i = 0; i < compound.getNumProperties(); i++) {
      PyTuple_SetItem(
          tuple, i,
          Py_BuildValue("s", compound.getPropertyHeader(i).getName().c_str()));
    }
  }
...
static PyObject *iObject_getTsIndex(PyObject *self, PyObject *args)
{
  ...
  if (obj->tsIndex == -1) {
    ...
    const int nb_ts =
        ((iArchive *)obj->mArchive)->mArchive->getNumTimeSamplings();
    for (int i = 0; i < nb_ts; ++i) {
      Abc::TimeSamplingPtr ts =
          ((iArchive *)obj->mArchive)
              ->mArchive->getTimeSampling((boost::uint32_t)i);
      if (ts == ts_ptr) {
        obj->tsIndex = i;
        break;
      }
...
static PyMethodDef iObject_methods[] = {
    ...
    {"getMetaData", (PyCFunction)iObject_getMetaData, METH_NOARGS,
     "Returns the string array storing the metadata, if it exists on this "
     "object. Otherwise it returns an empty tuple."},
    ...
    {"getNbStoredSamples", (PyCFunction)iObject_getNbStoredSamples, METH_NOARGS,
     "Returns the actual number of stored samples."},
    {"getPropertyNames", (PyCFunction)iObject_getPropertyNames, METH_NOARGS,
     "Returns a string list of all property names below this object."},
    ...
    {"getTsIndex", (PyCFunction)iObject_getTsIndex, METH_NOARGS,
     "Returns time sampling index used by this object."},
    {NULL, NULL}};
