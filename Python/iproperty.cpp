...
static PyObject *iProperty_getNbStoredSamples(PyObject *self, PyObject *args)
{
  ...
  return Py_BuildValue("I",
                       (unsigned int)iProperty_getNbStoredSamples_func(self));
  ALEMBIC_PYOBJECT_CATCH_STATEMENT
}
  ...
  if (prop->intent > 1) {                                                    \
    ...
    for (int i = 0; i < prop->intent; ++i)                                   \
      PyTuple_SetItem(tuple, i,                                              \
                      Py_BuildValue(python_cast, (cast_type)values[i]));     \
    delete[] values;                                                         \
  }                                                                          \
  else {                                                                     \
    tuple = PyTuple_New(1);                                                  \
...
static PyObject *iProperty_getValues(PyObject *self, PyObject *args)
{
  ...
  switch (prop->mPropType) {
    ...
    case propertyTP_uint64: {
      _GET_VALUE_INTENT(mUInt64Property, Abc::uint64_t, "K",
                        unsigned long long);
      /*
      tuple = PyTuple_New(1);
      Abc::uint64_t value;
    ...
    case propertyTP_uchar_array: {
      ...
      if (start >= sample->size()) {
        ...
        for (unsigned long long i = start; i < end; i++) {
          value = sample->get()[i];
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("I", (unsigned int)value));
        }
      }
      break;
    }
    ...
    case propertyTP_uint32_array: {
      ...
      if (start >= sample->size()) {
        ...
        for (unsigned long long i = start; i < end; i++) {
          value = sample->get()[i];
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("k", (unsigned long)value));
        }
      }
      break;
    }
    ...
    case propertyTP_uint64_array: {
      ...
      if (start >= sample->size()) {
        ...
        for (unsigned long long i = start; i < end; i++) {
          value = sample->get()[i];
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("K", (unsigned long long)value));
        }
      }
      break;
    }
    case propertyTP_int64_array: {
      ...
      if (start >= sample->size()) {
        ...
        for (unsigned long long i = start; i < end; i++) {
          value = sample->get()[i];
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("L", (long long)value));
        }
      }
      break;
    }
    ...
    case propertyTP_box2s_array: {
      ...
      if (start >= sample->size()) {
        ...
        for (unsigned long long i = start; i < end; i++) {
          value = sample->get()[i];
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.min.x));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.min.y));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.max.x));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.max.y));
        }
      }
      break;
    }
      ...
      if (start >= sample->size()) {
        ...
        for (unsigned long long i = start; i < end; i++) {
          ...
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.min.y));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.min.z));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.max.x));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.max.y));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("i", (int)value.max.z));
        }
      }
      break;
    }
    ...
    case propertyTP_c3c_array: {
      ...
      if (start >= sample->size()) {
        ...
        for (unsigned long long i = start; i < end; i++) {
          value = sample->get()[i];
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("I", (unsigned int)value.x));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("I", (unsigned int)value.y));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("I", (unsigned int)value.z));
        }
      }
      break;
    }
    ...
    case propertyTP_c4c_array: {
      ...
      if (start >= sample->size()) {
        ...
        for (unsigned long long i = start; i < end; i++) {
          value = sample->get()[i];
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("I", (unsigned int)value.r));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("I", (unsigned int)value.g));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("I", (unsigned int)value.b));
          PyTuple_SetItem(tuple, offset++,
                          Py_BuildValue("I", (unsigned int)value.a));
        }
      }
      break;
    }
...
static PyMethodDef iProperty_methods[] = {
    ...
    {"getSampleTimes", (PyCFunction)iProperty_getSampleTimes, METH_NOARGS,
     "Returns the TimeSampling this object is linked to."},
    {"getNbStoredSamples", (PyCFunction)iProperty_getNbStoredSamples,
     METH_NOARGS, "Returns the actual number of stored samples."},
    {"getSize", (PyCFunction)iProperty_getSize, METH_VARARGS,
     "Returns the size of the property. For single value properties, this "
     "method returns 1, for array value properties it returns the size of the "
     "array."},
    {"getValues", (PyCFunction)iProperty_getValues, METH_VARARGS,
     "Returns the values of the property at the (optional) sample index."},
    {"isCompound", (PyCFunction)iProperty_isCompound, METH_NOARGS,
     "To distinguish between an iProperty and an iCompoundProperty, always "
     "returns false for iProperty."},
    {NULL, NULL}};
...
static PyTypeObject iProperty_Type = {
    ...
    "This is the input property. It provides access to the propery's data, "
    "such as name, type and per sample values.", /* tp_doc */
    0,                                           /* tp_traverse */
    0,                                           /* tp_clear */
    0,                                           /* tp_richcompare */
    0,                                           /* tp_weaklistoffset */
...
#define _NEW_PROP_IMPL_(tp, base, singleprop, arrayprop)                  \
  if (prop->mIsArray) {                                                   \
    ...
    prop->m##base##arrayprop =                                            \
        new Abc::I##base##arrayprop(compound, in_propName);               \
  }                                                                       \
  else {                                                                  \
    prop->mPropType = tp;                                                 \
    prop->m##base##singleprop =                                           \
        new Abc::I##base##singleprop(compound, in_propName);              \
  }
...
#define _NEW_PROP_CASE_(pod, tp, base) \
  _NEW_PROP_CASE_IMPL_(pod, tp, base, Property, ArrayProperty)
...
PyObject *iProperty_new(Abc::ICompoundProperty &compound, char *in_propName)
{
  ...
  if (prop != NULL) {
    if (propHeader->isCompound()) {
      ...
      if (prop->mIsArray) {
        prop->intent = 0;
        prop->mBaseArrayProperty =
            new Abc::IArrayProperty(compound, in_propName);
        interpretation =
            prop->mBaseArrayProperty->getMetaData().get("interpretation");
      }
      else {
        prop->mBaseScalarProperty =
            new Abc::IScalarProperty(compound, in_propName);
        interpretation =
