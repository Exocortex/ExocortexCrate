...
static void recurseObjectChildren(PyObject *list, const Abc::IObject &obj)
{
  ...
  for (int i = 0; i < nbChildren; ++i) {
    ...
    PyObject *item =
        PyString_FromStringAndSize(fullName.c_str(), fullName.size());
...
static PyObject *iArchive_getSampleTimes(PyObject *self, PyObject *args)
{
  ...
  for (int i = 0; i < nb_ts; ++i)
    PyList_SetItem(
        _list, i,
        TimeSamplingCopy(iarchive->getTimeSampling((boost::uint32_t)i)));
  return _list;
  ALEMBIC_PYOBJECT_CATCH_STATEMENT
}
...
static PyMethodDef iArchive_methods[] = {
    ...
    {"getVersion", (PyCFunction)iArchive_getVersion, METH_NOARGS,
     "Returns the version of the archive loaded."},
    {"getIdentifiers", (PyCFunction)iArchive_getIdentifiers, METH_NOARGS,
     "Returns a flat string list of all of the identifiers available."},
    {"getObject", (PyCFunction)iArchive_getObject, METH_VARARGS,
     "Returns an iObject for the provided identifier string."},
    {"getSampleTimes", (PyCFunction)iArchive_getSampleTimes, METH_NOARGS,
     "Returns a two dimensional array of all TimeSamplings available in this "
     "file."},
    {"getCoreType", (PyCFunction)iArchive_getCoreType, METH_NOARGS,
     "Returns the core used by the archive (HDF5, Ogawa, or Unknown)."},
    {NULL, NULL}};
...
static PyTypeObject iArchive_Type = {
    ...
    "This is the input archive. It provides access to all of the archive's "
    "data, including TimeSamplings as well as objects.", /* tp_doc */
    0,                                                   /* tp_traverse */
    0,                                                   /* tp_clear */
    0,                                                   /* tp_richcompare */
    0,                                                   /* tp_weaklistoffset */
...
PyObject *iArchive_new(PyObject *self, PyObject *args)
{
  ...
  /*if(getNbOArchives() > 0)
  {
     PyErr_SetString(getError(), "Can only create iArchives if all oArchives are
  closed!");
     return NULL;
  }*/
  ...
  if (object != NULL) {
    ...
    object->mArchive =
        new Abc::IArchive(iFactory.getArchive(fileName, object->oType));
