...
bool SceneNodeXSI::replaceData(SceneNodeAlembicPtr fileNode,
                               ...
                               SceneNodeAlembicPtr& nextFileNode)
{
  ...
  SceneNodeAlembicPtr shapeFileNode =
      reinterpret<SceneNode, SceneNodeAlembic>(returnNode);

  // delete the merged shape from the tree. this is done so that we will not
  // look at the merged node when attach to next node to attach to

  // for(SceneChildIterator it = shapeFileNode->children.begin(),
  // SceneChildIterator endIt = shapeFileNode->children.end(); it != endIt;
  // it++){
  //   SceneNodeAlembicPtr childFileNode = reinterpret<SceneNode,
  //   SceneNodeAlembic>(*it);
  //
...
bool SceneNodeXSI::addChild(SceneNodeAlembicPtr fileNode,
                            const IJobStringParser& jobParams,
...
{
  ESS_LOG_WARNING(
      "XSINodeObjectCRef: " << nodeRef.GetAsText().GetAsciiString());
}
...
SceneNode::nodeTypeE getNodeType(X3DObject& xObj)
{
  ...
  if (xObj_GetType.IsEqualNoCase(L"#model")) {
    ...
    ICEAttribute strandPosition =
        xObj.GetActivePrimitive().GetGeometry().GetICEAttributeFromName(
            L"StrandPosition");
  ...
  // ESS_LOG_WARNING("Unknown type of XSI node, name:
  // "<<xObj.GetName().GetAsciiString()<<", type:
  // "<<xObj.GetType().GetAsciiString());
  //
...
struct CSGStackElement {
  ...
  CSGStackElement(XSI::CRef xnode, SceneNodePtr node)
      : xNode(xnode), eNode(node)
  {
  }
};
...
void XSIProgressBar::start(void) { prog.PutVisible(true); }
void XSIProgressBar::stop(void) { prog.PutVisible(false); }
void XSIProgressBar::incr(int step) { prog.Increment(step); }
bool XSIProgressBar::isCancelled(void) { return prog.IsCancelPressed(); }
void XSIProgressBar::setCaption(std::string& caption)
{
  ...
  if (!Application().IsInteractive()) {
    ...
    progStr << std::setw(3) << (prog.GetValue() * 100 / prog.GetMaximum())
            << " - " << caption;
                                       ...
                                       XSI_XformTypes::xte in_xte)
{
  ...
  while (true) {
    ...
    if (AbcG::IXform::matches(iObj.getMetaData())) {
      ...
      if (xte == XSI_XformTypes::UNKNOWN &&
          currNode->type == SceneNode::ITRANSFORM) {
        xte = in_xte;
      }
    }
...
Imath::M44f SceneNodeXSI::getGlobalTransFloat(double time)
{
  ...
  return ::CMatrix4_to_M44f(
      xObj.GetKinematics().GetGlobal().GetTransform(time).GetMatrix4());
}

Imath::M44d SceneNodeXSI::getGlobalTransDouble(double time)
{
  XSI::X3DObject xObj(nodeRef);
  return ::CMatrix4_to_M44d(
      xObj.GetKinematics().GetGlobal().GetTransform(time).GetMatrix4());
}
