    ...
    : AlembicObject(eNode, in_Job, oParent)
{
  ...
  mOrientationProperty = Abc::OQuatfArrayProperty(argGeomParams, ".orientation",
                                                  mPointsSchema.getMetaData(),
                                                  GetJob()->GetAnimatedTs());
  ...
  mMassProperty = Abc::OFloatArrayProperty(argGeomParams, ".mass",
                                           mPointsSchema.getMetaData(),
                                           GetJob()->GetAnimatedTs());
  mShapeTypeProperty = Abc::OUInt16ArrayProperty(argGeomParams, ".shapetype",
                                                 mPointsSchema.getMetaData(),
                                                 GetJob()->GetAnimatedTs());
...
XSI::CStatus AlembicPoints::Save(double time)
{
  ...
  // deal with each attribute. we scope here do free memory instantly
  {
    ...
    std::vector<Abc::V3f> positionVec;
    {
      ...
      if (attr.IsDefined() && attr.IsValid()) {
        ...
        ULONG count =
            (data.IsConstant() || attr.IsConstant()) ? 1 : data.GetCount();
        positionVec.resize((size_t)count);
        for (ULONG i = 0; i < count; i++) {
          positionVec[i].setValue(data[i].GetX(), data[i].GetY(),
                                  data[i].GetZ());
  ...
  for (int attrIndex = 0; attrIndex < 2; attrIndex++) {
    ICEAttribute attr = geo.GetICEAttributeFromName(
        attrIndex == 0 ? L"Orientation" : L"AngularVelocity");
          ...
          for (ULONG i = 0; i < count; i++) {
            ...
            if (data[i].GetRepresentation() == CRotationf::siAxisAngleRot) {
              ...
              rot.SetFromAxisAngle(
                  CVector3(axis.GetX(), axis.GetY(), axis.GetZ()), angle);
              ...
              CRotation rot;
              rot.SetFromXYZAngles(
                  CVector3(euler.GetX(), euler.GetY(), euler.GetZ()));
              CQuaternion quat = rot.GetQuaternion();
              vecVal.v.x = (float)quat.GetX();
              vecVal.v.y = (float)quat.GetY();
              vecVal.v.z = (float)quat.GetZ();
        ...
        if (count > 0) {
          ...
          for (ULONG i = 0; i < count; i++) {
            ...
            if (isConstant) {
              ...
              if (isConstant && (firstVal.GetType() == siICEShapeInstance ||
                                 firstVal.GetType() == siICEShapeReference))
                isConstant =
              ...
              switch (shape.GetType()) {
                ...
                case siICEShapeReference: {
                  ...
                  if (it == mInstanceMap.end()) {
                    // insert it
                    CRef ref =
                        Application().GetObjectFromID(shape.GetReferenceID());
                    mInstanceMap.insert(std::pair<unsigned long, size_t>(
                        shape.GetReferenceID(), mInstanceNames.size()));
        ...
        if (mInstanceNames.size() > 0) {
          if (!mInstancenamesProperty)
            mInstancenamesProperty = Abc::OStringArrayProperty(
                mPointsSchema.getArbGeomParams(), "instancenames",
                mPointsSchema.getMetaData(), GetJob()->GetAnimatedTs());
          ...
          if (vec.size() > 0) {
            if (!mShapeInstanceIDProperty)
              mShapeInstanceIDProperty = Abc::OUInt16ArrayProperty(
                  mPointsSchema.getArbGeomParams(), "shapeinstanceid",
                  mPointsSchema.getMetaData(), GetJob()->GetAnimatedTs());
    ...
    if (attr.IsDefined() && attr.IsValid()) {
      if (mNumSamples == 0)
        mShapeTimeProperty = Abc::OFloatArrayProperty(
            mPointsSchema.getArbGeomParams(), "shapetime",
            mPointsSchema.getMetaData(), GetJob()->GetAnimatedTs());
...
{
  ICENodeDef nodeDef;
  nodeDef = Application().GetFactory().CreateICENodeDef(L"alembic_points",
                                                        L"alembic_points");
  ...
  st = nodeDef.AddInputPort(ID_IN_identifier, ID_G_100, siICENodeDataString,
                            ...
                            ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddInputPort(ID_IN_renderpath, ID_G_100, siICENodeDataString,
                            siICENodeStructureSingle, siICENodeContextSingleton,
                            L"renderpath", L"renderpath", L"", ID_UNDEF,
                            ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddInputPort(
      ID_IN_renderidentifier, ID_G_100, siICENodeDataString,
      siICENodeStructureSingle, siICENodeContextSingleton, L"renderidentifier",
      L"renderidentifier", L"", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddInputPort(ID_IN_time, ID_G_100, siICENodeDataFloat,
                            ...
                            ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddInputPort(ID_IN_usevel, ID_G_100, siICENodeDataBool,
                            siICENodeStructureSingle, siICENodeContextSingleton,
                            L"usevel", L"usevel", false, ID_UNDEF, ID_UNDEF,
                            ID_UNDEF);
  st.AssertSucceeded();

  // Add output ports.
  st = nodeDef.AddOutputPort(ID_OUT_position, siICENodeDataVector3,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"position", L"position", ID_UNDEF, ID_UNDEF,
                             ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_velocity, siICENodeDataVector3,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"velocity", L"velocity", ID_UNDEF, ID_UNDEF,
                             ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_id, siICENodeDataLong,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"id", L"id", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_size, siICENodeDataFloat,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"size", L"size", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_scale, siICENodeDataVector3,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"scale", L"scale", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_orientation, siICENodeDataRotation,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"orientation", L"orientation", ID_UNDEF, ID_UNDEF,
                             ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_angularvelocity, siICENodeDataRotation,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"angularvelocity", L"angularvelocity", ID_UNDEF,
                             ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_age, siICENodeDataFloat,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"age", L"age", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_mass, siICENodeDataFloat,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"mass", L"mass", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_shape, siICENodeDataShape,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"shape", L"shape", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_shapeid, siICENodeDataLong,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"shapeid", L"shapeid", ID_UNDEF, ID_UNDEF,
                             ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_shapetime, siICENodeDataFloat,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"shapetime", L"shapetime", ID_UNDEF, ID_UNDEF,
                             ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_color, siICENodeDataColor4,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"color", L"color", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  ...
  switch (out_portID) {
          ...
          if (usevel) {
            ...
            if (velPtr == NULL) {
              ...
              for (ULONG i = 0; i < acc.GetCount(); i++)
                acc[i].Set(
                    ptr->get()[i].x +
                        alpha * velPtr->get()[i >= velPtr->size() ? 0 : i].x,
                    ptr->get()[i].y +
                        alpha * velPtr->get()[i >= velPtr->size() ? 0 : i].y,
                    ptr->get()[i].z +
                        alpha * velPtr->get()[i >= velPtr->size() ? 0 : i].z);
              done = true;
            }
          }
          else if (obj.getSchema().getIdsProperty().isConstant()) {
            Abc::IP3fArrayProperty posProp2 =
                obj.getSchema().getPositionsProperty();
            ...
            if (ptr->size() == pointsCeilSamplePtr->size()) {
              ...
              for (ULONG i = 0; i < acc.GetCount(); i++)
                acc[i].Set((1.0f - alpha) * ptr->get()[i].x +
                               alpha * pointsCeilSamplePtr->get()[i].x,
                           (1.0f - alpha) * ptr->get()[i].y +
                               alpha * pointsCeilSamplePtr->get()[i].y,
                           (1.0f - alpha) * ptr->get()[i].z +
                               alpha * pointsCeilSamplePtr->get()[i].z);
              done = true;
            }
          }
        }
        ...
        if (sampleInfo.alpha != 0.0 && usevel) {
          ...
          if (getArbGeomParamPropertyAlembic(obj, "angularvelocity", velProp)) {
            Abc::QuatfArraySamplePtr velPtr =
                velProp.getValue(sampleInfo.floorIndex);
            if (!(velPtr == NULL || velPtr->size() == 0)) {
              CQuaternionf quat, vel;
              for (ULONG i = 0; i < acc.GetCount(); i++) {
                quat.Set(ptr->get()[i].r, ptr->get()[i].v.x, ptr->get()[i].v.y,
                         ptr->get()[i].v.z);
                vel.Set(velPtr->get()[i >= velPtr->size() ? 0 : i].r,
    ...
    case ID_OUT_angularvelocity: {
      ...
      if (ptr == NULL || ptr->size() == 0) {
        ...
        CQuaternionf quat;
        for (ULONG i = 0; i < acc.GetCount(); i++) {
          quat.Set(ptr->get()[i].r, ptr->get()[i].v.z, ptr->get()[i].v.y,
                   ptr->get()[i].v.z);
          acc[i].Set(quat);
        }
      }
    ...
    case ID_OUT_shape: {
      ...
      IUInt16ArrayProperty shapeTypeProp = Abc::IUInt16ArrayProperty(
      obj.getSchema(), ".shapetype" );
      /*
      IUInt16ArrayProperty shapeInstanceIDProp = Abc::IUInt16ArrayProperty(
      obj.getSchema(), ".shapeinstanceid" );
      IStringArrayProperty shapeInstanceNamesProp = Abc::IStringArrayProperty(
      obj.getSchema(), ".instancenames" );
      ...
      shapeInstanceNamesProp.getValue(sampleInfo.floorIndex);
      * /
    ...
    case ID_OUT_shapeid: {
      ...
      /*if ( obj.getSchema().getPropertyHeader( ".shapetype" ) == NULL ||
      obj.getSchema().getPropertyHeader( ".shapeinstanceid" ) == NULL )
      {
         acc = outData.Resize(0,0);
         return CStatus::OK;
      }
    ...
    case ID_OUT_color: {
      ...
      if (ptr == NULL) {
        ...
        acc = outData.Resize(0, (ULONG)ptr->size());
        for (ULONG i = 0; i < acc.GetCount(); i++) {
          acc[i].Set(ptr->get()[i].r, ptr->get()[i].g, ptr->get()[i].b,
                     ptr->get()[i].a);
        }
      }
      break;
