...
Abc::OCompoundProperty AlembicCamera::GetCompound() { return mCameraSchema; }
...
if (sampleInfo.alpha != 0.0) {
  ...
  lensSqueezeRatio = (1.0 - sampleInfo.alpha) * lensSqueezeRatio +
                     sampleInfo.alpha * sample.getLensSqueezeRatio();
  focalLength = (1.0 - sampleInfo.alpha) * focalLength +
                sampleInfo.alpha * sample.getFocalLength();
  xaperture = (1.0 - sampleInfo.alpha) * xaperture +
              sampleInfo.alpha * sample.getHorizontalAperture() / 2.54f;
  yaperture = (1.0 - sampleInfo.alpha) * yaperture +
              sampleInfo.alpha * sample.getVerticalAperture() / 2.54f;
  nearClipping = (1.0 - sampleInfo.alpha) * nearClipping +
                 sampleInfo.alpha * sample.getNearClippingPlane();
  farClipping = (1.0 - sampleInfo.alpha) * farClipping +
                sampleInfo.alpha * sample.getFarClippingPlane();
  interestDist = (1.0 - sampleInfo.alpha) * interestDist +
                 sampleInfo.alpha * sample.getFocusDistance();
}
...
// The correct way to handle lens squeeze ratio according to the article "Maya
// to Softimage: Camera Interoperability"
