...
// Assume that if we find a single back slash, it was not intended to be the
// escape character
...
CRefArray refs =
    Application().FindObjects("{72936430-9B0C-4167-8CA7-C30FC2188BB9}");
// compound ICE nodes    (custom only?     found Class IDs using View ->
// Scripting -> SDK Explorer)
refs += Application().FindObjects("{D33B77CB-8926-463D-B08B-8769A968E690}");
// ICE nodes             (custom only?     found Class IDs using View ->
// Scripting -> SDK Explorer)
...
for (LONG i = 0; i < refs.GetCount(); i++) {
  ...
  if (tree.IsValid()) {
    Parameter path = ICENodeInputPort(tree.GetInputPorts().GetItem(L"path"))
                         .GetParameters()[0];
  ...
  for (LONG i = 0; i < refs.GetCount(); i++) {
    ...
    if (tree.IsValid()) {
      ...
      if (op.IsValid()) {
        for (LONG j = 0; j < paramName.GetCount(); j++)
          values[j] =
              CString(op.GetParameterValue(paramName[j])).GetAsciiString();
      }
    }
...
if (pathMap.size() == 0 && identifierMap.size() == 0) {
  Application().LogMessage(
      L"[ExocortexAlembic] No alembic operators / icetrees found!",
      siWarningMsg);
  return CStatus::OK;
}
...
LONG identifierCount = 0;
{
  ...
  prop = (CustomProperty)Application().GetActiveSceneRoot().AddProperty(
      L"CustomProperty");
  ...
  if (pathMap.size() > 0) {
    prop.AddParameter(L"path_search", CValue::siString, siPersistable, L"", L"",
                      L"", L"", L"", L"", L"", oParam);
    prop.AddParameter(L"path_replace", CValue::siString, siPersistable, L"",
                      L"", L"", L"", L"", L"", L"", oParam);
    ...
    for (stringMapIt it = pathMap.begin(); it != pathMap.end(); it++) {
      prop.AddParameter(L"_path_" + CString(pathCount), CValue::siString,
                        siPersistable, L"", L"", it->second.c_str(), L"", L"",
                        L"", L"", oParam);
      layout.AddItem(L"_path_" + CString(pathCount))
          .PutAttribute(siUINoLabel, true);
      pathCount++;
    }
  }
  if (identifierMap.size() > 0) {
    prop.AddParameter(L"identifier_search", CValue::siString, siPersistable,
                      L"", L"", L"", L"", L"", L"", L"", oParam);
    prop.AddParameter(L"identifier_replace", CValue::siString, siPersistable,
                      L"", L"", L"", L"", L"", L"", L"", oParam);
    ...
    for (stringMapIt it = identifierMap.begin(); it != identifierMap.end();
         it++) {
      prop.AddParameter(L"_identifier_" + CString(identifierCount),
                        CValue::siString, siPersistable, L"", L"",
                        it->second.c_str(), L"", L"", L"", L"", oParam);
      layout.AddItem(L"_identifier_" + CString(identifierCount))
          .PutAttribute(siUINoLabel, true);
      identifierCount++;
    }
  }
  ...
  logic += L"  if(!search){\n";
  logic +=
      L"    LogMessage('[ExocortexAlembic] No search string "
      L"specified!',siWarningMsg);\n";
  logic += L"    return;\n";
  logic += L"  }\n";
...
if (!inspectResult) {
  ...
  for (stringMapIt it = identifierMap.begin(); it != identifierMap.end();
       it++) {
    CString value =
        prop.GetParameterValue(L"_identifier_" + CString(identifierCount++));
    it->second = value.GetAsciiString();
  }
