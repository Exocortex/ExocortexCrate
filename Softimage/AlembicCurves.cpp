    ...
    : AlembicObject(eNode, in_Job, oParent)
{
  ...
  mRadiusProperty = Abc::OFloatArrayProperty(
      mCurvesSchema.getArbGeomParams(), ".radius", mCurvesSchema.getMetaData(),
      GetJob()->GetAnimatedTs());
  mColorProperty = Abc::OC4fArrayProperty(mCurvesSchema.getArbGeomParams(),
                                          ".color", mCurvesSchema.getMetaData(),
                                          GetJob()->GetAnimatedTs());
  mFaceIndexProperty = Abc::OInt32ArrayProperty(
      mCurvesSchema.getArbGeomParams(), ".face_index",
      mCurvesSchema.getMetaData(), GetJob()->GetAnimatedTs());
  mVertexIndexProperty = Abc::OInt32ArrayProperty(
      mCurvesSchema.getArbGeomParams(), ".vertex_index",
      mCurvesSchema.getMetaData(), GetJob()->GetAnimatedTs());
  ...
  customAttributes.defineCustomAttributes(prim.GetGeometry(), argGeomParamsProp,
                                          mCurvesSchema.getMetaData(),
                                          GetJob()->GetAnimatedTs());
}

AlembicCurves::~AlembicCurves() {}
Abc::OCompoundProperty AlembicCurves::GetCompound() { return mCurvesSchema; }
XSI::CStatus AlembicCurves::Save(double time)
{
  ...
  if (prim.GetType().IsEqualNoCase(L"crvlist")) {
    ...
    // ESS_LOG_WARNING("posSize: "<<posVec.size());
    ...
    if (mNumSamples == 0) {
      ...
      Abc::Int32ArraySample nbVerticesSample(&mNbVertices.front(),
                                             mNbVertices.size());
      ...
      if (!mKnotVectorProperty.valid()) {
        mKnotVectorProperty = Abc::OFloatArrayProperty(
            mCurvesSchema.getArbGeomParams(), ".knot_vector",
            mCurvesSchema.getMetaData(), GetJob()->GetAnimatedTs());
      }
      ...
      mCurvesSample.setType(curveData.m_lDegree == 3 ? AbcG::kCubic
                                                     : AbcG::kLinear);
      mCurvesSample.setWrap(curveData.m_bClosed ? AbcG::kPeriodic
                                                : AbcG::kNonPeriodic);
...
#define CHUNK_SIZE 200000  // same chunck size used in Arnold
    ...
    for (LONG j = 0; j < numCurves; j++) {
      ...
      for (LONG i = 0; i < 14; i++, posIndex++, posVecIndex++) {
        if (globalSpace)
          pos[posIndex] =
              MapObjectPositionToWorldSpace(globalXfo, pos[posIndex]);
    ...
    // note that the root point of the guide curves is not part of the hair
    // geometry so
    // you have to go through the hairGen operators input ports, find the
    // connected cluster
    // and iterate the faces in the cluster. the guide hairs are order by
    // cloickwise first hit face vertex
    ...
    if (hairGenOpRef.GetAsText().IsEmpty()) {
      Application().LogMessage(
          L"Fatal error: The hair doesn't have a hairGenOperator!",
          siWarningMsg);
      return CStatus::Fail;
    }
    ...
    if (!SIObject(emitterPrimRef).GetType().IsEqualNoCase(L"polymsh")) {
      Application().LogMessage(
          L"Error: The hair needs to be emitted from a polygon", siWarningMsg);
      return CStatus::Fail;
    }
      ...
      for (long i = 0; i < clusterElements.GetCount(); i++) {
        ...
        facetIndex = Facet(emitterGeo.GetFacets().GetItem(clusterElements[i]))
                         .GetPoints()
                         .GetIndexArray();
    ...
    if (attr.IsDefined() && attr.IsValid()) {
      ...
      if (data.GetCount() > 0) {
        mCurvesSample.setCurvesNumVertices(
            Abc::Int32ArraySample(&mNbVertices.front(), mNbVertices.size()));
      }
      else {
      ...
      if (vertexCount > 0) {
        mCurvesSample.setPositions(
            Abc::P3fArraySample(&posVec.front(), posVec.size()));
      }
      else {
    ...
    if (vertexCount > 0) {
      ...
      if (attr.IsDefined() && attr.IsValid()) {
        ...
        mCurvesSample.setUVs(AbcG::OV2fGeomParam::Sample(
            Abc::V2fArraySample(&mUvVec.front(), mUvVec.size()),
            AbcG::kVertexScope));
      }
      else {
      ...
      if (attr.IsDefined() && attr.IsValid()) {
        ...
        mCurvesSample.setVelocities(
            Abc::V3fArraySample(&mVelVec.front(), mVelVec.size()));
      }
      else {
...
if (sampleInfo.alpha != 0.0) {
  ...
  for (size_t i = 0, index = 0; i < xsiSize; i++, index++) {
    ...
    outputPos[(LONG)i].LinearlyInterpolate(
        outputPos[(LONG)i],
        CVector3(curvePos->get()[index].x, curvePos->get()[index].y,
                 curvePos->get()[index].z),
        sampleInfo.alpha);
  }
}
...
//   ESS_LOG_WARNING(""<<i<<": "<<outputPos[i].GetX()<<",
//   "<<outputPos[i].GetY()<<"< "<<outputPos[i].GetZ());
//}

Primitive(ctxt.GetOutputTarget())
    .GetGeometry()
    .GetPoints()
    .PutPositionArray(outputPos);
...
{
  ICENodeDef nodeDef;
  nodeDef = Application().GetFactory().CreateICENodeDef(L"alembic_curves",
                                                        L"alembic_curves");
  ...
  // Add output ports.
  st =
      nodeDef.AddOutputPort(ID_OUT_count, siICENodeDataLong,
                            siICENodeStructureSingle, siICENodeContextSingleton,
                            L"count", L"count", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_vertices, siICENodeDataLong,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"vertices", L"vertices", ID_UNDEF, ID_UNDEF,
                             ID_UNDEF);
  st.AssertSucceeded();
  st =
      nodeDef.AddOutputPort(ID_OUT_offset, siICENodeDataLong,
                            siICENodeStructureArray, siICENodeContextSingleton,
                            L"offset", L"offset", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  ...
  st.AssertSucceeded();
  st =
      nodeDef.AddOutputPort(ID_OUT_radius, siICENodeDataFloat,
                            siICENodeStructureArray, siICENodeContextSingleton,
                            L"radius", L"radius", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_color, siICENodeDataColor4,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"color", L"color", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_uv, siICENodeDataVector2,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"uv", L"uv", ID_UNDEF, ID_UNDEF, ID_UNDEF);
  st.AssertSucceeded();
  ...
  switch (out_portID) {
    ...
    case ID_OUT_position: {
      ...
      if (ptr == NULL)
        ...
        if (sampleInfo.alpha != 0.0) {
          Abc::P3fArraySamplePtr ptr2 =
              obj.getSchema().getValue(sampleInfo.ceilIndex).getPositions();
          float alpha = (float)sampleInfo.alpha;
          float ialpha = 1.0f - alpha;
          if (ptr2->size() == ptr->size()) {
            for (ULONG i = 0; i < acc.GetCount(); i++) {
              acc[i].Set(ialpha * ptr->get()[i].x + alpha * ptr2->get()[i].x,
            ...
            if (vel->size() == ptr->size()) {
              float timeAlpha =
                  getTimeOffsetFromSchema(obj.getSchema(), sampleInfo);
              for (ULONG i = 0; i < acc.GetCount(); i++) {
                acc[i].Set(ptr->get()[i].x + timeAlpha * vel->get()[i].x,
    ...
    case ID_OUT_radius: {
      ...
      Abc::IFloatArrayProperty prop =
          Abc::IFloatArrayProperty(obj.getSchema(), ".radius");
      if (!prop.valid()) {
        acc = outData.Resize(0, 0);
        return CStatus::OK;
      }
    ...
    case ID_OUT_color: {
      ...
      Abc::IC4fArrayProperty prop =
          Abc::IC4fArrayProperty(obj.getSchema(), ".color");
      if (!prop.valid()) {
        acc = outData.Resize(0, 0);
        return CStatus::OK;
      }
...
if (!validateCurveData(curvePos, curveNbVertices, pOrders, pKnotVec,
                       curveSample.getType())) {
  Application().LogMessage(L"[ExocortexAlembic] Skipping curve '" + identifier +
                               L"', invalid curve type.",
                           siWarningMsg);
  return CStatus::Fail;
}
...
for (size_t j = 0; j < curveNbVertices->size(); j++) {
  ...
  if (nbVertices == 0) {
    Application().LogMessage(
        L"[ExocortexAlembic] Softimage does not support 0 size curves in a "
        L"curve set, ignoring but this may introduce errors on animation.");
    continue;
  }
  ...
  if (nDegree != 1 && nDegree != 3) {
    Application().LogMessage(
        L"[ExocortexAlembic] Skipping curve with unsupported degree.");
    continue;
  }
  ...
  curveData.m_lDegree =
      nDegree;  // curveSample.getType() ==AbcG::kLinear ? 1 : 3;

  for (LONG k = 0; k < nbVertices; k++) {
    curveData.m_aControlPoints[k].Set(curvePos->get()[offset].x,
                                      curvePos->get()[offset].y,
                                      curvePos->get()[offset].z, 1.0);
    offset++;
  }

  if (pKnotVec) {
    curveData.m_aKnots.Resize(nbVertices + nDegree - 1);
    for (LONG k = 0; k < curveData.m_aKnots.GetCount() && k < pKnotVec->size();
         k++) {
