...
SICALLBACK XSILoadPlugin_2(PluginRegistrar& in_reg)
{
  ...
  in_reg.RegisterMenu(siMenuMainFileProjectID, L"exocortex_RunTest", false,
                      false);
...
int CreateSubTreeWithNopCubes(CRef& parentNode, CString& nodeName,
                              CString& path, CString& identifier, int subLevels,
                              int childrenPerLevel, int operatorsPerNode)
{
  ...
  for (int i = 0; i < operatorsPerNode; i++) {
    CustomOperator op =
        Application().GetFactory().CreateObject("exocortex_nop");
  ...
  if (subLevels > 0) {
    for (int i = 0; i < childrenPerLevel; i++) {
      ...
      numNodes += CreateSubTreeWithNopCubes(newNode, childNodeName, path,
                                            identifier, subLevels - 1,
                                            childrenPerLevel, operatorsPerNode);
    }
  }
...
SICALLBACK exocortex_run_test_Execute(CRef& in_ctxt)
{
  ...
  int numNodes =
      CreateSubTreeWithNopCubes(sceneRoot, name, path, identifier, 4, 5, 2);

  time(&end);
  double dif = difftime(end, start);

  std::stringstream s;
  s << "new nodes:" << numNodes << " total seconds:" << dif
    << " new nodes/sec:" << (numNodes / dif);
...
SICALLBACK exocortex_nop_Define(CRef& in_ctxt)
{
  ...
  CRef oPDef0 = oFactory.CreateParamDef(L"muted", CValue::siBool,
                                        siAnimatable | siPersistable, L"muted",
                                        L"muted", 0, 0, 1, 0, 1);
  CRef oPDef1 = oFactory.CreateParamDef(L"time", CValue::siFloat,
                                        siAnimatable | siPersistable, L"time",
                                        L"time", 1, -100000, 100000, 0, 1);
  CRef oPDef2 = oFactory.CreateParamDef(L"path", CValue::siString,
                                        siReadOnly | siPersistable, L"path",
                                        L"path", L"", L"", L"", L"", L"");
  CRef oPDef3 = oFactory.CreateParamDef(
      L"identifier", CValue::siString, siReadOnly | siPersistable,
      L"identifier", L"identifier", L"", L"", L"", L"", L"");
  CRef oPDef4 = oFactory.CreateParamDef(
      L"renderpath", CValue::siString, siReadOnly | siPersistable,
      L"renderpath", L"renderpath", L"", L"", L"", L"", L"");
  CRef oPDef5 = oFactory.CreateParamDef(
      L"renderidentifier", CValue::siString, siReadOnly | siPersistable,
      L"renderidentifier", L"renderidentifier", L"", L"", L"", L"", L"");
