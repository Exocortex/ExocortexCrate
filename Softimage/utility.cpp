...
std::string getIdentifierFromRef(CRef in_Ref, bool includeHierarchy)
{
  ...
  while (ref.IsValid()) {
    ...
    if (model.IsValid()) {
      ...
      if (!has3DObj) {
        result = std::string("/") +
                 std::string(obj.GetName().GetAsciiString()) + result;
        has3DObj = true;
      }
      result =
          std::string("/") +
...
CString getFullNameFromIdentifier(XSI::CRef importRootNode,
                                  std::string in_Identifier, bool bMergedLeaf)
{
...
// CRef getRefFromIdentifier(XSI::CRef importRootNode, std::string
...
CRefArray getOperators(CRef in_Ref)
{
  ...
  CComAPIHandler constructionHistory =
      comPrimitive.GetProperty(L"ConstructionHistory");
...
{
  gIsRefAnimatedMap.insert(std::pair<std::string, bool>(
      in_Ref.GetAsText().GetAsciiString(), animated));
  return animated;
}
...
CStatus alembicOp_Define(CRef& in_ctxt)
{
  ...
  oPDef = oFactory.CreateParamDef(L"muted", CValue::siBool,
                                  siAnimatable | siPersistable, L"muted",
                                  L"muted", 0, 0, 1, 0, 1);
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"time", CValue::siFloat,
                                  siAnimatable | siPersistable, L"time",
                                  L"time", 1, -100000, 100000, 0, 1);
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"path", CValue::siString, siPersistable,
                                  L"path", L"path", L"", L"", L"", L"", L"");
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"multifile", CValue::siBool, siPersistable,
                                  L"multifile", L"multifile", 0, 0, 1, 0, 1);
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"identifier", CValue::siString,
                                  siPersistable, L"identifier", L"identifier",
                                  L"", L"", L"", L"", L"");
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"renderpath", CValue::siString,
                                  siPersistable, L"renderpath", L"renderpath",
                                  L"", L"", L"", L"", L"");
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"renderidentifier", CValue::siString,
                                  siPersistable, L"renderidentifier",
                                  L"renderidentifier", L"", L"", L"", L"", L"");
  oCustomOperator.AddParameter(oPDef, oParam);

  // Info parameters
  oPDef = oFactory.CreateParamDef(
      L"startTime", CValue::siDouble, siReadOnly | siPersistable, L"StartTime",
      L"", 0.0, -std::numeric_limits<float>::max(),
      std::numeric_limits<float>::max(), -std::numeric_limits<float>::max(),
      std::numeric_limits<float>::max());
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(
      L"endTime", CValue::siDouble, siReadOnly | siPersistable, L"EndTime", L"",
      0.0, -std::numeric_limits<float>::max(),
      std::numeric_limits<float>::max(), -std::numeric_limits<float>::max(),
      std::numeric_limits<float>::max());
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(
      L"sampleRate", CValue::siDouble, siReadOnly | siPersistable,
      L"SampleRate", L"", 1.0, -std::numeric_limits<float>::max(),
      std::numeric_limits<float>::max(), -std::numeric_limits<float>::max(),
      std::numeric_limits<float>::max());
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"numPointsPrimitive", CValue::siInt4,
                                  siReadOnly | siPersistable, L"NumPointsGeo",
                                  L"", 0, 0, std::numeric_limits<int>::max(), 0,
                                  std::numeric_limits<int>::max());
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"numPointsCache", CValue::siInt4,
                                  siReadOnly | siPersistable, L"NumPointsCache",
                                  L"", 0, 0, std::numeric_limits<int>::max(), 0,
                                  std::numeric_limits<int>::max());
  oCustomOperator.AddParameter(oPDef, oParam);
  oPDef = oFactory.CreateParamDef(L"numSamples", CValue::siInt4,
                                  siReadOnly | siPersistable, L"NumSamples",
                                  L"", 0, 0, std::numeric_limits<int>::max(), 0,
                                  std::numeric_limits<int>::max());
  oCustomOperator.AddParameter(oPDef, oParam);
...
CStatus alembicOp_DefineLayout(CRef& in_ctxt)
{
  ...
  oLayout.AddItem(L"numPointsPrimitive", L"Primitive")
      .PutAttribute(siUINoSlider, true);
...
CStatus alembicOp_TimeSamplingInit(
    CRef& in_ctxt)  //, AbcA::TimeSamplingPtr timeSampling )
{
  ESS_PROFILE_FUNC();
  ...
  // p->timeSampling =  AbcA::TimeSamplingPtr(new
...
CStatus alembicOp_getFrameNum(CRef& in_ctxt, double sampleTime, int& frameNum)
{
  ...
  if (p->timeSampling->getTimeSamplingType().isUniform()) {
    ...
    // However, it safe to call this function with a large number if the
    // sampling type is uniform
...
CString replaceNumber(CString pathstr, int frameNum)
{
  ...
  std::stringstream ss;
  ss << path.substr(0, nStart) << std::setfill('0') << std::setw(nNumLen)
     << frameNum << path.substr(nEnd);

  return CString(ss.str().c_str());
}

CStatus alembicOp_Multifile(CRef& in_ctxt, bool multifile, double time,
                            XSI::CString& path)
{
  ESS_PROFILE_FUNC();
...
//
// Two legal file locations are checked (in relation to the Alembic install
// dir):
//
      ...
      if (path.FindString(L"alembic") != ULONG_MAX) {
        ...
        for (LONG k = 0; k < aLegalBaseNames.GetCount(); k++) {
          for (LONG m = 0; m < aLegalPaths.GetCount(); m++) {
            path =
                CUtils::BuildPath(aLegalPaths[m], aLegalBaseNames[k]) + suffix;
...
{
  return Abc::M44f(
      (float)m.GetValue(0, 0), (float)m.GetValue(0, 1), (float)m.GetValue(0, 2),
      (float)m.GetValue(0, 3), (float)m.GetValue(1, 0), (float)m.GetValue(1, 1),
      (float)m.GetValue(1, 2), (float)m.GetValue(1, 3), (float)m.GetValue(2, 0),
      (float)m.GetValue(2, 1), (float)m.GetValue(2, 2), (float)m.GetValue(2, 3),
      (float)m.GetValue(3, 0), (float)m.GetValue(3, 1), (float)m.GetValue(3, 2),
      (float)m.GetValue(3, 3));
}
