...
AlembicModel::AlembicModel(SceneNodePtr eNode, AlembicWriteJob* in_Job,
                           Abc::OObject oParent)
    : AlembicObject(eNode, in_Job, oParent)
{
  ...
  std::string uniqueName = eNode->name;

  AbcG::OXform xform(GetMyParent(), uniqueName, GetJob()->GetAnimatedTs());

  eNode->name =
      uniqueName;  // need to this so that the correct parent is retrived later
...
Abc::OCompoundProperty AlembicModel::GetCompound() { return mXformSchema; }
XSI::CStatus AlembicModel::Save(double time)
{
  ...
  for (std::list<SceneNodePtr>::iterator it = mExoSceneNode->children.begin();
       it != mExoSceneNode->children.end(); it++) {
  ...
  // store the transform
  SaveXformSample(mExoSceneNode, mXformSchema, mXformSample, time, bTransCache,
                  bGlobalSpace, bFlatten);

  if (mNumSamples == 0) {
    if (!mXformXSINodeType.valid()) {
      mXformXSINodeType = Abc::OUcharProperty(
          mXformSchema.getArbGeomParams(), ".xsiNodeType",
          mXformSchema.getMetaData(), GetJob()->GetAnimatedTs());
    }
  ...
  //   mOVisibility.set(visibility ?AbcG::kVisibilityVisible
