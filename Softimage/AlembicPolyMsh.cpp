...
CVector3 V3fToVector3(const Abc::V3f& v) { return CVector3(v.x, v.y, v.z); }
// TODO: should probably just using stl-style options everywhere
void CopyOptions(std::map<XSI::CString, XSI::CValue>& options_in,
                 CommonOptions& options_out)
{
  for (std::map<XSI::CString, XSI::CValue>::iterator it = options_in.begin();
       it != options_in.end(); it++) {
...
XSI::CStatus AlembicPolyMesh::Save(double time)
{
  ...
  if (options.GetBoolOption("exportFaceSets") &&
      mNumSamples !=
          0) {  // turn off faceset exports for all frames except the first
  ...
  if (mExoSceneNode->type == SceneNode::POLYMESH_SUBTREE) {
    ...
    mergePolyMeshSubtreeNode<IntermediatePolyMeshXSI>(meshSubtreeNode,
                                                      finalMesh, options, time);
  }
  else {
  ...
  //{
  //   if(mFaceCountVec.size() != faceCount || mFaceIndicesVec.size() !=
  //   sampleCount){
  ...
  if (!mFaceVaryingInterpolateBoundaryProperty) {
    mFaceVaryingInterpolateBoundaryProperty = Abc::OInt32Property(
        mMeshSchema, ".faceVaryingInterpolateBoundary",
...
SampleInfo sampleInfo =
    getSampleInfo(ctxt.GetParameterValue(L"time"), timeSampling, nSamples);
...
if (sampleInfo.alpha != 0.0) {
  ...
  for (size_t i = 0; i < meshPos->size(); i++)
    pos[(LONG)i].LinearlyInterpolate(
        pos[(LONG)i],
        CVector3(meshPos->get()[i].x, meshPos->get()[i].y, meshPos->get()[i].z),
        sampleInfo.alpha);
}
...
CGeometryAccessor accessor =
    mesh.GetGeometryAccessor(siConstructionModeModeling);
CLongArray counts;
...
if (meshNormalsParam.valid()) {
  Abc::N3fArraySamplePtr meshNormals =
      meshNormalsParam.getExpandedValue(sampleInfo.floorIndex).getVals();

  Operator op(ctxt.GetSource());
  updateOperatorInfo(op, sampleInfo, obj.getSchema().getTimeSampling(),
                     (int)normalValues.GetCount(),
                     (int)meshNormals->size() * 3);
  ...
  if (meshNormals->size() * 3 == normalValues.GetCount()) {
    ...
    if (sampleInfo.alpha != 0.0 /*&& !isAlembicMeshTopology(&iObj)*/) {
      meshNormals =
          meshNormalsParam.getExpandedValue(sampleInfo.ceilIndex).getVals();
      if (meshNormals->size() == normalValues.GetCount() / 3) {
        ...
        for (LONG i = 0; i < counts.GetCount(); i++) {
          for (LONG j = counts[i] - 1; j >= 0; j--) {
            CVector3 normal(normalValues[offsetOut],
                            normalValues[offsetOut + 1],
                            normalValues[offsetOut + 2]);
            normal.LinearlyInterpolate(
                normal, CVector3(meshNormals->get()[offsetIn + j].x,
                                 meshNormals->get()[offsetIn + j].y,
                                 meshNormals->get()[offsetIn + j].z),
                sampleInfo.alpha);
...
if (meshUvParam.valid()) {
  SampleInfo sampleInfo =
      getSampleInfo(ctxt.GetParameterValue(L"time"),
                    meshUvParam.getTimeSampling(), meshUvParam.getNumSamples());

  Abc::V2fArraySamplePtr meshUVs =
      meshUvParam.getExpandedValue(sampleInfo.floorIndex).getVals();
  ...
  if (meshUVs->size() * 3 == uvValues.GetCount()) {
    ...
    if (sampleInfo.alpha != 0.0) {
      ...
      offset = 0;
      for (LONG i = 0; i < sampleLookup.GetCount(); i++) {
        uvValues[offset] = uvValues[offset] * ialpha +
                           meshUVs->get()[sampleLookup[i]].x * sampleInfo.alpha;
        offset++;
        uvValues[offset] = uvValues[offset] * ialpha +
                           meshUVs->get()[sampleLookup[i]].y * sampleInfo.alpha;
        offset++;
        uvValues[offset] = 0.0;
        offset++;
      }
...
Operator op(ctxt.GetSource());
updateOperatorInfo(op, sampleInfo, timeSampling, (int)meshPos->size(),
                   (int)meshPos->size());
...

{
  ...
  for (size_t j = 0; j < meshPos->size(); j++) {
    pos[(LONG)j].Set(meshPos->get()[j].x, meshPos->get()[j].y,
                     meshPos->get()[j].z);
  }
}
...
if (meshFaceCount->size() > 0) {
  ...
  if (meshFaceCount->get()[0] == 0) {
    ...
    for (size_t j = 0; j < meshFaceCount->size(); j++) {
      ...
      // ESS_LOG_INFO("meshFaceIndices->size(): " << (unsigned
      // int)meshFaceIndices->size());
...
oPDef = oFactory.CreateParamDef(
    L"extend", CValue::siFloat, siAnimatable | siPersistable, L"extend",
    L"extend", 0.0f, -10000.0f, 10000.0f, 0.0f, 10.0f);
...
Operator op(ctxt.GetSource());
updateOperatorInfo(op, sampleInfo, timeSampling, pos.GetCount(),
                   pos.GetCount());
  ...
  switch (out_portID) {
    case ID_OUT_position: {
      ...
      if (ptr == NULL || ptr->size() == 0 ||
        ...
        if (sampleInfo.alpha != 0.0 && usevel) {
          const float alpha =
              (float)
                  sampleInfo.alpha;  // shouldn't we be using a time alpha here?
          ...
          if (velPtr == NULL || velPtr->size() == 0)
            ...
            for (ULONG i = 0; i < acc.GetCount(); ++i) {
              ...
              acc[i].Set(pt.x + alpha * vl.x, pt.y + alpha * vl.y,
                         pt.z + alpha * vl.z);
            }
            done = true;
          }
    ...
    case ID_OUT_faceIndices: {
      ...
      if (useMesh) {
        AbcG::IPolyMeshSchema::Sample sample;
        objMesh.getSchema().get(sample, sampleInfo.floorIndex);
        ptr = (out_portID == ID_OUT_faceCounts) ? sample.getFaceCounts()
...
XSI::CStatus Register_alembic_polyMesh(XSI::PluginRegistrar& in_reg)
{
  ...
  ICENodeDef nodeDef = Application().GetFactory().CreateICENodeDef(
      L"alembic_polyMesh2", L"alembic_polyMesh2");
  ...
  st = nodeDef.AddOutputPort(ID_OUT_velocity, siICENodeDataVector3,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"velocity", L"velocity");
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_faceCounts, siICENodeDataLong,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"faceCounts", L"faceCounts");
  st.AssertSucceeded();
  st = nodeDef.AddOutputPort(ID_OUT_faceIndices, siICENodeDataLong,
                             siICENodeStructureArray, siICENodeContextSingleton,
                             L"faceIndices", L"faceIndices");
  st.AssertSucceeded();
