...
struct nodeData {
  ...
  bool shifted, createdShifted, isPolyMeshNode;
...
AtVoid ReadParameterValue(AtNode* curve_node, FILE* fp,
                          const AtChar* param_name);

std::string getNameFromIdentifier(const std::string& identifier, long id = -1,
                                  long group = -1);

// executed only if (nodata.shifted), same code in multiple places, in a single
// unique function!
