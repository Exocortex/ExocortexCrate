#ifndef __ALEMBIC_NAMES_H__
#define __ALEMBIC_NAMES_H__

#define EXOCORTEX_ALEMBIC_CATEGORY _T("Alembic")

//#define EXOCORTEX_ALEMBIC_MESH_GEOMETRY _T("EA Mesh")
//#define EXOCORTEX_ALEMBIC_SIMPLE_PARTICLES _T("EA Particles")
//#define EXOCORTEX_ALEMBIC_POINT_CACHE_MODIFIER _T("EA Point Cache")

// MAX SCRIPT
#define ALEMBIC_MESH_GEOMETRY_CLASSID		Class_ID(0xe855567c, 0xbcd73b8b)
#define ALEMBIC_POINT_CACHE_MODIFIER_CLASSID	Class_ID(0x20730623, 0x5d650bdb)


#define ALEMBIC_CAMERA_MODIFIER_CLASSID		Class_ID(0x115150b9, 0x5b6d187a)
#define ALEMBIC_SIMPLE_SPLINE_CLASSID		Class_ID(0xa720ab9, 0x7efc73de)
#define ALEMBIC_MESH_MODIFIER_CLASSID		Class_ID(0x45365062, 0x60984267)
#define ALEMBIC_VIS_CTRL_CLASSID			Class_ID(0x27e512c0, 0x389b3971)
#define ALEMBIC_SIMPLE_PARTICLE_CLASSID		Class_ID(0x246b1c1e, 0x3b2f032f)
#define ALEMBIC_XFORM_CTRL_CLASSID			Class_ID(0x9bf4cdb, 0xe29448e)
#define ALEMBIC_EXPORTER_CLASSID	Class_ID(0x79d613a4, 0x4f21c3ad)

#endif //__ALEMBIC_NAMES_H__
