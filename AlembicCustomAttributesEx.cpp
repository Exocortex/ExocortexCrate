   ...
   if(exportAttr.IsDefined() && exportAttr.IsValid()){
      ...
      for(LONG i=0; i<exportArray.GetCount(); i++){
         ...
         if(!attr.IsDefined() || !attr.IsValid()){
            ESS_LOG_WARNING("Could not find attribute: "<<exportArray[i].GetAsciiString());
            continue;
         }
         ...
         if(attr.GetDataType() == siICENodeDataString && attr.GetStructureType() == siICENodeStructureSingle){
            //ESS_LOG_WARNING("Defining siICENodeDataString siICENodeStructureSingle");
            ...
            //ESS_LOG_WARNING("Defining siICENodeDataFloat siICENodeStructureSingle");
...
XSI::CStatus AlembicCustomAttributesEx::exportCustomAttributes(XSI::Geometry geo)
{
   for(propMap::iterator it = customProps.begin(); it != customProps.end(); it++){
      ...
      if(attr.GetDataType() == siICENodeDataString && attr.GetStructureType() == siICENodeStructureSingle){
         //ESS_LOG_WARNING("Writing siICENodeDataString siICENodeStructureSingle");
         ...
         //ESS_LOG_WARNING("Writing siICENodeDataVector3 siICENodeStructureSingle");
         ...
         //      outputVecArray.push_back( Abc::V3f( vecArraySubData[0].GetX(), vecArraySubData[0].GetY(), vecArraySubData[0].GetZ() ) );
         //   }
