...
{
  return &AlembicMeshTopoModifierDesc;
}

//--- Properties block -------------------------------

// IMPORTANT: increment this value if you have added new fields to the param
// block
static const int ALEMBIC_MESH_TOPO_MODIFIER_VERSION = 2;

static ParamBlockDesc2 AlembicMeshTopoModifierParams(
    0, _T(ALEMBIC_MESH_TOPO_MODIFIER_SCRIPTNAME), 0,
    GetAlembicMeshTopoModifierClassDesc(),
    P_AUTO_CONSTRUCT | P_AUTO_UI | P_VERSION,
    ALEMBIC_MESH_TOPO_MODIFIER_VERSION, 0,

    // rollout description
    IDD_ALEMBIC_MESH_TOPO_PARAMS, IDS_ALEMBIC, 0, 0, NULL,

    // params
    AlembicMeshTopoModifier::ID_PATH, _T("path"), TYPE_FILENAME,
    P_RESET_DEFAULT, IDS_PATH, p_default, "", p_ui, TYPE_EDITBOX, IDC_PATH_EDIT,
    p_assetTypeID, kExternalLink, p_end,

    AlembicMeshTopoModifier::ID_IDENTIFIER, _T("identifier"), TYPE_STRING,
    P_RESET_DEFAULT, IDS_IDENTIFIER, p_default, "", p_ui, TYPE_EDITBOX,
    IDC_IDENTIFIER_EDIT, p_end,

    AlembicMeshTopoModifier::ID_TIME, _T("time"), TYPE_FLOAT, P_ANIMATABLE,
    IDS_TIME, p_default, 0.0f, p_range, 0.0f, 1000.0f, p_ui, TYPE_SPINNER,
    EDITTYPE_FLOAT, IDC_TIME_EDIT, IDC_TIME_SPIN, 0.01f, p_end,

    AlembicMeshTopoModifier::ID_GEOMETRY, _T("geometry"), TYPE_BOOL,
    P_ANIMATABLE, IDS_GEOMETRY, p_default, FALSE, p_end,

    AlembicMeshTopoModifier::ID_NORMALS, _T("normals"), TYPE_BOOL, P_ANIMATABLE,
    IDS_NORMALS, p_default, FALSE, p_end,

    AlembicMeshTopoModifier::ID_UVS, _T("uvs"), TYPE_BOOL, P_ANIMATABLE,
    IDS_UVS, p_default, FALSE, p_end,

    AlembicMeshTopoModifier::ID_MUTED, _T("muted"), TYPE_BOOL, P_ANIMATABLE,
    IDS_MUTED, p_default, TRUE, p_ui, TYPE_SINGLECHEKBOX, IDC_MUTED_CHECKBOX,
    p_end,

    AlembicMeshTopoModifier::ID_IGNORE_SUBFRAME_SAMPLES,
    _T("Ignore subframe samples"), TYPE_BOOL, P_ANIMATABLE,
    IDS_IGNORE_SUBFRAME_SAMPLES, p_default, FALSE, p_ui, TYPE_SINGLECHEKBOX,
    IDC_IGNORE_SUBFRAME_SAMPLES, p_end,

    AlembicMeshTopoModifier::ID_MATERIAL_IDS, _T("materialIDs"), TYPE_BOOL,
    P_ANIMATABLE, IDS_MATERIAL_IDS, p_default, FALSE, p_end,
...
void AlembicMeshTopoModifier::EnumAuxFiles(AssetEnumCallback &nameEnum,
                                           DWORD flags)
{
...
void AlembicMeshTopoModifier::ModifyObject(TimeValue t, ModContext &mc,
                                           ObjectState *os, INode *node)
{
  ...
  this->pblock->GetValue(AlembicMeshTopoModifier::ID_PATH, t, strPath,
                         interval);

  MCHAR const *strIdentifier = NULL;
  this->pblock->GetValue(AlembicMeshTopoModifier::ID_IDENTIFIER, t,
  ...
  BOOL bGeometry;
  this->pblock->GetValue(AlembicMeshTopoModifier::ID_GEOMETRY, t, bGeometry,
                         interval);

  BOOL bNormals;
  this->pblock->GetValue(AlembicMeshTopoModifier::ID_NORMALS, t, bNormals,
                         interval);
  ...
  BOOL bMaterialIDs;
  this->pblock->GetValue(AlembicMeshTopoModifier::ID_MATERIAL_IDS, t,
                         bMaterialIDs, interval);

  BOOL bMuted;
  this->pblock->GetValue(AlembicMeshTopoModifier::ID_MUTED, t, bMuted,
                         interval);

  BOOL bIgnoreSubframeSamples;
  this->pblock->GetValue(AlembicMeshTopoModifier::ID_IGNORE_SUBFRAME_SAMPLES, t,
                         bIgnoreSubframeSamples, interval);

  // ESS_LOG_INFO( "AlembicMeshTopoModifier::ModifyObject strPath: " << strPath
...
RefResult AlembicMeshTopoModifier::NotifyRefChanged(const Interval &changeInt,
...
{
#else
RefResult AlembicMeshTopoModifier::NotifyRefChanged(Interval changeInt,
                                                    RefTargetHandle hTarget,
                                                    PartID &partID,
                                                    RefMessage message)
{
      ...
      if (hTarget == pblock) {
        ...
        switch (changing_param) {
          case ID_PATH: {
            ...
            Interval v;
            pblock->GetValue(AlembicMeshTopoModifier::ID_PATH, t, strPath, v);
...
void AlembicMeshTopoModifier::BeginEditParams(IObjParam *ip, ULONG flags,
                                              Animatable *prev)
{
  this->ip = ip;
  editMod = this;
...
void AlembicMeshTopoModifier::EndEditParams(IObjParam *ip, ULONG flags,
                                            Animatable *next)
{
