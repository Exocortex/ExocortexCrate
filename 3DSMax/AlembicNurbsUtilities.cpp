...
bool isAlembicNurbsCurveAnimated(AbcG::IObject *pIObj)
{
  ...
  if (AbcG::ICurves::matches((*pIObj).getMetaData())) {
    AbcG::ICurves objCurves = AbcG::ICurves(*pIObj, Abc::kWrapExisting);
    AbcG::ICurvesSchema cSchema = objCurves.getSchema();
    return !cSchema.getPositionsProperty().isConstant() ||
           !cSchema.getNumVerticesProperty().isConstant() ||
           (cSchema.getVelocitiesProperty() &&
            !cSchema.getVelocitiesProperty().isConstant()) ||
           (cSchema.getUVsParam() && !cSchema.getUVsParam().isConstant()) ||
           (cSchema.getNormalsParam() &&
            !cSchema.getNormalsParam().isConstant()) ||
...
bool LoadNurbs(NURBSSet &nset, Abc::P3fArraySamplePtr pCurvePos,
               ...
               AbcG::CurvePeriodicity wrap, TimeValue time,
               bool bLoadPositions = true)
{
  ESS_PROFILE_FUNC();

  if (!validateCurveData(pCurvePos, pCurveNbVertices, pOrders, pKnotVec,
                         type)) {
    return false;
  }
  ...
  for (size_t j = 0; j < pCurveNbVertices->size(); j++) {
    ...
    if (pKnotVec) {
      ...
      c->SetKnot(nNumMaxKnots - 1,
                 pKnotVec->get()[abcKnotOffset + nNumAbcKnots - 1]);
...
bool ModifyNurbs(Object *pObj, Abc::P3fArraySamplePtr pCurvePos,
                 Abc::Int32ArraySamplePtr pCurveNbVertices,
                 Abc::FloatArraySamplePtr pKnotVec, AbcG::CurveType type,
                 AbcG::CurvePeriodicity wrap, TimeValue time)
{
...
void AlembicImport_LoadNURBS_Internal(alembic_NURBSload_options &options)
{
  ...
  if (options.pObject && options.pObject->ClassID() == EDITABLE_SURF_CLASS_ID) {
    ...
    if (bDynamicTopo) {
      LoadNurbs(nset, pCurvePos, pCurveNbVertices, pOrders, pKnotVec,
                /*AbcG::kLinear, AbcG::kNonPeriodic*/ curveSample.getType(),
                curveSample.getWrap(), nTicks);

      Matrix3 mat(1);
      Object *newObject =
          CreateNURBSObject((IObjParam *)GetCOREInterface(), &nset, mat);
      ...
      ModifyNurbs(options.pObject, pCurvePos, pCurveNbVertices, pKnotVec,
                  curveSample.getType(), curveSample.getWrap(), nTicks);
    }
  }
}
                        ...
                        alembic_importoptions &options, INode **pMaxNode)
{
  ...
  bool isAnimated = isAlembicNurbsCurveAnimated(&iObj);
  {
    ...
    if (bReplaceExistingModifiers) {
      pModifier = FindModifier(pNode, ALEMBIC_NURBS_MODIFIER_CLASSID,
                               identifier.c_str());
    }
    if (!pModifier) {
      pModifier = static_cast<Modifier *>(GET_MAX_INTERFACE()->CreateInstance(
          OSM_CLASS_ID, ALEMBIC_NURBS_MODIFIER_CLASSID));
