...
struct matShader {
  ...
  matShader(std::string& target, std::string& type, std::string& name)
...
enum enumt { AREA_LIGHT, AMBIENT_LIGHT, NUM_INPUT_LIGHT_TYPES };
};

InputLightType::enumt readShader(AbcM::IMaterialSchema& matSchema,
                                 std::vector<matShader>& shaders)
{
  ...
  for (int j = 0; j < targetNames.size(); j++) {
    ...
    for (int k = 0; k < shaderTypeNames.size(); k++) {
      ...
      if (shaderName.find("rect") != std::string::npos ||
          shaderName.find("area") != std::string::npos) {
      ...
      shaders.push_back(
          matShader(targetNames[j], shaderTypeNames[k], shaderName));

      // ESS_LOG_WARNING("shaderName: "<<shaderName);

      Abc::ICompoundProperty propk =
          matSchema.getShaderParameters(targetNames[j], shaderTypeNames[k]);

      // ESS_LOG_WARNING("propertyName: "<<propk.getName());

      shaders.back().props.push_back(
          AbcProp(std::string("targetName"), targetNames[j], 1000000000));
      shaders.back().props.push_back(
          AbcProp(std::string("shaderType"), shaderTypeNames[j], 1000000000));
...
int AlembicImport_Light(const std::string& path, AbcG::IObject& iObj,
                        alembic_importoptions& options, INode** pMaxNode)
{
  ...
  if (options.attachToExisting) {
    ESS_LOG_WARNING(
        "Attach to existing for lights is not yet supported. Could not attach "
        << iObj.getFullName());
  ...
  if (lightType == InputLightType::AMBIENT_LIGHT) {
    nodeRes = createNode(iObj, LIGHT_CLASS_ID, Class_ID(OMNI_LIGHT_CLASS_ID, 0),
                         pMaxNode, bReplaceExisting);

    // Modifier* pModifier = FindModifier(*pMaxNode,
    ...
    // pMaxNode>GetParamBlockByID( 0 )->SetValue( GetParamIdByName( pModifier,
    // 0, "muted" ), zero, FALSE );

    GET_MAX_INTERFACE()->SelectNode(*pMaxNode);

    // set the ambient check box, intensity controller, and light colour
    // controller (not sure how to this in C++)
    ...
    for (int s = 0; s < shaders.size(); s++) {
      ...
      for (int i = 0; i < shaders[s].props.size(); i++) {
        ...
        const AbcA::DataType& datatype =
            shaders[s].props[i].propHeader.getDataType();
        const AbcA::MetaData& metadata =
            shaders[s].props[i].propHeader.getMetaData();
        ...
        if (datatype.getPod() == AbcA::kFloat32POD) {
          ...
          if (datatype.getExtent() == 1 &&
            ...
            addFloatController(evalStream, options, modkey,
                               std::string("rgb.controller.r"), path,
                               iObj.getFullName(), xStream.str());
            addFloatController(evalStream, options, modkey,
                               std::string("rgb.controller.g"), path,
                               iObj.getFullName(), yStream.str());
            addFloatController(evalStream, options, modkey,
                               std::string("rgb.controller.b"), path,
                               iObj.getFullName(), zStream.str());
          }
        }
        else {
        }
    ...
    nodeRes = createNode(iObj, HELPER_CLASS_ID, Class_ID(DUMMY_CLASS_ID, 0),
                         pMaxNode, bReplaceExisting);
  }
  ...
  for (int i = 0; i < shaders.size(); i++) {
    ...
    Modifier* pMod = createDisplayModifier("Shader Properties", shaders[i].name,
                                           shaders[i].props);

    std::string target = shaders[i].target;
    std::string type = shaders[i].type;

    addControllersToModifier("Shader Properties", shaders[i].name,
                             shaders[i].props, target, type, path,
                             iObj.getFullName(), options);
  }
