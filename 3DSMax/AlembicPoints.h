...
class AlembicPoints : public AlembicObject {
 ...
 private:
  struct shapeInfo {
    ...
    shapeInfo()
        : type(ShapeType_NbElements),
          animationTime(-1.0),
          instanceName(),
          pParentActionList(NULL)
    {
    }
  };

  // maps born actionList to shape type
  // needed so that we can find the parent shape in the case that there is no
  // shape assignment
  typedef std::map<INode *, shapeInfo> perActionListShapeMap;
  typedef std::map<INode *, shapeInfo>::iterator perActionListShapeMap_it;
  typedef std::map<INode *, shapeInfo>::const_iterator
      perActionListShapeMap_cit;
  ...
  struct meshInfo {
    ...
    meshInfo()
        : pMesh(NULL), nMatId(-1), bNeedDelete(FALSE), nMeshInstanceId(-1)
    {
      bbox.min = Abc::V3d(0.0, 0.0, 0.0);
      bbox.max = Abc::V3d(0.0, 0.0, 0.0);
    }
  };
  // typedef std::pair<Alembic::Util::Digest, Alembic::Util::Digest>
  // faceVertexHashPair;
  struct meshDigests {
  ...
  static void AlembicPoints::ConvertMaxEulerXYZToAlembicQuat(
      const Point3 &degrees, Abc::Quatd &quat);  // double check
  static void AlembicPoints::ConvertMaxAngAxisToAlembicQuat(
      const AngAxis &angAxis, Abc::Quatd &quat);  // double check
  ...
  meshInfo CacheShapeMesh(Mesh *pShapeMesh, BOOL bNeedDelete, Matrix3 meshTM,
                          int nMatId, int particleId, TimeValue ticks,
                          ShapeType &type, unsigned short &instanceId,
                          float &animationTime);
