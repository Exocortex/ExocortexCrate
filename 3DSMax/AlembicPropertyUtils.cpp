    ...
    std::vector<std::pair<std::string, std::string>>& nameValuePairs)
{
  ...
  //
  // std::vector<std::pair<std::string, std::string>> nameValuePairs;
  // nameValuePairs.push_back(std::pair<std::string, std::string>("Prop1",
  // "Prop1Val"));
  // nameValuePairs.push_back(std::pair<std::string, std::string>("Prop2",
  // "Prop2Val"));
  // nameValuePairs.push_back(std::pair<std::string, std::string>("Prop3",
  // "Prop3Val"));
  // nameValuePairs.push_back(std::pair<std::string, std::string>("Prop4",
  // "Prop4Val"));
  // createPropertiesDisplayModifier(std::string("My Properties"),
  // nameValuePairs);
  ...
  for (int i = 0; i < nameValuePairs.size(); i++) {
    std::string& name = nameValuePairs[i].first;
    // std::string& val = nameValuePairs[i].second;
    evalStream << "edittext e" << name << " \"" << name
               << "\" fieldWidth:140 labelOnTop:true"
               << "\n";
  }
  evalStream << ")"
             << "\n";
  ...
  evalStream << "custattributes.add $.modifiers[\"" << modname
             << "\"] propAttribute baseobject:false"
             << "\n";

  ExecuteMAXScriptScript(EC_UTF8_to_TCHAR((char*)evalStream.str().c_str()));
}

Modifier* createDisplayModifier(std::string modkey, std::string modname,
                                std::vector<AbcProp>& props, INode* pNode)
{
  ESS_PROFILE_FUNC();
  ...
  for (int i = 0; i < props.size(); i++) {
    ...
    if (datatype.getPod() == AbcA::kInt32POD && datatype.getExtent() == 1) {
      evalStream << name << " type:#integer animatable:true ui:e" << name
                 << " default:" << val;
    }
    else if (datatype.getPod() == AbcA::kFloat32POD &&
             datatype.getExtent() == 1) {
      evalStream << name << " type:#float animatable:true ui:e" << name
                 << " default:" << val;
    }
    else if (datatype.getPod() == AbcA::kFloat32POD &&
             datatype.getExtent() == 3) {
      // if(metadata.get("interpretation") == std::string("rgb")){
      //   evalStream<<name<<" type:#color animatable:false ui:e"<<name<<"
      //   default:(["<<val<<"] as color)";
      //}
      // else{
      // evalStream<<name<<" type:#point3 animatable:false ui:e"<<name<<"
      // ["<<val<<"]";
      evalStream << name << "x type:#float animatable:true ui:e" << name
                 << "x default:300\n";
      evalStream << name << "y type:#float animatable:true ui:e" << name
                 << "y default:300\n";
      evalStream << name << "z type:#float animatable:true ui:e" << name
                 << "z default:300\n";
      //}
    }
    else {
      evalStream << name << " type:#string animatable:false ui:e" << name
                 << " default:\"" << val << "\"";
    }
    evalStream << "\n";
  }
  ...
  for (int i = 0; i < props.size(); i++) {
    ...
    if (datatype.getPod() == AbcA::kInt32POD && datatype.getExtent() == 1) {
      evalStream << "label lbl" << name << " \"" << name
                 << "\" align:#left fieldWidth:140\n";
      evalStream << "spinner e" << name << " \"\" type:#integer "
                                           "range:[-9999999,9999999,0] "
                                           "align:#left labelOnTop:true\n";
    }
    else if (datatype.getPod() == AbcA::kFloat32POD &&
             datatype.getExtent() == 1) {
      evalStream << "label lbl" << name << " \"" << name
                 << "\" align:#left fieldWidth:140\n";
      evalStream << "spinner e" << name << " \"\" type:#float "
                                           "range:[-9999999,9999999,0]align:#"
                                           "left labelOnTop:true\n";
    }
    else if (datatype.getPod() == AbcA::kFloat32POD &&
             datatype.getExtent() == 3) {
      evalStream << "label lbl" << name << " \"" << name
                 << "\" align:#left fieldWidth:140\n";
      evalStream << "spinner e" << name << "x\"\" across:3 type:#float "
                                           "range:[-9999999,9999999,0] "
                                           "fieldWidth:39 readOnly:true\n";
      evalStream
          << "spinner e" << name
          << "y\"\" type:#float range:[-9999999,9999999,0] fieldWidth:39\n";
      evalStream
          << "spinner e" << name
          << "z\"\" type:#float range:[-9999999,9999999,0] fieldWidth:39\n";
    }
    else {
  ...
  evalStream << "custattributes.add " << nodeName << ".modifiers[\"" << modkey
             << "\"] propAttribute baseobject:false"
             << "\n";
  ...
  FPValue fpVal;
  ExecuteMAXScriptScript(EC_UTF8_to_TCHAR((char*)evalStream.str().c_str()), 0,
                         &fpVal);
  Modifier* pMod = (Modifier*)fpVal.r;
  return pMod;
}

void addFloatController(
    std::stringstream& evalStream, alembic_importoptions& options,
    const std::string& modkey, std::string propName, const std::string& file,
    const std::string& identifier,
    std::string
        propertyID /*, const std::string& propComponent=std::string(""), const std::string& propInterp=std::string("")*/)
{
  ...
  evalStream << "$" << mkStr << "." << propName
             << ".controller = AlembicFloatController()\n";
  evalStream << "$" << mkStr << "." << propName << ".controller.path = \""
             << file << "\"\n";
  evalStream << "$" << mkStr << "." << propName << ".controller.identifier = \""
             << identifier << "\"\n";
  evalStream << "$" << mkStr << "." << propName << ".controller.property = \""
             << propertyID << "\"\n";

  evalStream << "$" << mkStr << "." << propName
             << ".controller.time.controller = float_expression()\n";
  if (options.loadTimeControl) {
    evalStream << "$" << mkStr << "." << propName
               << ".controller.time.controller.AddScalarTarget \"current\" $'"
               << timeControlName << "'.time.controller\n";
    evalStream << "$" << mkStr << "." << propName
               << ".controller.time.controller.setExpression \"current\"\n";
  }
  else {
    evalStream << "$" << mkStr << "." << propName
               << ".controller.time.controller.setExpression \"S\"\n";
  }
}

void addControllersToModifier(const std::string& modkey,
  ...
  for (int i = 0; i < props.size(); i++) {
    ...
    if (datatype.getPod() == AbcA::kFloat32POD) {
      ...
      if (datatype.getExtent() == 1) {
        addFloatController(evalStream, options, modkey, propName, file,
                           identifier, propStream.str());
      }
      else if (datatype.getExtent() == 3) {
        std::stringstream xStream, yStream, zStream;

        xStream << propStream.str() << "." << metadata.get("interpretation")
                << ".x";
        yStream << propStream.str() << "." << metadata.get("interpretation")
                << ".y";
        zStream << propStream.str() << "." << metadata.get("interpretation")
                << ".z";

        addFloatController(evalStream, options, modkey,
                           propName + std::string("x"), file, identifier,
                           xStream.str());
        addFloatController(evalStream, options, modkey,
                           propName + std::string("y"), file, identifier,
                           yStream.str());
        addFloatController(evalStream, options, modkey,
                           propName + std::string("z"), file, identifier,
                           zStream.str());
      }
    }
    else {
...
void addControllersToModifierV2(const std::string& modkey,
                                const std::string& modname,
                                std::vector<AbcProp>& props,
                                const std::string& file,
                                const std::string& identifier,
                                const std::string& category,
                                alembic_importoptions& options, INode* pNode)
{
  ESS_PROFILE_FUNC();
  ...
  for (int i = 0; i < props.size(); i++) {
    ...
    if (datatype.getPod() == AbcA::kFloat32POD) {
      ...
      propStream << propName;
      if (datatype.getExtent() == 1) {
        addFloatControllerV2(evalStream, options, nodeName, modkey, propName,
                             file, identifier, category, propStream.str());
      }
      else if (datatype.getExtent() == 3) {
        std::stringstream xStream, yStream, zStream;
        ...
        addFloatControllerV2(evalStream, options, nodeName, modkey,
                             propName + std::string("x"), file, identifier,
                             category, xStream.str());
        addFloatControllerV2(evalStream, options, nodeName, modkey,
                             propName + std::string("y"), file, identifier,
                             category, yStream.str());
        addFloatControllerV2(evalStream, options, nodeName, modkey,
                             propName + std::string("z"), file, identifier,
                             category, zStream.str());
      }
    }
...
void addFloatControllerV2(std::stringstream& evalStream,
                          alembic_importoptions& options, std::string nodeName,
                          const std::string& modkey, std::string propName,
                          const std::string& file,
                          const std::string& identifier,
                          const std::string& category, std::string propertyID)
{
  std::string mkStr;
  if (!modkey.empty()) {
    std::stringstream mkStream;
  ...
  evalStream << nodeName << mkStr << "." << propName
             << ".controller = AlembicFloatController()\n";
  evalStream << nodeName << mkStr << "." << propName << ".controller.path = \""
             << file << "\"\n";
  evalStream << nodeName << mkStr << "." << propName
             << ".controller.identifier = \"" << identifier << "\"\n";
  evalStream << nodeName << mkStr << "." << propName
             << ".controller.propCategory = \"" << category << "\"\n";
  evalStream << nodeName << mkStr << "." << propName
...
bool readPropExt1(const Abc::ICompoundProperty& prop,
                  const AbcA::PropertyHeader& pheader, std::string& val,
                  bool& isConstant)
{
...
template <class PT, class FT>
bool readPropExt3(const Abc::ICompoundProperty& prop,
                  const AbcA::PropertyHeader& pheader, std::string& val,
                  bool& isConstant)
{
  if (PT::matches(pheader)) {
...
const int invalidStrTableSize =
    sizeof(invalidStrTable) / sizeof(invalidStrTable[0]);
...
void readInputProperties(Abc::ICompoundProperty prop,
                         std::vector<AbcProp>& props)
{
  if (!prop) {
    return;
  ...
  for (size_t i = 0; i < prop.getNumProperties(); i++) {
    ...
    if (invalidStrIndex > 0) {
      ESS_LOG_WARNING("Skipping property "
                      << pheader.getName()
                      << " because it contains an invalid character: "
                      << invalidStrTable[invalidStrIndex]);
      continue;
    }
    ...
    if (propType == AbcA::kCompoundProperty) {
      ...
      if (Abc::IBoolProperty::matches(pheader)) {
        // I need to know the name and type only if animated; an appropriate
        // controller will handle reading the data.
        // If not animated, the value will set directly on the light and/or
        // display modifier
      ...
      else if (readPropExt1<Abc::IInt32Property, int>(prop, pheader, displayVal,
                                                      bConstant))
        ;
      else if (readPropExt1<Abc::IFloatProperty, float>(prop, pheader,
                                                        displayVal, bConstant))
        ;
      else if (readPropExt3<Abc::IC3fProperty, Abc::C3f>(prop, pheader,
                                                         displayVal, bConstant))
        ;
      else if (readPropExt3<Abc::IV3fProperty, Abc::V3f>(prop, pheader,
                                                         displayVal, bConstant))
        ;
      else if (readPropExt3<Abc::IN3fProperty, Abc::N3f>(prop, pheader,
                                                         displayVal, bConstant))
        ;
        ...
        ESS_LOG_WARNING(
            ...
            << pheader.getName()
            << ", pod: " << getPodStr(pheader.getDataType().getPod())
      ...
      props.push_back(
          AbcProp(pheader.getName(), displayVal, pheader, bConstant, sortId));
    }
    else if (propType == AbcA::kArrayProperty) {
      // ESS_LOG_WARNING("Unsupported array property: "<<pheader.getName());

      // it the moment is unlikely we will support array properties in 3DS Max.
...
bool AlembicCustomAttributesEx::defineCustomAttributes(
    INode* node, Abc::OCompoundProperty& compoundProp,
    const AbcA::MetaData& metadata, unsigned int animatedTs)
{
  ...
  for (int i = 0, nNumParams = pblock->NumParams(); i < nNumParams; i++) {
    ...
    if (type == TYPE_STRING) {
      customProps[propName.str()] = new Abc::OStringProperty(
          compoundProp, propName.str().c_str(), metadata, animatedTs);
    }
    else if (type == TYPE_FLOAT) {
      customProps[propName.str()] = new Abc::OFloatProperty(
          compoundProp, propName.str().c_str(), metadata, animatedTs);
    }
    else if (type == TYPE_INT) {
      customProps[propName.str()] = new Abc::OInt32Property(
          compoundProp, propName.str().c_str(), metadata, animatedTs);
    }
  }
...
bool AlembicCustomAttributesEx::exportCustomAttributes(INode* node, double time)
{
  ...
  for (int i = 0; i < nNumParams; i++) {
    ...
    if (type == TYPE_STRING) {
      ...
      Abc::OStringProperty* stringProp =
          (Abc::OStringProperty*)customProps[propName.str()];
      if (stringProp) {
      ...
      Abc::OFloatProperty* floatProp =
          (Abc::OFloatProperty*)customProps[propName.str()];
      if (floatProp) {
        floatProp->set(value);
      }
    }
      ...
      Abc::OInt32Property* intProp =
          (Abc::OInt32Property*)customProps[propName.str()];
      if (intProp) {
        intProp->set(value);
      }
    }
...
void setupPropertyModifiers(AbcG::IObject& iObj, INode* pMaxNode,
                            const std::string& file,
                            const std::string& identifier,
                            alembic_importoptions& options,
                            const std::string prefix)
{
  ...
  if (userProps.valid()) {
    ...
    if (!propsVec.empty()) {
      ...
      addControllersToModifierV2(name, name, propsVec, file, identifier,
                                 "userProperties", options, pMaxNode);
    }
  }
  ...
  //      //addControllersToModifierV2("ArbGeom Properties", "ArbGeom
  //      Properties", propsVec, file, iObjXform.getFullName(), options,
  //      pMaxNode);
  //   }
  //}
}
