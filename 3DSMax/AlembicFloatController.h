...
class AlembicFloatController : public LockableStdControl {
 ...
 public:
  ...
  void setController(std::string call, std::string name, Interval &valid,
                     Interval interval, GetSetMethod method, void *ptr,
                     float fVal);

  virtual void GetValueLocalTime(TimeValue t, void *ptr, Interval &valid,
                                 GetSetMethod method = CTRL_ABSOLUTE);
  virtual void SetValueLocalTime(TimeValue t, void *ptr, int commit,
                                 GetSetMethod method = CTRL_ABSOLUTE);
  virtual void Extrapolate(Interval range, TimeValue t, void *val,
...
class AlembicFloatControllerClassDesc : public ClassDesc2 {
 public:
  ...
  // ClassDesc methods.
  // Max calls these functions to figure out what kind of plugin this class
  // represents
  ...
  {
    static const MSTR str(_T("Alembic Float Controller"));
    return str;
  }
  ...
  void *Create(BOOL loading = FALSE) { return new AlembicFloatController; }
  const TCHAR *InternalName()
  {
    return _T("AlembicFloatController");
  }  // returns fixed parsable name (scripter-visible name)
  HINSTANCE HInstance() { return hInstance; }  // returns owning module handle
};

// bool getCameraSampleVal(AbcG::ICamera& objCamera, SampleInfo& sampleInfo,
// AbcG::CameraSample sample, const char* name, double& sampleVal);
