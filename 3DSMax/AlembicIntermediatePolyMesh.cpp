    ...
    const AlembicIntermediatePolyMesh& srcMesh)
{
  ...
  //   if( destMesh.mIndexedNormals.indices[i] >
  //   destMesh.mIndexedNormals.values.size()){
  ...
  // Abc::uint32_t amountToOffsetSrcUvIndicesBy =
  // (Abc::uint32_t)destMesh.mUvVec.size();
  ...
  // I assume if that if a key is common, we combine the face lists (this
  // assumption is compatible with
  // our current method of saving out the mapIds and names)
  for (facesetmap_cit it = srcMesh.mFaceSetsMap.begin();
       it != srcMesh.mFaceSetsMap.end(); it++) {
    if (destMesh.mFaceSetsMap.find(it->first) ==
        destMesh.mFaceSetsMap.end()) {  // a new key
