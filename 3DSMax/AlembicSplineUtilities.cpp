...
class CurvePositionSampler {
 public:
  ...
  enum interpT { NONE, POSITION, VELOCITY };
  ...
  CurvePositionSampler(AbcG::ICurves &obj, SampleInfo &sampleInfo, float tAlpha,
                       float sAlpha)
  {
    ...
    if (isDynamicTopo) {  // interpolate based on velocity
      ...
      if (curvePos1->size() ==
          curvePos2->size()) {  // interpolate based on positions
        interp = POSITION;
      }
    }
  }
...
void AlembicImport_FillInShape_Internal(alembic_fillshape_options &options)
{
  ...
  SampleInfo sampleInfo =
      getSampleInfo(sampleTime, obj.getSchema().getTimeSampling(),
  ...
  {
      double startSeconds =
  obj.getSchema().getTimeSampling()->getSampleTime(sampleInfo.floorIndex);
      double endSeconds =
  obj.getSchema().getTimeSampling()->getSampleTime(sampleInfo.ceilIndex);
  ...
  if (curveSample.getType() != AbcG::kLinear &&
    ...
    ESS_LOG_ERROR("Skipping curve '" << options.pIObj->getFullName()
                                     << "', invalid curve type.");
    return;
  }
  ...
  CurvePositionSampler posSampler(obj, sampleInfo, fSamplerTimeAlpha,
                                  (float)sampleInfo.alpha);

  // Prepare the knots
  if (options.nDataFillFlags & ALEMBIC_DATAFILL_SPLINE_KNOTS) {
    Abc::Int32ArraySamplePtr curveNbVertices =
        curveSample.getCurvesNumVertices();
      ...
      for (int i = 0; i < options.pBezierShape->SplineCount(); i += 1) {
        ...
        for (int j = 0; j < knots; j++) {
          ...
          /*if (curveSample.getType() == AbcG::kCubic ) {
                   ...
                   p = ConvertAlembicPointToMaxPoint(posSampler[startVertex + k
           + 1 ]);
                   out = ConvertAlembicPointToMaxPoint(posSampler[startVertex +
           k + 2 ]);
                   * /

                   in = ConvertAlembicPointToMaxPoint(posSampler[startVertex +
           max( j - 1, 0 )]);
                   p = ConvertAlembicPointToMaxPoint(posSampler[startVertex +
           j]);
                   out = ConvertAlembicPointToMaxPoint(posSampler[startVertex +
           min( j + 1, knots - 1 )]);
           }
           else {*/
          in = ConvertAlembicPointToMaxPoint(
              posSampler[startVertex + std::max(j - 1, 0)]);
          p = ConvertAlembicPointToMaxPoint(posSampler[startVertex + j]);
          out = ConvertAlembicPointToMaxPoint(
              posSampler[startVertex + std::min(j + 1, knots - 1)]);

          if (pSpline->Closed()) {
            if (j == 0) {
              in = ConvertAlembicPointToMaxPoint(
                  posSampler[startVertex + knots - 1]);
            }
            else if (j == knots - 1) {
...
/*
int AlembicImport_Shape(const std::string &file, const std::string &identifier,
alembic_importoptions &options)
{
    ...
    {
        pAlembicSpline =
static_cast<AlembicSimpleSpline*>(GET_MAX_INTERFACE()->CreateInstance(SHAPE_CLASS_ID,
ALEMBIC_SIMPLE_SPLINE_CLASSID));
    ...
    unsigned int nDataFillFlags =
ALEMBIC_DATAFILL_VERTEX|ALEMBIC_DATAFILL_SPLINE_KNOTS;
    ...
    else if (pAlembicShape)
    {
        ...
        INode *node = GET_MAX_INTERFACE()->CreateObjectNode(newObject,
iObj.getName().c_str());
    ...
    SceneEntry *pEntry = options.sceneEnumProc.Append(node, newObject,
OBTYPE_CURVES, &std::string(iObj.getFullName()));
    options.currentSceneList.Append(pEntry);

    // Set the visibility controller
    AlembicImport_SetupVisControl(file.c_str(), identifier.c_str(), iObj, node,
options);

        return 0;
}*/

int AlembicImport_Shape(const std::string &path, AbcG::IObject &iObj,
                        alembic_importoptions &options, INode **pMaxNode)
{
  ...
  if (!pNode) {
    Object *newObject =
        reinterpret_cast<Object *>(GET_MAX_INTERFACE()->CreateInstance(
            SHAPE_CLASS_ID, EMPTY_SPLINE_OBJECT_CLASSID));
    if (newObject == NULL) {
      return alembic_failure;
    }
...

  {
    ...
    if (bReplaceExistingModifiers) {
      pModifier = FindModifier(pNode, ALEMBIC_SPLINE_TOPO_MODIFIER_CLASSID,
                               identifier.c_str());
    }
    if (!pModifier) {
      pModifier = static_cast<Modifier *>(GET_MAX_INTERFACE()->CreateInstance(
          OSM_CLASS_ID, ALEMBIC_SPLINE_TOPO_MODIFIER_CLASSID));
  ...
  if ((!isDynamicTopo) && isAlembicSplinePositions(&iObj, isGeomContant)) {
    ...
    bool bCreatedModifier = false;
    if (bReplaceExistingModifiers) {
      pModifier = FindModifier(pNode, ALEMBIC_SPLINE_GEOM_MODIFIER_CLASSID,
                               identifier.c_str());
    }
    if (!pModifier) {
      pModifier = static_cast<Modifier *>(GET_MAX_INTERFACE()->CreateInstance(
          OSM_CLASS_ID, ALEMBIC_SPLINE_GEOM_MODIFIER_CLASSID));
      bCreatedModifier = true;
    }
    ...
    if (bCreatedModifier) {
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "identifier"), zero,
          EC_UTF8_to_TCHAR(identifier.c_str()));
      // pModifier->GetParamBlockByID( 0 )->SetValue( GetParamIdByName(
      // pModifier, 0, "geoAlpha" ), zero, 1.0f );
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "time"), zero, 0.0f);
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "muted"), zero, FALSE);
