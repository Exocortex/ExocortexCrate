  ...
  if (node->IsTarget()) {
    ...
    if (ln) {
      ...
      switch (lobj->SuperClassID()) {
        case LIGHT_CLASS_ID:
          return SceneEntry(node, obj, OBTYPE_LTARGET, pFullname);
          break;
        case CAMERA_CLASS_ID:
          return SceneEntry(node, obj, OBTYPE_CTARGET, pFullname);
          break;
      }
    }
  }
...
SceneEntry *SceneEnumProc::Append(INode *node, Object *obj, int type,
                                  std::string *providedfullname)
{
