...
size_t getNumXformChildren(AbcG::IObject& iObj)
{
  ...
  // An XForm with xForm(s) with an xForm child will require a dummy node (and a
  // single xform can be attached it child geometry node)
...
void AlembicImport_FillInXForm_Internal(alembic_fillxform_options& options)
{
  ...
  if (options.pObjectCache->numSamples > 0) {
...

    {
      ...
      sampleInfo =
          getSampleInfo(SampleTime, pObj->getSchema().getTimeSampling(),
                        options.pObjectCache->numSamples);
    }

    if (g_bVerboseLogging) {
      ESS_LOG_INFO("SampleInfo.alpha: "
                   << sampleInfo.alpha << "SampleInfo(fi, ci): "
                   << sampleInfo.floorIndex << ", " << sampleInfo.ceilIndex);
    }

    matrix = options.pObjectCache->getXformMatrix((int)sampleInfo.floorIndex);

    // const Abc::Box3d &box3d =
    // obj.getSchema().getChildBoundsProperty().getValue( sampleInfo.floorIndex
    // );

    // options.maxBoundingBox = Box3(Point3(box3d.min.x, box3d.min.y,
    // box3d.min.z),
    ...
    if (sampleInfo.alpha != 0.0) {
      ...
      Abc::M44d ceilMatrix =
          options.pObjectCache->getXformMatrix((int)sampleInfo.ceilIndex);
      // sample.getMatrix();
      matrix =
          (1.0 - sampleInfo.alpha) * matrix + sampleInfo.alpha * ceilMatrix;
    }
  }

  {
    ...
    Matrix3 objMatrix(Point3(matrix.getValue()[0], matrix.getValue()[1],
                             matrix.getValue()[2]),
                      Point3(matrix.getValue()[4], matrix.getValue()[5],
                             matrix.getValue()[6]),
                      Point3(matrix.getValue()[8], matrix.getValue()[9],
                             matrix.getValue()[10]),
                      Point3(matrix.getValue()[12], matrix.getValue()[13],
                             matrix.getValue()[14]));

    ConvertAlembicMatrixToMaxMatrix(objMatrix, options.maxMatrix);

    if (options.bIsCameraTransform) {
      // Cameras in Max are already pointing down the negative z-axis (as is
      // expected from Alembic).
      // So we rotate it by 90 degrees so that it is pointing down the positive
      // y-axis.
...
int AlembicImport_DummyNode(AbcG::IObject& iObj, alembic_importoptions& options,
                            INode** pMaxNode, const std::string& importName)
{
  ESS_PROFILE_FUNC();
  Object* dObj = static_cast<Object*>(
  ...
  if (obj.getSchema().getChildBoundsProperty().valid()) {
    const Abc::Box3d& box3d = obj.getSchema().getChildBoundsProperty().getValue(
        sampleInfo.floorIndex);
    ...
    if (options.crateBuildNum == -1 || options.crateBuildNum >= 139) {
      ...
      // thus, since Ymin < yMax, then -Ymin > -Ymax. So we need to swap the y
      // components
      ...
      pDummy->SetBox(Box3(Point3(box3d.min.x, box3d.min.y, box3d.min.z),
                          Point3(box3d.max.x, box3d.max.y, box3d.max.z)));
    }
  }
  ...
  *pMaxNode = GET_MAX_INTERFACE()->CreateObjectNode(
      dObj, EC_UTF8_to_TCHAR(importName.c_str()));

  SceneEntry* pEntry = options.sceneEnumProc.Append(
      *pMaxNode, dObj, OBTYPE_DUMMY, &std::string(iObj.getFullName()));
...
// AlembicImport_XForm
///////////////////////////////////////////////////////////////////////////////////////////////////
int AlembicImport_XForm(INode* pParentNode, INode* pMaxNode,
                        AbcG::IObject& iObjXform, AbcG::IObject* p_iObjGeom,
                        const std::string& file, alembic_importoptions& options)
{
  ESS_PROFILE_FUNC();
  ...
  // AbcNodeUtils::printObjectProperties(iObj, ObjectPrint::PROPERTIES |
  // ObjectPrint::USER_PROPERTIES | ObjectPrint::ARB_GEOM_PROPERTIES);
  ...
  //   addControllersToModifierV2("User Properties", "User Properties",
  //   propsVec, file, iObjXform.getFullName(), options, pMaxNode);
  //}
  ...
  //   std::sort(propsVec.begin(), propsVec.end(), sortFunc);
  //   createDisplayModifier("ArbGeom Properties", "ArbGeom Properties",
  //   propsVec, pMaxNode);

  //   //addControllersToModifierV2("ArbGeom Properties", "ArbGeom Properties",
  //   propsVec, file, iObjXform.getFullName(), options, pMaxNode);
  //}

  if (!options.attachToExisting && options.loadUserProperties) {
    setupPropertyModifiers(iObjXform, pMaxNode, file, iObjXform.getFullName(),
                           options);
  }

  // TODO: this method should take this bool a parameter instead
  bool bIsCamera = p_iObjGeom && p_iObjGeom->valid() &&
                   AbcG::ICamera::matches(p_iObjGeom->getMetaData());
  ...
  if (!isConstant) {
    ...
    if (options.attachToExisting) {
      ...
      if (pAnimatable &&
          pAnimatable->ClassID() == ALEMBIC_XFORM_CONTROLLER_CLASSID) {
    ...
    if (!pControl) {
      pControl = static_cast<AlembicXformController*>(
          GetCOREInterface()->CreateInstance(CTRL_MATRIX3_CLASS_ID,
    ...
    if (bCreatedController) {
      ...
      pControl->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pControl, 0, "camera"), zero,
          (bIsCamera ? TRUE : FALSE));
      pControl->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pControl, 0, "muted"), zero, TRUE);
    ...
    if (pControl) {
      ...
      if (strcmp(EC_MSTR_to_UTF8(name).c_str(), "Position/Rotation/Scale") !=
          0) {
    ...
    if (pParentNode != NULL) {
      pMaxNode->SetNodeTM(
          zero, xformOptions.maxMatrix * pParentNode->GetObjectTM(zero));
    }
    else {
