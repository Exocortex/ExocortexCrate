...
AlembicWriteJob::AlembicWriteJob(const std::string& in_FileName,
                                 std::map<std::string, bool>& objectsMap,
                                 const std::vector<double>& in_Frames,
                                 Interface* i)
{
  mApplication = i;
  mMeshErrors = 0;
...
void AlembicWriteJob::SetOption(const std::string& in_Name,
                                const bool& in_Value)
{
...
bool AlembicWriteJob::PreProcess()
{
  ...
  bool bSelectParents = GetOption(
      "includeParentNodes"); /*|| !bFlattenHierarchy || bTransformCache*/
  ...
  if (bObjectsParameterExists) {
    // Might be better to use refineSelection here, but call a function that
    // sets up dccSelected flag first, then delete this function from codebase
    selectNodes(exoSceneRoot, mObjectsMap, bSelectParents, bSelectChildren,
                !bTransformCache);

    bool bAllResolved = true;

    if (bObjectsParameterExists) {
      for (SceneNode::SelectionT::iterator it = mObjectsMap.begin();
           it != mObjectsMap.end(); it++) {
        if (it->second == false) {
          bAllResolved = false;
    ...
    refineSelection(exoSceneRoot, bSelectParents, bSelectChildren,
                    !bTransformCache);
    removeUnselectedNodes(exoSceneRoot);
  }
  ...
  try {
    if (bUseOgawa) {
      mArchive = CreateArchiveWithInfo(
          Alembic::AbcCoreOgawa::WriteArchive(), mFileName.c_str(),
      ...
      mArchive = CreateArchiveWithInfo(
          Alembic::AbcCoreHDF5::WriteArchive(true), mFileName.c_str(),
          getExporterName("3DS Max " EC_QUOTE(crate_Max_Version)).c_str(),
          getExporterFileName(sceneFileName).c_str(),
          Abc::ErrorHandler::kThrowPolicy);
    }
  ...
  if (frames.size() > 1) {
    ...
    AbcA::TimeSamplingType samplingType((boost::uint32_t)frames.size(),
                                        timePerCycle);
  ...
  try {
    while (!sceneStack.empty()) {
      ...
      if (eNode->type == SceneNode::SCENE_ROOT) {
        // we do not want to export the Scene_Root (the alembic archive has one
        // already)
      }
      ...
      //}
        ...
        pNewObject.reset(new AlembicCurves(eNode, this, oParent));
               ...
               eNode->type == SceneNode::PARTICLES_TP) {
        if (bParticleMesh) {
          pNewObject.reset(new AlembicPolyMesh(eNode, this, oParent));
        }
        else {
      ...
      // else{
      //   ESS_LOG_WARNING("Unknown type: not exporting "<<eNode->name);//Export
      //   as transform, and give warning?
      //}
