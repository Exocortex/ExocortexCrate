    ...
    : AlembicObject(eNode, in_Job, oParent)
{
  ...
  AbcG::OCurves curves(GetOParent(), curveName,
                       GetCurrentJob()->GetAnimatedTs());
  mCurvesSchema = curves.getSchema();

  // create all properties
  // mInTangentProperty = OV3fArrayProperty(mCurvesSchema.getArbGeomParams(),
  // ".inTangent", mCurvesSchema.getMetaData(), GetCurrentJob()->GetAnimatedTs()
  // );
  // mOutTangentProperty = OV3fArrayProperty(mCurvesSchema.getArbGeomParams(),
  // ".outTangent", mCurvesSchema.getMetaData(),
  // GetCurrentJob()->GetAnimatedTs() );
  // mRadiusProperty = OFloatArrayProperty(mCurvesSchema.getArbGeomParams(),
  // ".radius", mCurvesSchema.getMetaData(), GetCurrentJob()->GetAnimatedTs() );
  // mColorProperty = OC4fArrayProperty(mCurvesSchema.getArbGeomParams(),
  // ".color", mCurvesSchema.getMetaData(), GetCurrentJob()->GetAnimatedTs() );
}
...
bool AlembicCurves::Save(double time, bool bLastFrame)
{
  ...
  if (obj->ClassID() == EDITABLE_SURF_CLASS_ID) {
    ...
    curvesSample.setWrap(pShapeObject->CurveClosed(ticks, 0)
                             ? AbcG::kPeriodic
                             : AbcG::kNonPeriodic);
  ...
  if (knotVector.size() > 0 && orders.size() > 0) {
    if (!mKnotVectorProperty.valid()) {
      mKnotVectorProperty = Abc::OFloatArrayProperty(
          ...
          mCurvesSchema.getMetaData(), mJob->GetAnimatedTs());
    }
    mKnotVectorProperty.set(Abc::FloatArraySample(knotVector));

    if (!mOrdersProperty.valid()) {
      mOrdersProperty = Abc::OUInt16ArrayProperty(
          mCurvesSchema.getArbGeomParams(), ".orders",
          mCurvesSchema.getMetaData(), mJob->GetAnimatedTs());
    }
  ...
  Abc::Int32ArraySample nbVerticesSample(&nbVertices.front(),
                                         nbVertices.size());
