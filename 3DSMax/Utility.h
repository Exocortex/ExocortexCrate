...
void AlembicDebug_PrintMeshData(Mesh &mesh,
                                std::vector<VNormal> &sgVertexNormals);
...
inline Point3 ConvertAlembicNormalToMaxNormal_Normalized(
    const Abc::V3f &alembicPoint)
{
...
inline Quat ConvertAlembicQuatToMaxQuat(const Abc::Quatf &alembicQuat,
                                        bool bNormalize)
{
...
// inline Abc::Quatf ConvertMaxQuatToAlembicQuat( const Abc::Quatf &alembicQuat,
// bool bNormalize)
//{
//	Abc::Quatf q(alembicQuat.v.x, alembicQuat.v.z, -alembicQuat.v.y,
//-alembicQuat.r);
//
//    if (bNormalize)
//		q.normalize();
...
inline void ConvertMaxAngAxisToAlembicQuat(const AngAxis &angAxis,
                                           Abc::Quatd &quat)
{
...
TriObject *GetTriObjectFromNode(INode *iNode, const TimeValue t,
                                bool &deleteIt);
...
class AlembicPathAccessor : public IAssetAccessor {
 public:
  ...
  {
    return pAnimatable->GetParamBlockByID(0)->GetAssetUser(
        GetParamIdByName(pAnimatable, 0, "path"), 0);
  }

  bool SetAsset(const MaxSDK::AssetManagement::AssetUser &aNewAssetUser)
  {
    pAnimatable->GetParamBlockByID(0)->SetValue(
        GetParamIdByName(pAnimatable, 0, "path"), 0, aNewAssetUser);
    return true;
  }
  ...
  {
    return MaxSDK::AssetManagement::kExternalLink;
  }
...
#define GET_MAXSCRIPT_NODE(pNode) \
  "mynode2113 = maxOps.getNodeByHandle(" << pNode->GetHandle() << ");\n"
