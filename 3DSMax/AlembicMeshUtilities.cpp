...
void validateMeshes(alembic_fillmesh_options &options, char *szName)
{
  if (options.pMNMesh != NULL) {
    // ESS_LOG_WARNING( "options.pMNMesh->MNDebugPrint()
    // ------------------------------------------------------------ section: "
    // << szName );
...
void AlembicImport_FillInPolyMesh_Internal(alembic_fillmesh_options &options)
{
  ...
  if (objMesh.valid()) {
    ...
    sampleInfo =
        getSampleInfo(sampleTime, objMesh.getSchema().getTimeSampling(),
                      objMesh.getSchema().getNumSamples());
  }
  else {
    sampleInfo =
        getSampleInfo(sampleTime, objSubD.getSchema().getTimeSampling(),
  ...
  if (hasDynamicTopo) {
    // do slower check to see if the topology did not change between this frame
    // and the previous frame
    hasDynamicTopo = frameHasDynamicTopology(
        &polyMeshSample, &sampleInfo,
        &(objMesh.getSchema().getFaceIndicesProperty()));
  }

  // ESS_LOG_WARNING("dynamicTopology: "<<hasDynamicTopo<<" time:
  // "<<sampleTime);
  ...
  // note that the fillInMesh call will crash if the points are not initilaized
  // (tested max 2013)
  if ((options.nDataFillFlags & ALEMBIC_DATAFILL_FACELIST) ||
    ...
    if (currentNumVerts != meshPos->size() &&
        !options.pMNMesh->GetFlag(MN_MESH_RATSNEST)) {
    ...
    validateMeshes(options,
                   "ALEMBIC_DATAFILL_FACELIST | ALEMBIC_DATAFILL_VERTEX");
  }

  if (options.nDataFillFlags & ALEMBIC_DATAFILL_VERTEX) {
    ESS_PROFILE_SCOPE("ALEMBIC_DATAFILL_VERTEX");
    Abc::V3f const *pPositionArray =
        (meshPos.get() != NULL) ? meshPos->get() : NULL;
    Abc::V3f const *pVelocityArray =
        (meshVel.get() != NULL) ? meshVel->get() : NULL;
    ...
    if (pPositionArray) {
      ...
      if (((options.nDataFillFlags &
            ~ALEMBIC_DATAFILL_IGNORE_SUBFRAME_SAMPLES) &&
           sampleInfo.alpha != 0.0f) ||
          ((options.nDataFillFlags &
            ALEMBIC_DATAFILL_IGNORE_SUBFRAME_SAMPLES) &&
           fRoundedTimeAlpha != 0.0f)) {
        bool bSampleInterpolate = false;
        bool bVelInterpolate = false;

        if (objMesh.valid()) {
          {
            // ESS_PROFILE_SCOPE("AlembicImport_FillInPolyMesh_Internal - 2nd
            // mesh sample read");
            ...
            Abc::IP3fArrayProperty positionPropertyCeil =
                objMesh.getSchema().getPositionsProperty();
      ...
      for (int i = 0; i < vArray.size(); i++) {
        if (options.bAdditive) {
          // pMeshVerties[i].p +=
          options.pObject->SetPoint(
              i, options.pObject->GetPoint(i) +
                     ConvertAlembicPointToMaxPoint(vArray[i]));
        }
        else {
          // pMeshVerties[i].p = ConvertAlembicPointToMaxPoint(vArray[i]);
          options.pObject->SetPoint(i,
                                    ConvertAlembicPointToMaxPoint(vArray[i]));
        }
      }
  ...
  Abc::int32_t const *pMeshFaceCount =
      (meshFaceCount.get() != NULL) ? meshFaceCount->get() : NULL;
  Abc::int32_t const *pMeshFaceIndices =
      (meshFaceIndices.get() != NULL) ? meshFaceIndices->get() : NULL;
  ...
  if (options.nDataFillFlags & ALEMBIC_DATAFILL_FACELIST) {
    ...
    if (sampleCount == numIndices) {
      ...
      ESS_LOG_WARNING(
          "faceCount, index array mismatch. Not filling in indices (did you "
          "check 'dynamic topology' when exporting?).");
    }
  }

  // 3DS Max tends to crash if FillInMesh is not called. The call will build a
  // winged-edge data structure, so good topology is required.
  // 3DS Max duplicate points to fix bad topology. Thus, it is important for
  // this call to happen last (after the last geometry modifier is
  // applied). Since this case is difficult to detect, we will throw an error
  // some when multiple modifiers exist, a and there is bad topology.

  // if (( options.nDataFillFlags & ALEMBIC_DATAFILL_VERTEX )){//||
  // We seem to get crash if FillInPolyMesh is called in a modifier other that
  // the topology one
  if (options.nDataFillFlags & ALEMBIC_DATAFILL_FACELIST) {
    // if( !options.pMNMesh->GetFlag( MN_MESH_FILLED_IN ) ){//This flag never
    // seems to be set
    if (options.pMNMesh->ENum() == 0) {
      ESS_PROFILE_SCOPE("FillInMesh");
      // ESS_LOG_WARNING("Filling in polymesh, fileName: " << options.fileName
      ...
      if (options.pMNMesh->GetFlag(MN_MESH_RATSNEST) &&
          !(options.nDataFillFlags & ALEMBIC_DATAFILL_VERTEX)) {
        ESS_LOG_ERROR(
            "Mesh is a 'Rat's Nest' (more than 2 faces per edge),\n fileName: "
            << options.fileName << " identifier: " << options.identifier
            << ".\n Please import with the \"Load Geometry from Topology "
               "modifier\" option active");
      }
    }
    else if (options.pMNMesh->VNum() > meshPos->size()) {
      ESS_LOG_WARNING(
          "Mesh has bad topology. Multiple geometry modifiers not fully "
          "supported, fileName: "
          << options.fileName << " identifier: " << options.identifier);
    }
  }
  ...
  if (objMesh.valid() && (options.nDataFillFlags & ALEMBIC_DATAFILL_NORMALS)) {
    ...
    AbcG::IN3fGeomParam meshNormalsParam =
        objMesh.getSchema().getNormalsParam();
    if (meshNormalsParam.valid()) {
      ...
      bool normalsFloor = getIndexAndValues(
          meshFaceIndices, meshNormalsParam, sampleInfo.floorIndex,
          normalValuesFloor, normalIndicesFloor);
      bool normalsCeil = false;
      if (!hasDynamicTopo) {
        normalsCeil = getIndexAndValues(meshFaceIndices, meshNormalsParam,
                                        sampleInfo.ceilIndex, normalValuesCeil,
                                        normalIndicesCeil);
      }
      ...
      if (!normalsFloor) {
        ...
        // The last check ensures that we will not exceed the array bounds of
        // PMeshNormalsFloor
        ESS_LOG_ERROR("Mesh normals are not set because their index count ("
                      << normalIndicesFloor.size()
                      << ") doesn't match the face index count (" << sampleCount
                      << ")");
      }
      else {
        ...
        // NOTE: options.pMNMesh->ClearSpecifiedNormals() will make
        // getSpecifiedNormals return NULL again
        MNNormalSpec *normalSpec = options.pMNMesh->GetSpecifiedNormals();
        normalSpec->SetParent(options.pMNMesh);
        if (normalSpec->GetNumFaces() != numFaces ||
            normalSpec->GetNumNormals() != (int)normalValuesFloor.size()) {
        ...
        if (sampleInfo.alpha != 0.0f && normalsCeil &&
            normalValuesFloor.size() == normalValuesCeil.size() &&
            !hasDynamicTopo) {
          for (int i = 0; i < normalValuesFloor.size(); i++) {
            Abc::V3f interpolatedNormal =
                normalValuesFloor[i] +
                (normalValuesCeil[i] - normalValuesFloor[i]) *
                    float(sampleInfo.alpha);
            interpolatedNormal *= options.fVertexAlpha;
            normalSpec->Normal(i) =
                ConvertAlembicNormalToMaxNormal_Normalized(interpolatedNormal);
          }
        }
        else {
          for (int i = 0; i < normalValuesFloor.size(); i++) {
        ...
        // set normal indices
        {
          ...
          for (int i = 0; i < numFaces; i++) {
            ...
            for (int j = 0; j < degree; j++) {
              normalFace.SetNormalID(degree - j - 1,
                                     (int)normalIndicesFloor[offset]);
              offset++;
            }
          }
        }

        // 3DS Max documentation on MNMesh::checkNormals() - checks our flags
        // and calls BuildNormals, ComputeNormals as needed.
        // MHahn: Probably not necessary since we explicility setting every
        // normals
        // normalSpec->CheckNormals();

        // since we commented out check normals above, we need to set these
        // explicitly.
        normalSpec->SetFlag(MESH_NORMAL_NORMALS_BUILT, TRUE);
        normalSpec->SetFlag(MESH_NORMAL_NORMALS_COMPUTED, TRUE);

        // Also allocates space for the RVert array which we need for doing any
        // normal vector queries
        // options.pMNMesh->checkNormals(TRUE);
        // MHahn: switched to build normals call, since all we need to is build
        // the RVert array. Note: documentation says we only need
        // to this if we query the MNMesh to ask about vertices. Do we actually
  ...
  if (options.nDataFillFlags & ALEMBIC_DATAFILL_UVS) {
    ...
    if (meshUvParam.valid()) {
      SampleInfo sampleInfo =
          getSampleInfo(sampleTime, meshUvParam.getTimeSampling(),
      ...
      bool uvFloor =
          getIndexAndValues(meshFaceIndices, meshUvParam, sampleInfo.floorIndex,
                            uvValuesFloor, uvIndicesFloor);
      bool uvCeil = false;
      if (!hasDynamicTopo) {
        uvCeil = getIndexAndValues(meshFaceIndices, meshUvParam,
      ...
      if (!uvFloor || uvValuesFloor.size() == 0) {
        ESS_LOG_WARNING(
            "Mesh UVs are in an invalid state in Alembic file, ignoring.");
      }
      else {
        ...
        if (options.pMNMesh->MNum() > uvI) {
          ...
          // set values
          Point3 *mapV = map->v;
          if (sampleInfo.alpha != 0.0f && uvCeil &&
              uvValuesFloor.size() == uvValuesCeil.size()) {
            for (int i = 0; i < uvValuesFloor.size(); i++) {
              Abc::V2f uv = uvValuesFloor[i] +
                            (uvValuesCeil[i] - uvValuesFloor[i]) *
                                float(sampleInfo.alpha);
          ...
          if (bBadUVindices) {
            ESS_LOG_WARNING(
                "Corrected out of bounds UV indices to prevent crash.");
          }
        }
        else {
          ESS_LOG_WARNING(
              "UV channels have not been allocated. Cannot load uv channel "
              << uvI);
        }
      }
    }
  ...
  if (options.nDataFillFlags & ALEMBIC_DATAFILL_MATERIALIDS) {
    ...
    if (objMesh.valid() &&
        objMesh.getSchema().getPropertyHeader(".materialids"))
      materialIds =
          Abc::IUInt32ArrayProperty(objMesh.getSchema(), ".materialids");
    else if (objSubD.valid() &&
             objSubD.getSchema().getPropertyHeader(".materialids"))
      materialIds =
          Abc::IUInt32ArrayProperty(objSubD.getSchema(), ".materialids");

    // If we don't detect a material id property then we try the facesets.  The
    // order in the face set array is assumed
    // to be the material id
    if (materialIds.valid() && materialIds.getNumSamples() > 0) {
      Abc::UInt32ArraySamplePtr materialIdsPtr =
          materialIds.getValue(sampleInfo.floorIndex);
      ...
      for (size_t j = 0; j < faceSetNames.size(); j++) {
        ...
        AbcG::IFaceSetSchema::Sample faceSetSample =
            faceSet.getValue(sampleInfo.floorIndex);
        ...
        if (matIDs.size() == faceSetNames.size()) {
          ...
          // ESS_LOG_WARNING("matID used: "<<nMatId<<" -- "<<(j < matIDs.size()
          // ? matIDs[j] : -1));
        }
  ...
  // This isn't required if we notify 3DS Max properly via the channel flags for
  // vertex changes.
  // options.pMNMesh->MNDebugPrint();
  if ((options.nDataFillFlags & ALEMBIC_DATAFILL_FACELIST) ||
...
bool AlembicImport_IsPolyObject(AbcG::IPolyMeshSchema::Sample &polyMeshSample)
{
  ...
  // Go through each face and check the number of vertices.  If a face does not
  // have 3 vertices,
...
void addAlembicMaterialsModifier(INode *pNode, AbcG::IObject &iObj)
{
  ...
  if (objMesh.valid()) {
    sampleInfo =
        getSampleInfo(nLastSample, objMesh.getSchema().getTimeSampling(),
                      objMesh.getSchema().getNumSamples());
  }
  else {
    sampleInfo =
        getSampleInfo(nLastSample, objSubD.getSchema().getTimeSampling(),
                      objSubD.getSchema().getNumSamples());
  }

  Abc::IStringArrayProperty matNamesProperty;
  if (objMesh.valid() &&
      objMesh.getSchema().getPropertyHeader(".materialnames")) {
    matNamesProperty =
        Abc::IStringArrayProperty(objMesh.getSchema(), ".materialnames");
  }
  else if (objSubD.valid() &&
           objSubD.getSchema().getPropertyHeader(".materialnames")) {
    matNamesProperty =
        Abc::IStringArrayProperty(objSubD.getSchema(), ".materialnames");
  }
  ...
  if (!matNamesProperty.valid() ||
    ...
    if (objMesh.valid()) {
      ...
      sampleInfo = getSampleInfo(0, objSubD.getSchema().getTimeSampling(),
                                 objSubD.getSchema().getNumSamples());
    }
    ...
    Abc::StringArraySamplePtr matNamesSamplePtr =
        matNamesProperty.getValue(sampleInfo.floorIndex);
  ...
  exeBuffer << "(\n";
  exeBuffer
      << "parameters AlembicMaterialPRM rollout:AlembicMaterialModifierRLT\n";
  exeBuffer << "(\n";
  ...
  exeBuffer << "custattributes.add mynode2113.modifiers[\"Alembic Materials\"] "
               "AlembicMaterialCA baseobject:false\n";

  for (size_t j = 0; j < faceSetNames.size(); j++) {
    const char *name = faceSetNames[j].c_str();
    exeBuffer << "mynode2113.modifiers[#Alembic_Materials]._materials["
              << (j + 1) << "] = \"" << name << "\" \n";
  }
...
int AlembicImport_PolyMesh(const std::string &path, AbcG::IObject &iObj,
                           alembic_importoptions &options, INode **pMaxNode)
{
  ...
  dataFillOptions.nDataFillFlags =
      ALEMBIC_DATAFILL_VERTEX | ALEMBIC_DATAFILL_FACELIST;
  dataFillOptions.nDataFillFlags |=
      options.importNormals ? ALEMBIC_DATAFILL_NORMALS : 0;
  dataFillOptions.nDataFillFlags |=
      options.importUVs ? ALEMBIC_DATAFILL_UVS : 0;
  dataFillOptions.nDataFillFlags |=
      options.importBboxes ? ALEMBIC_DATAFILL_BOUNDINGBOX : 0;
  dataFillOptions.nDataFillFlags |=
      options.importMaterialIds ? ALEMBIC_DATAFILL_MATERIALIDS : 0;
    ...
    if (bHashAvailable) {
      ...
      if (cachedNode) {
        ...
        if (options.enableInstances) {
          GET_MAX_INTERFACE()
              ->CloneNodes(nodes, offset, expandHierarchies, NODE_INSTANCE,
                           &resultSource, &resultTarget);
        }
        else if (options.enableReferences) {
          GET_MAX_INTERFACE()
              ->CloneNodes(nodes, offset, expandHierarchies, NODE_REFERENCE,
                           &resultSource, &resultTarget);
        }

        if (resultSource.Count() == resultTarget.Count() &&
            resultTarget.Count() == 1) {
  ...
  if (!options.attachToExisting && options.loadUserProperties) {
    setupPropertyModifiers(iObj, *pMaxNode, path, iObj.getFullName(), options,
                           std::string("Shape"));

    AbcG::IObject parentXform = iObj.getParent();
    if (parentXform.valid()) {
      setupPropertyModifiers(parentXform, *pMaxNode, path, iObj.getFullName(),
                             options);
    }
  }
  ...
  if (isAlembicMeshTopology(&iObj)) {
    ...
    if (bReplaceExistingModifiers) {
      pModifier = FindModifier(pNode, ALEMBIC_MESH_TOPO_MODIFIER_CLASSID,
                               identifier.c_str());
    }
    if (!pModifier) {
      pModifier = static_cast<Modifier *>(GET_MAX_INTERFACE()->CreateInstance(
          OSM_CLASS_ID, ALEMBIC_MESH_TOPO_MODIFIER_CLASSID));
    ...
    pModifier->GetParamBlockByID(0)->SetValue(
        GetParamIdByName(pModifier, 0, "path"), zero,
        EC_UTF8_to_TCHAR(path.c_str()));

    if (bCreatedModifier) {
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "identifier"), zero,
          EC_UTF8_to_TCHAR(identifier.c_str()));
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "time"), zero, 0.0f);
      if (isDynamicTopo) {
        pModifier->GetParamBlockByID(0)->SetValue(
            GetParamIdByName(pModifier, 0, "geometry"), zero, TRUE);
        pModifier->GetParamBlockByID(0)->SetValue(
            GetParamIdByName(pModifier, 0, "normals"), zero,
            (options.importNormals ? TRUE : FALSE));
        pModifier->GetParamBlockByID(0)->SetValue(
            GetParamIdByName(pModifier, 0, "uvs"), zero, FALSE);
      }
      else {
        pModifier->GetParamBlockByID(0)->SetValue(
            GetParamIdByName(pModifier, 0, "geometry"), zero, FALSE);
        pModifier->GetParamBlockByID(0)->SetValue(
            GetParamIdByName(pModifier, 0, "normals"), zero, FALSE);
        pModifier->GetParamBlockByID(0)->SetValue(
            GetParamIdByName(pModifier, 0, "uvs"), zero, FALSE);
      }

      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "muted"), zero, FALSE);
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "materialIDs"), zero,
          options.importMaterialIds);
  ...
  if (/*!isDynamicTopo &&*/ options
          .importUVs /*&& isAlembicMeshUVWs( &iObj, isUVWContant )*/) {
    ...
    if (meshUVsParam.valid()) {
      ...
      Abc::V2fArraySamplePtr meshUVs =
          meshUVsParam.getExpandedValue(0).getVals();
      if (meshUVs->size() > 0) {
        // check if we have a uv set names prop
        std::vector<std::string> uvSetNames;
        if (objMesh.valid() &&
            objMesh.getSchema().getPropertyHeader(".uvSetNames") != NULL) {
          Abc::IStringArrayProperty uvSetNamesProp =
              Abc::IStringArrayProperty(objMesh.getSchema(), ".uvSetNames");
          ...
          for (size_t i = 0; i < ptr->size(); i++) {
            uvSetNames.push_back(ptr->get()[i].c_str());
                 ...
                 objSubD.getSchema().getPropertyHeader(".uvSetNames") != NULL) {
          Abc::IStringArrayProperty uvSetNamesProp =
              Abc::IStringArrayProperty(objSubD.getSchema(), ".uvSetNames");
          Abc::StringArraySamplePtr ptr = uvSetNamesProp.getValue(0);
          for (size_t i = 0; i < ptr->size(); i++) {
            uvSetNames.push_back(ptr->get()[i].c_str());
          }
        ...
        for (int i = 0; i < uvSetNames.size(); i++) {
          ...
          bool bCreatedModifier = false;
          if (bReplaceExistingModifiers) {
            pModifier = FindModifier(pNode, ALEMBIC_MESH_UVW_MODIFIER_CLASSID,
                                     identifierStream.str().c_str());
          }
          if (!pModifier) {
            pModifier =
                static_cast<Modifier *>(GET_MAX_INTERFACE()->CreateInstance(
                    OSM_CLASS_ID, ALEMBIC_MESH_UVW_MODIFIER_CLASSID));
            bCreatedModifier = true;
          }
          pModifier->DisableMod();
          ...
          if (bCreatedModifier) {
            pModifier->GetParamBlockByID(0)->SetValue(
                GetParamIdByName(pModifier, 0, "identifier"), zero,
                EC_UTF8_to_TCHAR(identifierStream.str().c_str()));
            pModifier->GetParamBlockByID(0)->SetValue(
                GetParamIdByName(pModifier, 0, "time"), zero, 0.0f);
            pModifier->GetParamBlockByID(0)->SetValue(
                GetParamIdByName(pModifier, 0, "muted"), zero, FALSE);
          ...
          if (!meshUvParam.isConstant()) {
            ...
            AlembicImport_ConnectTimeControl(controllerName.str().c_str(),
                                             options);
          }
  ...
  if ((!isDynamicTopo) && isAlembicMeshPositions(&iObj, isGeomContant)) {
    ...
    bool bCreatedModifier = false;
    if (bReplaceExistingModifiers) {
      pModifier = FindModifier(pNode, ALEMBIC_MESH_GEOM_MODIFIER_CLASSID,
                               identifier.c_str());
    }
    if (!pModifier) {
      pModifier = static_cast<Modifier *>(GET_MAX_INTERFACE()->CreateInstance(
          OSM_CLASS_ID, ALEMBIC_MESH_GEOM_MODIFIER_CLASSID));
      bCreatedModifier = true;
    }
    ...
    if (bCreatedModifier) {
      pModifier->GetParamBlockByID(0)->SetValue(
          ...
          EC_UTF8_to_TCHAR(identifier.c_str()));
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "geoAlpha"), zero, 1.0f);
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "time"), zero, 0.0f);
      pModifier->GetParamBlockByID(0)->SetValue(
          GetParamIdByName(pModifier, 0, "muted"), zero, FALSE);
  ...
  if ((!isDynamicTopo) && isAlembicMeshNormals(&iObj, isNormalsContant)) {
    ...
    bool bCreatedModifier = false;
    if (bReplaceExistingModifiers) {
      pModifier = FindModifier(pNode, ALEMBIC_MESH_NORMALS_MODIFIER_CLASSID,
                               identifier.c_str());
    }
    if (!pModifier) {
      pModifier = static_cast<Modifier *>(GET_MAX_INTERFACE()->CreateInstance(
          OSM_CLASS_ID, ALEMBIC_MESH_NORMALS_MODIFIER_CLASSID));
      bCreatedModifier = true;
    }
  ...
  // SceneEntry *pEntry = options.sceneEnumProc.Append(pNode, newObject,
  // OBTYPE_MESH, &std::string(iObj.getFullName()));
  // options.currentSceneList.Append(pEntry);

  // Set the visibility controller
  AlembicImport_SetupVisControl(path.c_str(), identifier.c_str(), iObj, pNode,
                                options);
