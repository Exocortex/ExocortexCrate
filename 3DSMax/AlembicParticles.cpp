...
{
  return &s_AlembicParticlesClassDesc;
}
...
static ParamBlockDesc2 AlembicParticlesParams(
    0, _T(ALEMBIC_SIMPLE_PARTICLE_SCRIPTNAME), 0,
    GetAlembicParticlesClassDesc(), P_AUTO_CONSTRUCT | P_AUTO_UI | P_VERSION,
    ALEMBIC_PARTICLES_VERSION, 0,

    // rollout description
    IDD_ALEMBIC_PARTICLE_PARAMS, IDS_ALEMBIC, 0, 0, NULL,

    // params
    AlembicParticles::ID_PATH, _T("path"), TYPE_FILENAME, P_RESET_DEFAULT,
    IDS_PATH, p_default, "", p_ui, TYPE_EDITBOX, IDC_PATH_EDIT, p_assetTypeID,
    AssetManagement::kExternalLink, p_end,

    AlembicParticles::ID_IDENTIFIER, _T("identifier"), TYPE_STRING,
    ...
    p_default, 0.0f, p_range, 0.0f, 1000.0f, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT,
    IDC_TIME_EDIT, IDC_TIME_SPIN, 0.01f, p_end,

    AlembicParticles::ID_MUTED, _T("muted"), TYPE_BOOL, P_ANIMATABLE, IDS_MUTED,
    p_default, FALSE, p_ui, TYPE_SINGLECHEKBOX, IDC_MUTED_CHECKBOX, p_end,

    AlembicParticles::ID_VIEWPORT_PERCENT, _T("Viewport %"), TYPE_FLOAT,
    P_ANIMATABLE, IDS_VIEWPORT_PERCENT, p_default, 100.0f, p_range, 0.0f,
    100.0f, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_VIEWPORT_PERCENT_EDIT,
    IDC_VIEWPORT_PERCENT_SPIN, 1.0f, p_end,

    AlembicParticles::ID_RENDER_AS_TICKS, _T("Render as ticks"), TYPE_BOOL,
    P_ANIMATABLE, IDS_RENDER_AS_TICKS, p_default, FALSE, p_ui,
    TYPE_SINGLECHEKBOX, IDC_RENDER_AS_TICKS_CHECKBOX, p_end,
...
AlembicParticles::AlembicParticles() : SimpleParticle()
{
  pblock2 = NULL;
  m_pBoxMaker = NULL;
  m_pSphereMaker = NULL;
...
void AlembicParticles::clearViewportMeshCache()
{
  ...
  for (InstanceMeshCache::iterator it = m_InstanceMeshCache.begin();
       it != m_InstanceMeshCache.end(); it++) {
...
void AlembicParticles::UpdateParticles(TimeValue t, INode *node)
{
  ...
  this->pblock2->GetValue(AlembicParticles::ID_IDENTIFIER, t, strIdentifier,
                          interval);
  ...
  BOOL bRenderAsTicks;
  this->pblock2->GetValue(AlembicParticles::ID_RENDER_AS_TICKS, t,
                          bRenderAsTicks, interval);
  ...
  this->pblock2->GetValue(AlembicParticles::ID_VIEWPORT_PERCENT, t,
                          m_fViewportPercent, interval);
  ...
  SampleInfo sampleInfo =
      GetSampleAtTime(m_iPoints, iSampleTime, floorSample, ceilSample);
  ...
  GetParticlePositions(m_iPoints, floorSample, ceilSample, sampleInfo,
                       m_objToWorld, parts.points);
  GetParticleVelocities(floorSample, ceilSample, sampleInfo, m_objToWorld,
                        parts.vels);
  GetParticleRadii(m_iPoints, sampleInfo, parts.radius);
  GetParticleAges(m_iPoints, sampleInfo, parts.ages);
  GetParticleOrientations(m_iPoints, sampleInfo, m_objToWorld,
                          m_ParticleOrientations);
...
void AlembicParticles::BuildEmitter(TimeValue t, Mesh &mesh)
{
  ...
  MCHAR const* strPath = NULL;
      ...
      MCHAR const* strIdentifier = NULL;
      this->pblock2->GetValue( AlembicParticles::ID_IDENTIFIER, t,
  ...
  Abc::V3f alembicMinPt(float(bbox.min.x), float(bbox.min.y),
  float(bbox.min.z));
  Abc::V3f alembicMaxPt(float(bbox.max.x), float(bbox.max.y),
  float(bbox.max.z));
  Point3 maxMinPt;
  Point3 maxMaxPt;
  ...
  Point3 minPt(min(maxMinPt.x, maxMaxPt.x) - EPSILON, min(maxMinPt.y,
  maxMaxPt.y) - EPSILON, min(maxMinPt.z, maxMaxPt.z) - EPSILON);
  Point3 maxPt(max(maxMinPt.x, maxMaxPt.x) + EPSILON, max(maxMinPt.y,
  maxMaxPt.y) + EPSILON, max(maxMinPt.z, maxMaxPt.z) + EPSILON);
...
MarkerType AlembicParticles::GetMarkerType() { return POINT_MRKR; }
...
/**		Returns the world space size of the specified particle in at the
time
passed.
...
/**		Returns a value indicating where the particle geometry (mesh)
lies in
...
/**	Returns the age of the specified particle -- the length of time it has
been
'alive'.
The Particle Age texture map and the Particle Motion Blur texture map use
this method.
...
/**		Returns the life of the particle -- the length of time the
particle will be
'alive'.
The Particle Age texture map and the Particle Motion Blur texture map use
this method.
...
\param i The index of the particle. */
TimeValue AlembicParticles::ParticleLife(TimeValue t, int i) { return -1; }
bool AlembicParticles::GetAlembicIPoints(AbcG::IPoints &iPoints,
                                         std::string strFile,
...
SampleInfo AlembicParticles::GetSampleAtTime(
    AbcG::IPoints &iPoints, TimeValue t,
    AbcG::IPointsSchema::Sample &floorSample,
    AbcG::IPointsSchema::Sample &ceilSample) const
{
  ESS_PROFILE_FUNC();
  double sampleTime = GetSecondsFromTimeValue(t);
  SampleInfo sampleInfo =
      getSampleInfo(sampleTime, iPoints.getSchema().getTimeSampling(),
...
int AlembicParticles::GetNumParticles(
    const AbcG::IPointsSchema::Sample &floorSample) const
{
...
void AlembicParticles::GetParticlePositions(
    AbcG::IPoints &iPoints, const AbcG::IPointsSchema::Sample &floorSample,
    const AbcG::IPointsSchema::Sample &ceilSample, const SampleInfo &sampleInfo,
    const Matrix3 &objToWorld, Tab<Point3> &points) const
{
  ESS_PROFILE_FUNC();
  Abc::P3fArraySamplePtr floorPositions = floorSample.getPositions();
  if (floorPositions != NULL && floorPositions->valid() &&
      floorPositions->size() > 0) {
    ...
    for (int i = 0; i < alembicPositions.size(); i++) {
      points[i] =
          ConvertAlembicPointToMaxPoint(alembicPositions[i]) * objToWorld;
    }
  }
}
void AlembicParticles::GetParticleVelocities(
    const AbcG::IPointsSchema::Sample &floorSample,
    const AbcG::IPointsSchema::Sample &ceilSample, const SampleInfo &sampleInfo,
    Matrix3 objToWorld, Tab<Point3> &vels) const
{
  // ESS_LOG_WARNING( "Particle velocities are not transformed into world
  // space." );
  ESS_PROFILE_FUNC();
  ...
  Abc::V3fArraySamplePtr floorVelocities = floorSample.getVelocities();
  if (floorVelocities != NULL && floorVelocities->valid() &&
      floorVelocities->size() > 0) {
    int j = 0;
...
void AlembicParticles::GetParticleIds(
    const AbcG::IPointsSchema::Sample &floorSample,
    const SampleInfo &sampleInfo, std::vector<Abc::uint64_t> &idVec) const
{
  ESS_PROFILE_FUNC();
...
void AlembicParticles::GetParticleRadii(AbcG::IPoints &iPoints,
                                        const SampleInfo &sampleInfo,
                                        Tab<float> &radius) const
{
  ESS_PROFILE_FUNC();
  bool useDefaultValues = true;
  AbcG::IFloatGeomParam widthsParam = iPoints.getSchema().getWidthsParam();
  if (widthsParam != NULL && widthsParam.getNumSamples() > 0) {
    Abc::FloatArraySamplePtr floorSamples =
        widthsParam.getExpandedValue(sampleInfo.floorIndex).getVals();
    if (floorSamples != NULL && floorSamples->valid() &&
        floorSamples->size() > 0) {
      int j = 0;
...
void AlembicParticles::GetParticleAges(AbcG::IPoints &iPoints,
                                       const SampleInfo &sampleInfo,
                                       Tab<TimeValue> &ages) const
{
  ESS_PROFILE_FUNC();
  bool useDefaultValues = true;
...
void AlembicParticles::GetParticleOrientations(
    AbcG::IPoints &iPoints, const SampleInfo &sampleInfo, Matrix3 objToWorld,
    std::vector<Quat> &particleOrientations) const
{
  ESS_PROFILE_FUNC();
  // ESS_LOG_WARNING( "Particle orientations are not transformed into world
  // space." );
  bool useDefaultValues = true;
  ...
  if (getArbGeomParamPropertyAlembic(iPoints, "orientation", orientProperty)) {
    Abc::QuatfArraySamplePtr floorSamples =
        orientProperty.getValue(sampleInfo.floorIndex);
    if (floorSamples != NULL && floorSamples->valid() &&
        floorSamples->size() > 0) {
      std::vector<Quat> maxOrientations(particleOrientations.size());
      int j = 0;
      int jIncrement =
          floorSamples->size() == particleOrientations.size() ? 1 : 0;
      ...
      if (sampleInfo.alpha != 0.0f) {
        ...
        if (getArbGeomParamPropertyAlembic(iPoints, "angularvelocity",
                                           angVelProperty)) {
          float timeAlpha = getTimeOffsetFromObject(iPoints, sampleInfo);

          Abc::QuatfArraySamplePtr floorAngVelSamples =
              angVelProperty.getValue(sampleInfo.floorIndex);
          if (floorAngVelSamples != NULL && floorAngVelSamples->size() > 0) {
            j = 0;
            jIncrement =
                floorAngVelSamples->size() == particleOrientations.size() ? 1
                                                                          : 0;
            for (int i = 0; i < maxOrientations.size(); i++) {
              Quat q = maxOrientations[i];
              Quat v =
                  ConvertAlembicQuatToMaxQuat((*floorAngVelSamples)[j], false);
              j += jIncrement;
              v = v * timeAlpha;
              if (v.w != 0.0f) {
                q = v * q;
...
void AlembicParticles::GetParticleScales(AbcG::IPoints &iPoints,
                                         const SampleInfo &sampleInfo,
                                         const Matrix3 &objToWorld,
                                         std::vector<Point3> &scales) const
{
  ESS_PROFILE_FUNC();
...
void AlembicParticles::GetParticleShapeTypes(
    AbcG::IPoints &iPoints, const SampleInfo &sampleInfo,
    std::vector<AlembicPoints::ShapeType> &instanceShapeType) const
{
  ESS_PROFILE_FUNC();
  bool useDefaultValues = true;
  Abc::IUInt16ArrayProperty shapeTypeProperty;
  if (getArbGeomParamPropertyAlembic(iPoints, "shapetype", shapeTypeProperty)) {
    Abc::UInt16ArraySamplePtr floorSamples =
        shapeTypeProperty.getValue(sampleInfo.floorIndex);
    if (floorSamples != NULL && floorSamples->size() > 0) {
      int j = 0;
      int jIncrement = floorSamples->size() == instanceShapeType.size() ? 1 : 0;
      for (int i = 0; i < instanceShapeType.size(); i++) {
        instanceShapeType[i] =
            static_cast<AlembicPoints::ShapeType>((*floorSamples)[j]);
        j += jIncrement;
      }
      useDefaultValues = false;
    }
...
void AlembicParticles::GetParticleShapeInstanceIds(
    AbcG::IPoints &iPoints, const SampleInfo &sampleInfo,
    std::vector<unsigned short> &instanceShapeIds) const
{
  ESS_PROFILE_FUNC();
  bool useDefaultValues = true;
  Abc::IUInt16ArrayProperty shapeIdProperty;
  if (getArbGeomParamPropertyAlembic(iPoints, "shapeinstanceid",
                                     shapeIdProperty)) {
    Abc::IStringArrayProperty shapeNameProperty;
    if (getArbGeomParamPropertyAlembic(iPoints, "instancenames",
                                       shapeNameProperty)) {
      Abc::UInt16ArraySamplePtr floorSamples =
          shapeIdProperty.getValue(sampleInfo.floorIndex);
      if (floorSamples != NULL && floorSamples->size() > 0) {
        int j = 0;
        int jIncrement =
            floorSamples->size() == instanceShapeIds.size() ? 1 : 0;
...
void AlembicParticles::FillParticleShapeNodes(AbcG::IPoints &iPoints,
                                              const SampleInfo &sampleInfo)
{
  ESS_PROFILE_FUNC();
  ...
  if (getArbGeomParamPropertyAlembic(iPoints, "instancenames",
                                     shapeInstanceNameProperty)) {
    const size_t sIndex = shapeInstanceNameProperty.getNumSamples() - 1;
    m_InstanceShapeNames =
        shapeInstanceNameProperty.getValue(sIndex);  // sampleInfo.floorIndex);
...
void AlembicParticles::GetParticleShapeInstanceTimes(
    AbcG::IPoints &iPoints, const SampleInfo &sampleInfo,
    std::vector<TimeValue> &instanceShapeTimes) const
{
  ESS_PROFILE_FUNC();
  bool useDefaultValues = true;
  ...
  if (getArbGeomParamPropertyAlembic(iPoints, "shapetime", shapeTimeProperty)) {
    Abc::FloatArraySamplePtr floorSamples =
        shapeTimeProperty.getValue(sampleInfo.floorIndex);
    if (floorSamples != NULL && floorSamples->size() > 0) {
      int j = 0;
      int jIncrement =
          floorSamples->size() == instanceShapeTimes.size() ? 1 : 0;
...
void AlembicParticles::GetParticleColors(AbcG::IPoints &iPoints,
                                         const SampleInfo &sampleInfo,
                                         std::vector<VertColor> &colors) const
{
  ESS_PROFILE_FUNC();
  bool useDefaultValues = true;
  Abc::IC4fArrayProperty colorProperty;
  if (getArbGeomParamPropertyAlembic(iPoints, "color", colorProperty)) {
    Abc::C4fArraySamplePtr floorSamples =
        colorProperty.getValue(sampleInfo.floorIndex);
    if (floorSamples != NULL && floorSamples->size() > 0) {
      int j = 0;
...
Mesh *AlembicParticles::GetMultipleRenderMesh(TimeValue t, INode *inode,
                                              View &view, BOOL &needDelete,
                                              int meshNumber)
{
  // ESS_LOG_INFO( "AlembicParticles::GetMultipleRenderMesh( t: " << t << "
  // meshNumber: " << meshNumber << ", t: " << t << " )" );
  return GetMultipleRenderMesh_Internal(t, inode, view, needDelete, meshNumber,
                                        false);
}

Mesh *AlembicParticles::GetMultipleRenderMesh_Internal(TimeValue t,
                                                       INode *inode, View &view,
                                                       BOOL &needDelete,
                                                       int meshNumber,
                                                       bool bUseCache)
{
  ...
  switch (m_InstanceShapeType[meshNumber]) {
    ...
    case AlembicPoints::ShapeType_Instance:
      pMesh =
          BuildInstanceMesh(meshNumber, t, inode, view, needDelete, bUseCache);
      break;
...
Mesh *AlembicParticles::GetRenderMesh(TimeValue t, INode *inode, View &view,
                                      BOOL &needDelete)
{
...

  {
    ...
    for (int i = 0; i < NumberOfRenderMeshes(); i++) {
      ...
      Mesh *pMesh =
          GetMultipleRenderMesh_Internal(t, inode, nullView, curNeedDelete, i);
  ...
  for (int i = 0; i < NumberOfRenderMeshes(); i++) {
    ...
    Interval meshTMValid = FOREVER;
    {
      ...
      GetMultipleRenderMeshTM_Internal(t, inode, nullView, i, meshTM,
                                       meshTMValid);
    }
    ...
    // the following two steps are necessary because meshTM can contain a scale
    // factor
...

    {
      ...
      if (pRenderMeshNormalSpec) {
        ...
        if (pNormalSpec && curNumFaces == pNormalSpec->GetNumFaces()) {
          ESS_PROFILE_SCOPE(
              "GetRenderMesh - Build and set normals - copy normals from "
              "normal spec");

          for (int j = 0, curIndex = faceOffset; j < curNumFaces;
               j++, curIndex++) {
            mpFace[curIndex].SetNormalID(
                0, pNormalSpec->Face(j).GetNormalID(0) + normOffset);
            mpFace[curIndex].SetNormalID(
                1, pNormalSpec->Face(j).GetNormalID(1) + normOffset);
            mpFace[curIndex].SetNormalID(
                2, pNormalSpec->Face(j).GetNormalID(2) + normOffset);
          ...
          ESS_PROFILE_SCOPE(
              "GetRenderMesh - Build and set normals - compute from smoothing "
              "groups");
          ...
          for (int j = 0, curIndex = faceOffset; j < curNumFaces;
               j++, curIndex++) {
            for (int f = 0; f < 3; f++) {
              pRenderMeshNormalSpec->SetNormal(
                  curIndex, f,
                  sgNormals.GetVertexNormal(pMesh, j, f) * meshTM_I_T);
            }
          }
...

    {
      ...
      for (int mp = 0; mp < numMaps; mp++) {
        ...
        if (tvertOffset[mp] == 0) {  // the map channel needs expansion
          ...
          int numMapVerts =
              3 * faceNum +
              1;  // triple number of faces covers the maximum + one extra
        ...
        for (int j = 0, curIndex = tvertOffset[mp]; j < tvertsToAdd;
             j++, curIndex++) {
...
void AlembicParticles::GetMultipleRenderMeshTM(TimeValue t, INode *inode,
                                               View &view, int meshNumber,
                                               Matrix3 &meshTM,
                                               Interval &meshTMValid)
{
  ...
  if (LOG) {
    ESS_LOG_WARNING("GetMultipleRenderMeshTM_Internal at time = "
                    << t << ", mesh #:" << meshNumber
                    << ", numOfParticles: " << parts.Count());
  }

  GetMultipleRenderMeshTM_Internal(t, inode, view, meshNumber, meshTM,
                                   meshTMValid);
...
void AlembicParticles::GetMultipleRenderMeshTM_Internal(
    TimeValue t, INode *inode, View &view, int meshNumber, Matrix3 &meshTM,
    Interval &meshTMValid, bool bCalledFromViewport)
{
  ESS_PROFILE_FUNC();
  ...
  // for scanline motion blur we need to be able sample at numerous times. So we
  // can't read from the stored state directly
  // also, Mental Ray seems to sample at some offset before the current tick,
  // but only once per particle
  // the velocity array is used instead of multiple calls to the MeshTm method
  if (t != m_currTick) {
    const float tDiff = static_cast<float>(GetSecondsFromTimeValue(t) -
                                           GetSecondsFromTimeValue(m_currTick));

    pos += parts.vels[meshNumber] * TIME_TICKSPERSEC * tDiff;

    if (m_outputOrientationMotionBlurWarning) {
      ESS_LOG_WARNING(
          "Not advancing orientation of particles for sub-sample motion blur.");
    ...
    // Point3 pos2 = pos + static_cast<float>(sampleInfo.alpha) *
    // parts.vels[meshNumber];
  }
...
{
  ESS_PROFILE_FUNC();
  // ESS_LOG_INFO( "AlembicParticles::GetParticleMeshNode( meshNumber: " <<
  // meshNumber << " )" );
  if (meshNumber > parts.Count() || !parts.Alive(meshNumber)) {
    return displayNode;
  }
...
// Note: the display and hittest methods called often, so we definitely should
// have caching
int AlembicParticles::Display(TimeValue t, INode *inode, ViewExp *vpt,
                              int flags)
{
  ESS_PROFILE_FUNC();
  if (!OKtoDisplay(t)) {
  ...
  gw->setRndLimits(GW_WIREFRAME | GW_EDGES_ONLY |
                   (rlim & GW_Z_BUFFER));  // removed BC on 4/29/99 DB
  ...
  // the particles fail to clear properly in 3DS MAX 2010 if the we do not
  ...
  int nNumParticlesToRender =
      (int)ceil((float)NumberOfRenderMeshes() * fFraction);
  ...
  if (m_bRenderAsTicks) {
    ...
    for (int ii = 0;; ii++) {
      ...
      Mesh *mesh = NULL;
      {
        ...
        GetMultipleRenderMeshTM_Internal(t, inode, nullView, i, elemToObj,
                                         meshTMValid, true);
      }
      {
        ESS_PROFILE_SCOPE("Display::GetMultipleRenderMesh_Internal");
        mesh =
            GetMultipleRenderMesh_Internal(t, inode, nullView, deleteMesh, i);
      }
      ...
      if (mesh && m_InstanceShapeType[i] != AlembicPoints::ShapeType_Point) {
        ...
        gw->setColor(FILL_COLOR, m_VCArray[i].x, m_VCArray[i].y,
                     m_VCArray[i].z);
        gw->setTransform(Matrix3(1));
        gw->marker(&parts.points[i], POINT_MRKR);
      }
...
int AlembicParticles::HitTest(TimeValue t, INode *inode, int type, int crossing,
...
Mesh *AlembicParticles::BuildPointMesh(int meshNumber, TimeValue t, INode *node,
                                       View &view, BOOL &needDelete)
{
  ESS_PROFILE_FUNC();
  // Mesh *pMesh = NULL;
...
Mesh *AlembicParticles::BuildBoxMesh(int meshNumber, TimeValue t, INode *node,
                                     View &view, BOOL &needDelete)
{
  ESS_PROFILE_FUNC();
  Mesh *pMesh = NULL;
  ...
  if (!m_pBoxMaker) {
    m_pBoxMaker =
        static_cast<GenBoxObject *>(GET_MAX_INTERFACE()->CreateInstance(
...
Mesh *AlembicParticles::BuildSphereMesh(int meshNumber, TimeValue t,
                                        INode *node, View &view,
                                        BOOL &needDelete)
{
  ESS_PROFILE_FUNC();
  Mesh *pMesh = NULL;
  ...
  if (!m_pSphereMaker) {
    m_pSphereMaker =
        static_cast<GenSphere *>(GET_MAX_INTERFACE()->CreateInstance(
            GEOMOBJECT_CLASS_ID, Class_ID(SPHERE_CLASS_ID, 0)));
...
Mesh *AlembicParticles::BuildCylinderMesh(int meshNumber, TimeValue t,
                                          INode *node, View &view,
                                          BOOL &needDelete)
{
  ESS_PROFILE_FUNC();
  ...
  if (!m_pCylinderMaker) {
    m_pCylinderMaker =
        static_cast<GenCylinder *>(GET_MAX_INTERFACE()->CreateInstance(
            GEOMOBJECT_CLASS_ID, Class_ID(CYLINDER_CLASS_ID, 0)));
...
Mesh *AlembicParticles::BuildConeMesh(int meshNumber, TimeValue t, INode *node,
                                      View &view, BOOL &needDelete)
{
  ESS_PROFILE_FUNC();
  // 3DS Max doesn't seem to have a cone builder, so just return a cylinder
  // fornow.
  return BuildCylinderMesh(meshNumber, t, node, view, needDelete);
}

Mesh *AlembicParticles::BuildDiscMesh(int meshNumber, TimeValue t, INode *node,
                                      View &view, BOOL &needDelete)
{
  ESS_PROFILE_FUNC();
  Mesh *pMesh = NULL;
...
Mesh *AlembicParticles::BuildRectangleMesh(int meshNumber, TimeValue t,
                                           INode *node, View &view,
                                           BOOL &needDelete)
{
  ESS_PROFILE_FUNC();
...
Mesh *AlembicParticles::BuildInstanceMesh(int meshNumber, TimeValue t,
                                          INode *node, View &view,
                                          BOOL &needDelete, bool bUseCache)
{
  ESS_PROFILE_FUNC()
  needDelete = FALSE;
...
Mesh *AlembicParticles::BuildNbElementsMesh(int meshNumber, TimeValue t,
                                            INode *node, View &view,
                                            BOOL &needDelete)
{
  Mesh *pMesh = NULL;
...
RefResult AlembicParticles::NotifyRefChanged(const Interval &iv,
                                             RefTargetHandle hTarg,
                                             PartID &partID, RefMessage msg,
                                             BOOL propagate)
#else
RefResult AlembicParticles::NotifyRefChanged(Interval iv, RefTargetHandle hTarg,
                                             PartID &partID, RefMessage msg)
#endif
{
  ESS_PROFILE_FUNC();
...
void AlembicParticles::BeginEditParams(IObjParam *ip, ULONG flags,
                                       Animatable *prev)
{
  this->ip = ip;
  s_EditObject = this;
...
void AlembicParticles::EndEditParams(IObjParam *ip, ULONG flags,
                                     Animatable *next)
{
...
RefTargetHandle AlembicParticles::Clone(RemapDir &remap)
{
  ...
  particle->ReplaceReference(ALEMBIC_SIMPLE_PARTICLE_REF_PBLOCK2,
                             remap.CloneRef(pblock2));
...
int AlembicImport_Points(const std::string &file, AbcG::IObject &iObj,
                         alembic_importoptions &options, INode **pMaxNode)
{
  ...
  AlembicParticles *pParticleObj =
      static_cast<AlembicParticles *>(GetCOREInterface()->CreateInstance(
          GEOMOBJECT_CLASS_ID, ALEMBIC_SIMPLE_PARTICLE_CLASSID));
  ...
  TimeValue zero(0);
  pParticleObj->GetParamBlockByID(0)->SetValue(
      GetParamIdByName(pParticleObj, 0, "path"), zero,
      EC_UTF8_to_TCHAR(file.c_str()));
  pParticleObj->GetParamBlockByID(0)->SetValue(
      GetParamIdByName(pParticleObj, 0, "identifier"), zero,
      EC_UTF8_to_TCHAR(identifier.c_str()));
  pParticleObj->GetParamBlockByID(0)->SetValue(
      GetParamIdByName(pParticleObj, 0, "time"), zero, 0.0f);
  pParticleObj->GetParamBlockByID(0)->SetValue(
      GetParamIdByName(pParticleObj, 0, "muted"), zero, FALSE);

  // Create the object node
  Abc::IObject parent = iObj.getParent();
  std::string name = removeXfoSuffix(parent.getName().c_str());
  INode *pNode = GET_MAX_INTERFACE()->CreateObjectNode(
      pParticleObj, EC_UTF8_to_TCHAR(name.c_str()));
  if (pNode == NULL) {
    return alembic_failure;
  }
  *pMaxNode = pNode;
  ...
  if (!options.attachToExisting && options.loadUserProperties) {
    setupPropertyModifiers(iObj, *pMaxNode, file, iObj.getFullName(), options,
                           std::string("Shape"));

    AbcG::IObject parentXform = iObj.getParent();
    if (parentXform.valid()) {
      setupPropertyModifiers(parentXform, *pMaxNode, file, iObj.getFullName(),
                             options);
    }
  }
  ...
  SceneEntry *pEntry = options.sceneEnumProc.Append(
      pNode, pParticleObj, OBTYPE_POINTS, &std::string(iObj.getFullName()));
  options.currentSceneList.Append(pEntry);

  // Set the visibility controller
  AlembicImport_SetupVisControl(file.c_str(), identifier.c_str(), iObj, pNode,
                                options);

  // if( !isConstant )
  // our isConstant only takes into account point size, but other things such as
  // the shape mesh can change
  {
