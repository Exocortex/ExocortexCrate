...
bool SceneNodeMax::addChild(SceneNodeAlembicPtr fileNode,
                            const IJobStringParser& jobParams,
...
Imath::M44f SceneNodeMax::getGlobalTransFloat(double time)
{
  ...
  return Abc::M44f(alembicMatrix.GetRow(0).x, alembicMatrix.GetRow(0).y,
...
Imath::M44d SceneNodeMax::getGlobalTransDouble(double time)
{
  ...
  Matrix3 alembicMatrix;
  ConvertMaxMatrixToAlembicMatrix(out, alembicMatrix);
  return Abc::M44d(alembicMatrix.GetRow(0).x, alembicMatrix.GetRow(0).y,
                   alembicMatrix.GetRow(0).z, 0, alembicMatrix.GetRow(1).x,
                   alembicMatrix.GetRow(1).y, alembicMatrix.GetRow(1).z, 0,
                   alembicMatrix.GetRow(2).x, alembicMatrix.GetRow(2).y,
                   alembicMatrix.GetRow(2).z, 0, alembicMatrix.GetRow(3).x,
...
MeshData SceneNodeMax::getMeshData(double time)
{
  ...
  if (meshData.obj->CanConvertToType(Class_ID(POLYOBJ_CLASS_ID, 0))) {
    meshData.polyObj = reinterpret_cast<PolyObject*>(
        meshData.obj->ConvertToType(ticks, Class_ID(POLYOBJ_CLASS_ID, 0)));
    meshData.polyMesh = &meshData.polyObj->GetMesh();
  }
  else if (meshData.obj->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0))) {
    meshData.triObj = reinterpret_cast<TriObject*>(
        meshData.obj->ConvertToType(ticks, Class_ID(TRIOBJ_CLASS_ID, 0)));
...
Mtl* SceneNodeMax::getMtl() { return node->GetMtl(); }
int SceneNodeMax::getMtlID() { return -1; }
SceneNode::nodeTypeE getNodeType(INode* node)
{
  ...
  if (obj->CanConvertToType(MATTERWAVES_CLASS_ID)) {
    ESS_LOG_WARNING(
        "We only support merged mesh export for thinking particles.");
...
struct CSGStackElement {
  ...
  //{}
  CSGStackElement(INode* inode, SceneNodePtr enode, std::string p)
      : iNode(inode), eNode(enode), path(p)
  {
  }
};

SceneNodeMaxPtr createNodeMax(INode* pNode, SceneNode::nodeTypeE type,
                              bool isCameraTransform,
                                      ...
                                      bcsgSelection::types selectionOption)
{
  ...
  SceneNodeMaxPtr exoRoot =
      createNodeMax(pRootNode, SceneNode::SCENE_ROOT, false, selectionOption,
                    std::string("/"));
  ...
  while (!sceneStack.empty()) {
    ...
    std::string path = sElement.path;
    path += "/";
    path += EC_MCHAR_to_UTF8(pNode->GetName());
    sceneStack.pop_back();
    ...
    if (isShapeNode(type)) {
      newNode = createNodeMax(pNode, SceneNode::ETRANSFORM,
                              type == SceneNode::CAMERA, selectionOption, path);
      // newNode->name+="Xfo";

      std::stringstream geoPath;
      geoPath << path << "/" << EC_MCHAR_to_UTF8(pNode->GetName()) << "Shape";
      SceneNodePtr geoNode =
          createNodeMax(pNode, type, false, bcsgSelection::NONE, geoPath.str());
      geoNode->name += "Shape";
    ...
    //}
    // else{ //import
    //    newNode = createNodeMax(xRef, type);//doesn't identify EXTRANFORMS
    //    because these are built-in to the shape node
    //    if(type == SceneNode::ITRANSFORM || type ==
    //    SceneNode::NAMESPACE_TRANSFORM){
    //       //newNode->name+="Xfo";
    //    }
    // else{
    // //newNode->name+="Shape";
