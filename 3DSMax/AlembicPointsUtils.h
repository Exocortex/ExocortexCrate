...
class particleGroupInterface {
 public:
  ...
  // I use this std::map to keep to obtain iteration index for each particle
  // group.
  ...
  particleGroupInterface(IParticleObjectExt* particlesExt, Object* obj,
                         INode* node, ExoNullView* pNullView)
      : m_pParticlesExt(particlesExt),
        m_pSystemObject(obj),
        m_pSystemNode(node),
        m_pNullView(pNullView),
        m_pCurrParticleGroup(NULL),
        m_nCurrParticleGroupId(-1),
        m_pCurrGroupContainer(NULL),
        m_pCurrGroupSystem(NULL),
        m_pCurrGroupMtl(NULL),
        m_pCurrRender(NULL)
  {
  }
