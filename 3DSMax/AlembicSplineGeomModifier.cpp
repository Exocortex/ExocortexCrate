...
ClassDesc2* GetAlembicSplineGeomModifierClassDesc() {return &AlembicSplineGeomModifierDesc;}

//--- Properties block -------------------------------

static ParamBlockDesc2 AlembicSplineGeomModifierParams(
	0,
	_T(ALEMBIC_SPLINE_GEOM_MODIFIER_SCRIPTNAME),
	0,
    ...
    // params
	AlembicSplineGeomModifier::ID_PATH, _T("path"), TYPE_FILENAME, P_RESET_DEFAULT, IDS_PATH,
	    p_default, "",
	    p_ui,        TYPE_EDITBOX,		IDC_PATH_EDIT,
		p_assetTypeID,	kExternalLink,
		p_end,
        
	AlembicSplineGeomModifier::ID_IDENTIFIER, _T("identifier"), TYPE_STRING, P_RESET_DEFAULT, IDS_IDENTIFIER,
	    p_default, "",
	    p_ui,        TYPE_EDITBOX,		IDC_IDENTIFIER_EDIT,
		p_end,

	AlembicSplineGeomModifier::ID_TIME, _T("time"), TYPE_FLOAT, P_ANIMATABLE, IDS_TIME,
		p_default,       0.0f,
		p_range,         0.0f, 1000.0f,
		p_ui,            TYPE_SPINNER,       EDITTYPE_FLOAT, IDC_TIME_EDIT,    IDC_TIME_SPIN, 0.01f,
		p_end,

	AlembicSplineGeomModifier::ID_MUTED, _T("muted"), TYPE_BOOL, P_ANIMATABLE, IDS_MUTED,
		p_default,       TRUE,
...
void AlembicSplineGeomModifier::EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags)  {
...
void AlembicSplineGeomModifier::ModifyObject (TimeValue t, ModContext &mc, ObjectState *os, INode *node) 
{
...
ESS_PROFILE_FUNC();
	...
	this->pblock->GetValue( AlembicSplineGeomModifier::ID_PATH, t, strPath, interval);

	MCHAR const* strIdentifier = NULL;
	this->pblock->GetValue( AlembicSplineGeomModifier::ID_IDENTIFIER, t, strIdentifier, interval);
 
	float fTime;
	this->pblock->GetValue( AlembicSplineGeomModifier::ID_TIME, t, fTime, interval);
	...
	BOOL bMuted;
	this->pblock->GetValue( AlembicSplineGeomModifier::ID_MUTED, t, bMuted, interval);
...
RefResult AlembicSplineGeomModifier::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget,
										   PartID& partID, RefMessage message, BOOL propagate) {
#else
RefResult AlembicSplineGeomModifier::NotifyRefChanged (Interval changeInt, RefTargetHandle hTarget,
...
void AlembicSplineGeomModifier::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
{
	this->ip = ip;
    editMod  = this;
...
void AlembicSplineGeomModifier::EndEditParams( IObjParam *ip, ULONG flags, Animatable *next )
{
