...
AlembicXForm::AlembicXForm(SceneNodePtr eNode, AlembicWriteJob *in_Job,
                           Abc::OObject oParent)
    : AlembicObject(eNode, in_Job, oParent), customAttributes("User Properties")
{
  AbcG::OXform xform(GetOParent(), eNode->name,
                     GetCurrentJob()->GetAnimatedTs());

  mXformSchema = xform.getSchema();

  mOVisibility =
      CreateVisibilityProperty(xform, GetCurrentJob()->GetAnimatedTs());

  if (!bMergedSubtreeNodeParent) {
    Abc::OCompoundProperty userProperties = mXformSchema.getUserProperties();
    customAttributes.defineCustomAttributes(mMaxNode, userProperties,
                                            mXformSchema.getMetaData(),
                                            GetCurrentJob()->GetAnimatedTs());
  }
}
...
{
  // we have to clear this prior to destruction this is a workaround for
  // issue-171
  mOVisibility.reset();
}
...
bool AlembicXForm::Save(double time, bool bLastFrame)
{
  ...
  if (this->mExoSceneNode->type == SceneNode::ITRANSFORM) {
    ...
    // thus, since Zmin < zMax, then -Zmin > -Zmax. So we need to swap the z
    // components
  ...
  // SaveUserProperties(mExoSceneNode, mXformSchema,
  // GetCurrentJob()->GetAnimatedTs(), time, bFirstFrame);
  ...
  if (bFlatten) {  // a hack needed to handle namespace xforms
    ...
    Imath::M44d parentGlobalTransInv =
        mExoSceneNode->parent->getGlobalTransDouble(time).invert();
    Imath::M44d transform =
        mExoSceneNode->getGlobalTransDouble(time) * parentGlobalTransInv;
    mXformSample.setMatrix(transform);
    mXformSchema.set(mXformSample);
  }

  bool bVisibility = mExoSceneNode->getVisibility(time);
  mOVisibility.set(bVisibility ? AbcG::kVisibilityVisible
...
Abc::OCompoundProperty AlembicXForm::GetCompound() { return mXformSchema; }
