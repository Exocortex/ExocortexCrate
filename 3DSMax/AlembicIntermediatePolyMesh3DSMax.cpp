...
static Point3 basic_tva[3] = {Point3(0.0, 0.0, 0.0), Point3(1.0, 0.0, 0.0),
                              Point3(1.0, 1.0, 0.0)};
static Point3 basic_tvb[3] = {Point3(1.0, 1.0, 0.0), Point3(0.0, 1.0, 0.0),
                              Point3(0.0, 0.0, 0.0)};
...
// Retrieves a normal if the smoothing groups overlap or there is// only one in
// the list
...
bool GetOption(std::map<std::string, bool> &mOptions,
...
void IntermediatePolyMesh3DSMax::GetIndexedNormalsFromSpecifiedNormals(
    MNMesh *polyMesh, Imath::M44f &transform44f_I_T,
    IndexedNormals &indexedNormals)
{
  EC_ASSERT(polyMesh != NULL);
  MNNormalSpec *normalSpec = polyMesh->GetSpecifiedNormals();
  EC_ASSERT(normalSpec && normalSpec->GetNumNormals() > 0 &&
            normalSpec->GetNumFaces() > 0);
...
void IntermediatePolyMesh3DSMax::GetIndexedNormalsFromSpecifiedNormals(
    Mesh *triMesh, Imath::M44f &transform44f_I_T,
    IndexedNormals &indexedNormals)
{
...
void IntermediatePolyMesh3DSMax::GetIndexedNormalsFromSmoothingGroups(
    MNMesh *polyMesh, Imath::M44f &transform44f_I_T,
    std::vector<Abc::int32_t> &faceIndices, IndexedNormals &indexedNormals)
{
  EC_ASSERT(polyMesh != NULL);
  MNNormalSpec *normalSpec = polyMesh->GetSpecifiedNormals();
  ...
  createIndexedArray<Abc::N3f, SortableV3f>(faceIndices, expandedNormals,
                                            indexedNormals.values,
                                            indexedNormals.indices);
}
...
void IntermediatePolyMesh3DSMax::GetIndexedUVsFromChannel(
    MNMesh *polyMesh, int chanNum, IndexedUVs &indexedUVs)
{
  EC_ASSERT(polyMesh != NULL);
  ...
  if (map->FNum() != polyMesh->FNum()) {
    ESS_LOG_INFO("Warning: Can't export PolyMesh UV Channel #"
                 << chanNum << " as its map face count (" << map->FNum()
                 << ") doesn't match face count of mesh (" << polyMesh->FNum()
                 << ")");
    return;
  }
...
void IntermediatePolyMesh3DSMax::GetIndexedUVsFromChannel(
    Mesh *triMesh, int chanNum, IndexedUVs &indexedUVs)
{
  EC_ASSERT(triMesh != NULL);
  ...
  if (map.getNumFaces() != triMesh->getNumFaces()) {
    ESS_LOG_INFO("Warning: Can't export TriMesh UV Channel #"
                 << chanNum << " as its map face count (" << map.getNumFaces()
                 << ") doesn't match face count of mesh ("
                 << triMesh->getNumFaces() << ")");
    return;
  }
...
void IntermediatePolyMesh3DSMax::Save(SceneNodePtr eNode,
                                      const Imath::M44f &transform44f,
                                      const CommonOptions &options, double time)
{
  ...
  LONG vertCount =
      (polyMesh != NULL) ? polyMesh->VNum() : triMesh->getNumVerts();
  ...
  for (LONG i = 0; i < vertCount; i++) {
    Point3 &maxPoint =
        (polyMesh != NULL) ? polyMesh->V(i)->p : triMesh->getVert(i);
  ...
  LONG faceCount =
      (polyMesh != NULL) ? polyMesh->FNum() : triMesh->getNumFaces();
  ...
  for (LONG f = 0; f < faceCount; f++) {
    ...
    for (int i = degree - 1; i >= 0; i -= 1) {
      int vertIndex =
          (polyMesh != NULL) ? polyMesh->F(f)->vtx[i] : triMesh->faces[f].v[i];
  ...
  if (options.GetBoolOption("exportNormals")) {
    if (polyMesh != NULL) {
      MNNormalSpec *normalSpec = polyMesh->GetSpecifiedNormals();
      if (normalSpec && normalSpec->GetNumNormals() > 0 &&
          normalSpec->GetNumFaces() > 0) {
        GetIndexedNormalsFromSpecifiedNormals(polyMesh, transform44f_I_T,
                                              mIndexedNormals);
      }
      else {
        polyMesh->buildNormals();
        GetIndexedNormalsFromSmoothingGroups(polyMesh, transform44f_I_T,
                                             mFaceIndicesVec, mIndexedNormals);
      }
    }
    if (triMesh != NULL) {
      MeshNormalSpec *normalSpec = triMesh->GetSpecifiedNormals();
      if (normalSpec && normalSpec->GetNumNormals() > 0 &&
          normalSpec->GetNumFaces() > 0) {
        GetIndexedNormalsFromSpecifiedNormals(triMesh, transform44f_I_T,
                                              mIndexedNormals);
      }
      else {
        triMesh->buildNormals();
        GetIndexedNormalsFromSmoothingGroups(triMesh, transform44f_I_T,
                                             mFaceIndicesVec, mIndexedNormals);
      }
    }
  }
  ...
  if (options.GetBoolOption("exportUVs")) {
    if (polyMesh != NULL) {
      ...
      for (int mp = 1; mp < numMaps; mp++) {
        ...
        if (map->numv <= 0 ||
            map->numf <= 0) {  // probably checking the dead flag is enough,
    ...
    for (int i = 0; i < numFaces; i += 1) {
      ...
      if (bFirstFrame || pMatMerge) {
        ...
        if (it == mFaceSets.end()) {
          ...
          facesetmap_ret_pair ret =
              mFaceSets.insert(facesetmap_insert_pair(matId, faceSet));
          it = ret.first;
          numMatId++;
        }
  ...
  mesh.GetICEAttributeFromName(L"PointVelocity");
     if(velocitiesAttr.IsDefined() && velocitiesAttr.IsValid())
     {
        ...
        if(!mVelocityProperty.valid())
           mVelocityProperty = OV3fArrayProperty(mMeshSchema, ".velocities",
  mMeshSchema.getMetaData(), GetJob()->GetAnimatedTs());
...
bool IntermediatePolyMesh3DSMax::mergeWith(
    const CommonIntermediatePolyMesh &srcMesh)
{
  ESS_PROFILE_FUNC();

  IntermediatePolyMesh3DSMax &destMesh = *this;
  const IntermediatePolyMesh3DSMax &srcMeshMax =
      (IntermediatePolyMesh3DSMax &)srcMesh;
  ...
  for (FaceSetMap::const_iterator it = srcMeshMax.mFaceSets.begin();
    ...
    if (destMesh.mFaceSets.find(it->first) ==
        destMesh.mFaceSets.end()) {  // a new key
