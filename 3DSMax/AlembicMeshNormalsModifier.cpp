...
{
  return &AlembicMeshNormalsModifierDesc;
}

//--- Properties block -------------------------------

static ParamBlockDesc2 AlembicMeshNormalsModifierParams(
    0, _T(ALEMBIC_MESH_NORMALS_MODIFIER_SCRIPTNAME), 0,
    GetAlembicMeshNormalsModifierClassDesc(), P_AUTO_CONSTRUCT | P_AUTO_UI, 0,

    // rollout description
    IDD_ALEMBIC_MESH_NORMALS_PARAMS, IDS_ALEMBIC, 0, 0, NULL,

    // params
    AlembicMeshNormalsModifier::ID_PATH, _T("path"), TYPE_FILENAME,
    P_RESET_DEFAULT, IDS_PATH, p_default, "", p_ui, TYPE_EDITBOX, IDC_PATH_EDIT,
    p_assetTypeID, kExternalLink, p_end,

    AlembicMeshNormalsModifier::ID_IDENTIFIER, _T("identifier"), TYPE_STRING,
    P_RESET_DEFAULT, IDS_IDENTIFIER, p_default, "", p_ui, TYPE_EDITBOX,
    IDC_IDENTIFIER_EDIT, p_end,

    AlembicMeshNormalsModifier::ID_TIME, _T("time"), TYPE_FLOAT, P_ANIMATABLE,
    IDS_TIME, p_default, 0.0f, p_range, 0.0f, 1000.0f, p_ui, TYPE_SPINNER,
    EDITTYPE_FLOAT, IDC_TIME_EDIT, IDC_TIME_SPIN, 0.01f, p_end,

    /*AlembicMeshNormalsModifier::ID_GEOALPHA, _T("geoAlpha"), TYPE_FLOAT,
       P_ANIMATABLE, IDS_GEOALPHA,
            p_default,       1.0f,
            p_range,         0.0f, 1.0f,
...
void AlembicMeshNormalsModifier::ModifyObject(TimeValue t, ModContext &mc,
                                              ObjectState *os, INode *node)
{
  ...
  this->pblock->GetValue(AlembicMeshNormalsModifier::ID_PATH, t, strPath,
                         interval);

  MCHAR const *strIdentifier = NULL;
  this->pblock->GetValue(AlembicMeshNormalsModifier::ID_IDENTIFIER, t,
                         strIdentifier, interval);

  float fTime;
  this->pblock->GetValue(AlembicMeshNormalsModifier::ID_TIME, t, fTime,
                         interval);
  ...
  // this->pblock->GetValue( AlembicMeshNormalsModifier::ID_GEOALPHA, t,
  // fGeoAlpha, interval);
  ...
  BOOL bMuted;
  this->pblock->GetValue(AlembicMeshNormalsModifier::ID_MUTED, t, bMuted,
                         interval);
...
RefResult AlembicMeshNormalsModifier::NotifyRefChanged(
    const Interval &changeInt, RefTargetHandle hTarget, PartID &partID,
    RefMessage message, BOOL propagate)
{
#else
RefResult AlembicMeshNormalsModifier::NotifyRefChanged(Interval changeInt,
      ...
      if (hTarget == pblock) {
        ...
        switch (changing_param) {
          case ID_PATH: {
            ...
            Interval v;
            pblock->GetValue(AlembicMeshNormalsModifier::ID_PATH, t, strPath,
                             v);
...
void AlembicMeshNormalsModifier::BeginEditParams(IObjParam *ip, ULONG flags,
                                                 Animatable *prev)
{
  this->ip = ip;
  editMod = this;
...
void AlembicMeshNormalsModifier::EndEditParams(IObjParam *ip, ULONG flags,
                                               Animatable *next)
{
