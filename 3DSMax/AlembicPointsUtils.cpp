...
splitTypeT setPerParticleMeshRenderSetting(Object* obj, TimeValue ticks,
                                           splitTypeT nNewValue)
{
...
bool particleGroupInterface::getCurrentParticleMeshTM(Matrix3& meshTM)
{
  ...
  m_pCurrRender->GetMultipleRenderMeshTM(
      m_pCurrGroupContainer, m_currTicks, m_pSystemObject, m_pCurrGroupSystem,
      *m_pNullView, m_nCurrParticleGroupId, meshTM, meshValid);
...
Mesh* particleGroupInterface::getCurrentParticleMesh(BOOL& bNeedDelete)
{
  ...
  Mesh* pMesh = m_pCurrRender->GetMultipleRenderMesh(
      m_pCurrGroupContainer, m_currTicks, m_pSystemObject, m_pCurrGroupSystem,
      *m_pNullView, bNeedDelete, m_nCurrParticleGroupId);
...
int particleGroupInterface::getCurrentMtlId()
{
  IParticleChannelIntR* chMtlIndex =
      GetParticleChannelMtlIndexRInterface(m_pCurrGroupContainer);
  if (chMtlIndex) {
...
bool getParticleSystemMesh(TimeValue ticks, INode* node,
                           IntermediatePolyMesh3DSMax* mesh,
                           materialsMergeStr* pMatMerge, AlembicWriteJob* mJob,
                           int nNumSamples, bool bEnableVelocityExport)
{
  ...
  ParticleObject* pParticleObject =
      (ParticleObject*)obj->GetInterface(I_PARTICLEOBJ);
  ...
  if (pParticleObject && !iSystem) {
    ...
    // TODO: try using particlesExt->GetRenderMeshVertexSpeed with pFlow instead
    // of calculating the velocities manually
    ...
    //	mdata.pMesh = pAlembicParticles->GetMultipleRenderMesh(ticks, node
    ///*system node or particle node?*/ , nullView, mdata.bNeedDelete, i);
    ...
    Mesh* pMesh =
        pParticleObject->GetRenderMesh(ticks, node, nullView, bNeedDelete);
    ...
    if (bEnableVelocityExport) {  // TODO...
      ...
      IParticleObjectExt* particlesExt =
          GetParticleObjectExtInterface(pParticleObject);
      if (particlesExt) {
        Tab<Point3> perVertexVelocities;
        // particlesExt->UpdateParticles(node, ticks);
        particlesExt->GetRenderMeshVertexSpeed(ticks, node, nullView,
                                               perVertexVelocities);

        if (perVertexVelocities.Count() != pMesh->getNumVerts()) {
          ESS_LOG_WARNING(
              "Warning: Mesh Vertices count and velocities count do not "
              "match.");
        }
        ...
        if (perVertexVelocities.Count() == pMesh->getNumVerts()) {
          ...
          for (int j = 0; j < perVertexVelocities.Count(); j++) {
            Abc::V4f pVelocity4 = ConvertMaxVectorToAlembicVector4(
                perVertexVelocities[j] * TIME_TICKSPERSEC);
            pVelocity4 = pVelocity4 * nodeWorldTransInv;
            mesh->mVelocitiesVec.push_back(
                Abc::V3f(pVelocity4.x, pVelocity4.y, pVelocity4.z));
          }
        }
      }
      else {
    ...
    SceneNodeMaxParticlesPtr inputSceneNode(
        new SceneNodeMaxParticles(pMesh, pMtl));
  ...
  splitTypeT oldSplitType =
      setPerParticleMeshRenderSetting(obj, ticks, kRender_splitType_particle);
  ...
  for (int i = 0; i < particlesExt->NumParticles(); i++) {
    ...
    if (bEnableVelocityExport) {
      ...
      Abc::V4f pPosition4 = ConvertMaxPointToAlembicPoint4(
      ...
      // Abc::V3f pAngularVelocity =
      // ConvertMaxNormalToAlembicNormal(angAxis.axis);
      ...
      for (int j = 0; j < pMesh->getNumVerts(); j++) {
        Abc::V3f meshVertex = ConvertMaxPointToAlembicPoint(
            pMesh->getVert(j) *
            meshTM);  // the mesh vertex in particle system space
      ...
      if (perVertexVelocities.Count() != pMesh->getNumVerts()) {
        ESS_LOG_WARNING(
            "Warning: Mesh Vertices count and velocities count do not match.");
      }
    ...
    // IParticleChannelIntR* chMtlIndex =
    // GetParticleChannelMtlIndexRInterface(pCont);
    // int nMatId = -1;
    // if(chMtlIndex){
    //	chMtlIndex->GetValue(meshId);
    //}

    pMatMerge->currUniqueHandle = Animatable::GetHandleByAnim(
        groupInterface.m_pCurrParticleGroup->GetActionList());
    pMatMerge->bPreserveIds = true;

    Imath::M44f transform44f;
    ConvertMaxMatrixToAlembicMatrix(meshTM, transform44f);
    SceneNodeMaxParticlesPtr inputSceneNode(
        new SceneNodeMaxParticles(pMesh, groupInterface.m_pCurrGroupMtl));
...
void getParticleGroups(TimeValue ticks, Object* obj, INode* node,
...
void getParticleSystemRenderMeshes(TimeValue ticks, Object* obj, INode* node,
                                   std::vector<particleMeshData>& meshes)
{
  ...
  if (pSimpleParticle) {
    ...
    AlembicParticles* pAlembicParticles = NULL;
    if (obj->CanConvertToType(ALEMBIC_SIMPLE_PARTICLE_CLASSID)) {
      pAlembicParticles = reinterpret_cast<AlembicParticles*>(
          obj->ConvertToType(ticks, ALEMBIC_SIMPLE_PARTICLE_CLASSID));
    }
    ...
    if (pAlembicParticles) {
      ...
      for (int i = 0; i < numParticles; i++) {
        ...
        pAlembicParticles->GetMultipleRenderMeshTM_Internal(
            ticks, NULL, nullView, i, mdata.meshTM, interval);

        // Matrix3 objectToWorld = inode->GetObjectTM( ticks );
        // mdata.meshTM = mdata.meshTM * Inverse(objectToWorld);

        mdata.pMesh = pAlembicParticles->GetMultipleRenderMesh(
            ticks, node /*system node or particle node?*/, nullView,
            mdata.bNeedDelete, i);
    ...
    if (particleRender) {
      ...
      for (int g = 0; g < groups.size(); g++) {
        ...
        mdata.animHandle =
            Animatable::GetHandleByAnim(groups[g]->GetActionList());

        mdata.meshTM.IdentityMatrix();

        mdata.pMesh = particleRender->GetRenderMesh(
            pCont, ticks, obj, pNode, nullView, mdata.bNeedDelete);
...
const Mesh* GetShapeMesh(IParticleObjectExt* pExt, int particleId,
                         TimeValue ticks)
{
  ...
  IParticleChannelMeshR* pShapeChannel =
      GetParticleChannelShapeRInterface(chCont);
  if (!pShapeChannel) {
    return NULL;
  }

  // IParticleChannelINodeR* pNodeChannel =
  // GetParticleChannelRefNodeRInterface(chCont);
  // if(!pNodeChannel){
  //	return;
  //}
  ...
  // bool bIsShared = pShapeChannel->IsShared();//if true, particle meshes are
  // not unique, and we should avoid writing out copies
