...
// available to use
static AlembicVisibilityControllerClassDesc
    sAlembicVisibilityControllerClassDesc;
...
static ParamBlockDesc2 AlembicVisibilityControllerParams(
    0, _T(ALEMBIC_VISIBILITY_CONTROLLER_SCRIPTNAME), 0,
    GetAlembicVisibilityControllerClassDesc(), P_AUTO_CONSTRUCT | P_AUTO_UI, 0,

    // rollout description
    IDD_ALEMBIC_VISIBILITY_PARAMS, IDS_ALEMBIC, 0, 0, NULL,

    // params
    AlembicVisibilityController::ID_PATH, _T("path"), TYPE_FILENAME,
    P_RESET_DEFAULT, IDS_PATH, p_default, "", p_ui, TYPE_EDITBOX, IDC_PATH_EDIT,
    p_assetTypeID, AssetManagement::kExternalLink, p_end,

    AlembicVisibilityController::ID_IDENTIFIER, _T("identifier"), TYPE_STRING,
    P_RESET_DEFAULT, IDS_IDENTIFIER, p_default, "", p_ui, TYPE_EDITBOX,
    IDC_IDENTIFIER_EDIT, p_end,

    AlembicVisibilityController::ID_TIME, _T("time"), TYPE_FLOAT, P_ANIMATABLE,
    IDS_TIME, p_default, 0.0f, p_range, 0.0f, 1000.0f, p_ui, TYPE_SPINNER,
    EDITTYPE_FLOAT, IDC_TIME_EDIT, IDC_TIME_SPIN, 0.01f, p_end,

    AlembicVisibilityController::ID_MUTED, _T("muted"), TYPE_BOOL, P_ANIMATABLE,
...
void AlembicVisibilityController::SetValueLocalTime(TimeValue t, void *ptr,
                                                    int commit,
                                                    GetSetMethod method)
{
}

void *AlembicVisibilityController::CreateTempValue() { return new float; }
...
void AlembicVisibilityController::Extrapolate(Interval range, TimeValue t,
                                              void *val, Interval &valid,
                                              int type)
{
}
...
RefResult AlembicVisibilityController::NotifyRefChanged(const Interval &iv,
                                                        RefTargetHandle hTarg,
                                                        PartID &partID,
                                                        RefMessage msg,
                                                        BOOL propagate)
#else
RefResult AlembicVisibilityController::NotifyRefChanged(Interval iv,
                                                        RefTargetHandle hTarg,
                                                        PartID &partID,
                                                        RefMessage msg)
#endif
      ...
      if (hTarg == pblock) {
        ...
        switch (changing_param) {
          case ID_PATH: {
            ...
            Interval v;
            pblock->GetValue(AlembicVisibilityController::ID_PATH, t, strPath,
                             v);
...
void AlembicVisibilityController::EndEditParams(IObjParam *ip, ULONG flags,
                                                Animatable *next)
{
...
void AlembicImport_FillInVis_Internal(alembic_fillvis_options &options)
{
  ...
  AbcG::IVisibilityProperty visibilityProperty =
      getAbcVisibilityProperty(*options.pIObj);
  ...
  SampleInfo sampleInfo =
      getSampleInfo(sampleTime, getTimeSamplingFromObject(*options.pIObj),
                    visibilityProperty.getNumSamples());

  boost::int8_t rawVisibilityValue =
