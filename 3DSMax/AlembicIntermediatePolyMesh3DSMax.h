...
typedef std::map<AnimHandle, meshMaterialsMap>::iterator
    mergedMeshMaterialsMap_it;
typedef std::map<AnimHandle, meshMaterialsMap>::const_iterator
    mergedMaterialsMap_cit;
...
struct materialsMergeStr {
  ...
  materialsMergeStr()
      : currUniqueHandle(NULL), nNextMatId(0), bPreserveIds(false)
  {
  }
...
class MeshSmoothingGroupNormals {
 ...
 public:
  ...
  {
    if (mpMesh != NULL) {
      // If we do not a smoothing group, we can't base ourselves on anything
      // else,
      ...
      // If we did not find any normals or the normals offset each other for
      // some
...
class IntermediatePolyMesh3DSMax : public CommonIntermediatePolyMesh {
 private:
  void GetIndexedNormalsFromSpecifiedNormals(MNMesh *polyMesh,
                                             Imath::M44f &transform44f,
                                             IndexedNormals &indexedNormals);
  void GetIndexedNormalsFromSpecifiedNormals(Mesh *triMesh,
                                             Imath::M44f &transform44f,
                                             IndexedNormals &indexedNormals);

  void GetIndexedNormalsFromSmoothingGroups(
      MNMesh *polyMesh, Imath::M44f &transform44f,
      std::vector<Abc::int32_t> &faceIndices, IndexedNormals &indexedNormals);
  void GetIndexedNormalsFromSmoothingGroups(
      Mesh *triMesh, Imath::M44f &transform44f,
      std::vector<Abc::int32_t> &faceIndices, IndexedNormals &indexedNormals);

  void GetIndexedUVsFromChannel(MNMesh *polyMesh, int chanNum,
                                IndexedUVs &indexedUVs);
  void GetIndexedUVsFromChannel(Mesh *triMesh, int chanNum,
                                IndexedUVs &indexedUVs);
