...
{
  return &AlembicMeshGeomModifierDesc;
}
...
static ParamBlockDesc2 AlembicMeshGeomModifierParams(
    0, _T(ALEMBIC_MESH_GEOM_MODIFIER_SCRIPTNAME), 0,
    GetAlembicMeshGeomModifierClassDesc(),
    P_AUTO_CONSTRUCT | P_AUTO_UI | P_VERSION,
    ALEMBIC_MESH_GEOMETRY_MODIFIER_VERSION, 0,

    // rollout description
    IDD_ALEMBIC_MESH_GEOM_PARAMS, IDS_ALEMBIC, 0, 0, NULL,

    // params
    AlembicMeshGeomModifier::ID_PATH, _T("path"), TYPE_FILENAME,
    P_RESET_DEFAULT, IDS_PATH, p_default, "", p_ui, TYPE_EDITBOX, IDC_PATH_EDIT,
    p_assetTypeID, kExternalLink, p_end,

    AlembicMeshGeomModifier::ID_IDENTIFIER, _T("identifier"), TYPE_STRING,
    P_RESET_DEFAULT, IDS_IDENTIFIER, p_default, "", p_ui, TYPE_EDITBOX,
    IDC_IDENTIFIER_EDIT, p_end,

    AlembicMeshGeomModifier::ID_TIME, _T("time"), TYPE_FLOAT, P_ANIMATABLE,
    IDS_TIME, p_default, 0.0f, p_range, 0.0f, 1000.0f, p_ui, TYPE_SPINNER,
    EDITTYPE_FLOAT, IDC_TIME_EDIT, IDC_TIME_SPIN, 0.01f, p_end,

    AlembicMeshGeomModifier::ID_GEOALPHA, _T("geoAlpha"), TYPE_FLOAT,
    P_ANIMATABLE, IDS_GEOALPHA, p_default, 1.0f, p_range, 0.0f, 1.0f, p_ui,
    TYPE_SPINNER, EDITTYPE_FLOAT, IDC_GEOALPHA_EDIT, IDC_GEOALPHA_SPIN, 0.1f,
    p_end,

    AlembicMeshGeomModifier::ID_MUTED, _T("muted"), TYPE_BOOL, P_ANIMATABLE,
    IDS_MUTED, p_default, TRUE, p_ui, TYPE_SINGLECHEKBOX, IDC_MUTED_CHECKBOX,
    p_end,

    AlembicMeshGeomModifier::ID_ADDITIVE, _T("additive"), TYPE_BOOL,
    P_ANIMATABLE, IDS_ADDITIVE, p_default, FALSE, p_ui, TYPE_SINGLECHEKBOX,
    IDC_ADDITIVE_CHECKBOX, p_end,
...
RefResult AlembicMeshGeomModifier::NotifyRefChanged(const Interval &changeInt,
...
{
#else
RefResult AlembicMeshGeomModifier::NotifyRefChanged(Interval changeInt,
                                                    RefTargetHandle hTarget,
                                                    PartID &partID,
                                                    RefMessage message)
{
      ...
      if (hTarget == pblock) {
        ...
        switch (changing_param) {
          case ID_PATH: {
            ...
            Interval v;
            pblock->GetValue(AlembicMeshGeomModifier::ID_PATH, t, strPath, v);
...
void AlembicMeshGeomModifier::BeginEditParams(IObjParam *ip, ULONG flags,
                                              Animatable *prev)
{
  this->ip = ip;
  editMod = this;
...
void AlembicMeshGeomModifier::EndEditParams(IObjParam *ip, ULONG flags,
                                            Animatable *next)
{
