...
{
  return &AlembicSplineTopoModifierDesc;
}
...
static ParamBlockDesc2 AlembicSplineTopoModifierParams(
    0, _T(ALEMBIC_SPLINE_TOPO_MODIFIER_SCRIPTNAME), 0,
    GetAlembicSplineTopoModifierClassDesc(),
    P_AUTO_CONSTRUCT | P_AUTO_UI | P_VERSION,
    ALEMBIC_SPLINE_TOPO_MODIFIER_VERSION, 0,

    // rollout description
    IDD_ALEMBIC_SPLINE_TOPO_PARAMS, IDS_ALEMBIC, 0, 0, NULL,

    // params
    AlembicSplineTopoModifier::ID_PATH, _T("path"), TYPE_FILENAME,
    P_RESET_DEFAULT, IDS_PATH, p_default, "", p_ui, TYPE_EDITBOX, IDC_PATH_EDIT,
    p_assetTypeID, kExternalLink, p_end,

    AlembicSplineTopoModifier::ID_IDENTIFIER, _T("identifier"), TYPE_STRING,
    P_RESET_DEFAULT, IDS_IDENTIFIER, p_default, "", p_ui, TYPE_EDITBOX,
    IDC_IDENTIFIER_EDIT, p_end,

    AlembicSplineTopoModifier::ID_TIME, _T("time"), TYPE_FLOAT, P_ANIMATABLE,
    IDS_TIME, p_default, 0.0f, p_range, 0.0f, 1000.0f, p_ui, TYPE_SPINNER,
    EDITTYPE_FLOAT, IDC_TIME_EDIT, IDC_TIME_SPIN, 0.01f, p_end,

    AlembicSplineTopoModifier::ID_GEOMETRY, _T("geometry"), TYPE_BOOL,
    P_ANIMATABLE, IDS_GEOMETRY, p_default, FALSE, p_end,

    /*AlembicSplineTopoModifier::ID_NORMALS, _T("normals"), TYPE_BOOL,
    P_ANIMATABLE, IDS_NORMALS,
            p_default,       FALSE,
            p_end,

    AlembicSplineTopoModifier::ID_UVS, _T("uvs"), TYPE_BOOL, P_ANIMATABLE,
    IDS_UVS,
            p_default,       FALSE,
            p_end,*/
...
void AlembicSplineTopoModifier::ModifyObject(TimeValue t, ModContext &mc,
                                             ObjectState *os, INode *node)
{
  ...
  this->pblock->GetValue(AlembicSplineTopoModifier::ID_PATH, t, strPath,
                         interval);

  MCHAR const *strIdentifier = NULL;
  this->pblock->GetValue(AlembicSplineTopoModifier::ID_IDENTIFIER, t,
                         strIdentifier, interval);

  float fTime;
  this->pblock->GetValue(AlembicSplineTopoModifier::ID_TIME, t, fTime,
                         interval);
  ...
  BOOL bGeometry;
  this->pblock->GetValue(AlembicSplineTopoModifier::ID_GEOMETRY, t, bGeometry,
                         interval);

  /*BOOL bNormals;
  this->pblock->GetValue( AlembicSplineTopoModifier::ID_NORMALS, t, bNormals,
  interval);

  BOOL bUVs;
  this->pblock->GetValue( AlembicSplineTopoModifier::ID_UVS, t, bUVs,
  interval);*/

  BOOL bMuted;
  this->pblock->GetValue(AlembicSplineTopoModifier::ID_MUTED, t, bMuted,
                         interval);

  BOOL bIgnoreSubframeSamples;
  this->pblock->GetValue(AlembicSplineTopoModifier::ID_IGNORE_SUBFRAME_SAMPLES,
                         t, bIgnoreSubframeSamples, interval);
...
RefResult AlembicSplineTopoModifier::NotifyRefChanged(const Interval &changeInt,
...
{
#else
RefResult AlembicSplineTopoModifier::NotifyRefChanged(Interval changeInt,
                                                      RefTargetHandle hTarget,
                                                      PartID &partID,
                                                      RefMessage message)
{
      ...
      if (hTarget == pblock) {
        ...
        switch (changing_param) {
          case ID_PATH: {
            ...
            Interval v;
            pblock->GetValue(AlembicSplineTopoModifier::ID_PATH, t, strPath, v);
...
void AlembicSplineTopoModifier::BeginEditParams(IObjParam *ip, ULONG flags,
                                                Animatable *prev)
{
  this->ip = ip;
  editMod = this;
...
void AlembicSplineTopoModifier::EndEditParams(IObjParam *ip, ULONG flags,
                                              Animatable *next)
{
