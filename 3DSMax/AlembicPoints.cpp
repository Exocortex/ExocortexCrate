...
#define PARTICLECHANNELLOCALOFFSETR_INTERFACE \
  Interface_ID(0x12ec5d1d, 0x1eb34500)
#define GetParticleChannelLocalOffsetRInterface(obj) \
  ((IParticleChannelIntR *)obj->GetInterface(        \
      PARTICLECHANNELLOCALOFFSETR_INTERFACE))
    ...
    : AlembicObject(eNode, in_Job, oParent)
{
  ...
  AbcG::OPoints points(GetOParent(), pointsName.c_str(),
  ...
  mInstanceNamesProperty = Abc::OStringArrayProperty(
      argGeomParams, ".instancenames", mPointsSchema.getMetaData(),
      GetCurrentJob()->GetAnimatedTs());

  // particle attributes
  mScaleProperty = Abc::OV3fArrayProperty(argGeomParams, ".scale",
                                          mPointsSchema.getMetaData(),
                                          GetCurrentJob()->GetAnimatedTs());
  mOrientationProperty = Abc::OQuatfArrayProperty(
      argGeomParams, ".orientation", mPointsSchema.getMetaData(),
      GetCurrentJob()->GetAnimatedTs());
  mAngularVelocityProperty = Abc::OQuatfArrayProperty(
      argGeomParams, ".angularvelocity", mPointsSchema.getMetaData(),
      GetCurrentJob()->GetAnimatedTs());
  mAgeProperty = Abc::OFloatArrayProperty(argGeomParams, ".age",
                                          mPointsSchema.getMetaData(),
                                          GetCurrentJob()->GetAnimatedTs());
  mMassProperty = Abc::OFloatArrayProperty(argGeomParams, ".mass",
                                           mPointsSchema.getMetaData(),
                                           GetCurrentJob()->GetAnimatedTs());
  mShapeTypeProperty = Abc::OUInt16ArrayProperty(
      argGeomParams, ".shapetype", mPointsSchema.getMetaData(),
      GetCurrentJob()->GetAnimatedTs());
  mShapeTimeProperty = Abc::OFloatArrayProperty(
      argGeomParams, ".shapetime", mPointsSchema.getMetaData(),
      GetCurrentJob()->GetAnimatedTs());
  mShapeInstanceIDProperty = Abc::OUInt16ArrayProperty(
      argGeomParams, ".shapeinstanceid", mPointsSchema.getMetaData(),
      GetCurrentJob()->GetAnimatedTs());
  mColorProperty = Abc::OC4fArrayProperty(argGeomParams, ".color",
                                          mPointsSchema.getMetaData(),
                                          GetCurrentJob()->GetAnimatedTs());
}

AlembicPoints::~AlembicPoints() {}
Abc::OCompoundProperty AlembicPoints::GetCompound() { return mPointsSchema; }
bool AlembicPoints::Save(double time, bool bLastFrame)
{
  ...
  SimpleParticle *pSimpleParticle =
      (SimpleParticle *)obj->GetInterface(I_SIMPLEPARTICLEOBJ);
  ...
  if (obj->CanConvertToType(MATTERWAVES_CLASS_ID)) {
    pThinkingParticleMat = reinterpret_cast<ParticleMat *>(
        obj->ConvertToType(ticks, MATTERWAVES_CLASS_ID));
    pTPMasterSystemInt = reinterpret_cast<TP_MasterSystemInterface *>(
        obj->GetInterface(IID_TP_MASTERSYSTEM));
  }
#endif

  const bool bAutomaticInstancing =
      GetCurrentJob()->GetOption("automaticInstancing");
  ...
  // Note: settting the render state to true breaks the shape node instancing
  // export
  ...
  Abc::M44d nodeWorldTrans(
      alembicMatrix.GetRow(0).x, alembicMatrix.GetRow(0).y,
...

  {
    ...
    for (int i = 0; i < numParticles; ++i) {
      ...
      // Particle size is a uniform scale multiplier in XSI.  In Max, I need to
      // learn where to get this
      ...
      if (pThinkingParticleMat) {
        ...
        ESS_PROFILE_SCOPE(
            "AlembicPoints::SAVE - numParticlesLoop - ThinkingParticles");
        age = (float)GetSecondsFromTimeValue(ageValue);

        // pos =
        // ConvertMaxPointToAlembicPoint(*particlesExt->GetParticlePositionByIndex(i));
        pos = ConvertMaxPointToAlembicPoint(pTPMasterSystemInt->Position(i));
        // vel =
        // ConvertMaxVectorToAlembicVector(*particlesExt->GetParticleSpeedByIndex(i)
        // * TIME_TICKSPERSEC);
        vel = ConvertMaxVectorToAlembicVector(pTPMasterSystemInt->Velocity(i) *
                                              TIME_TICKSPERSEC);
        scale = ConvertMaxScaleToAlembicScale(pTPMasterSystemInt->Scale(i));
        scale *= pTPMasterSystemInt->Size(i);

        // ConvertMaxEulerXYZToAlembicQuat(*particlesExt->GetParticleOrientationByIndex(i),
        // orientation);
        ...
        /*alignmentMat = Abc::M44d( alignmentMatMax.GetRow(0).x,
           alignmentMatMax.GetRow(0).y,  alignmentMatMax.GetRow(0).z,  0,
                 alignmentMatMax.GetRow(1).x,  alignmentMatMax.GetRow(1).y,
           alignmentMatMax.GetRow(1).z,  0,
                 alignmentMatMax.GetRow(2).x,  alignmentMatMax.GetRow(2).y,
           alignmentMatMax.GetRow(2).z,  0,
                 alignmentMatMax.GetRow(3).x,  alignmentMatMax.GetRow(3).y,
           alignmentMatMax.GetRow(3).z,  1);*/
        // orientation = ConvertMaxQuatToAlembicQuat(extracctuat(alignmentMat),
        // true);

        alignmentMat = alignmentMat * nodeWorldTransInv;
        orientation = extractQuat(alignmentMat);

        // ConvertMaxAngAxisToAlembicQuat(*particlesExt->GetParticleSpinByIndex(i),
        // spin);
        ...
        Mesh *pMesh = NULL;
        {
          ESS_PROFILE_SCOPE(
              "AlembicPoints::SAVE - numParticlesLoop - ThinkingParticles - "
              "GetParticleRenderMesh");
          pMesh = pThinkingParticleMat->GetParticleRenderMesh(
              ticks, mMaxNode, nullView, bNeedDelete, i, meshTM, bChanged);
        }
        ...
        if (pMesh) {
          ESS_PROFILE_SCOPE(
              ...
              "CacheShapeMesh");
          meshInfo mi =
              CacheShapeMesh(pMesh, bNeedDelete, meshTM, nMatId, i, ticks,
                             shapetype, shapeInstanceId, shapeInstanceTime);
        ...
        pos = ConvertMaxPointToAlembicPoint(
            *particlesExt->GetParticlePositionByIndex(i));
        vel = ConvertMaxVectorToAlembicVector(
            *particlesExt->GetParticleSpeedByIndex(i) * TIME_TICKSPERSEC);
        scale = ConvertMaxScaleToAlembicScale(
            *particlesExt->GetParticleScaleXYZByIndex(i));
        ConvertMaxEulerXYZToAlembicQuat(
            *particlesExt->GetParticleOrientationByIndex(i), orientation);
        ConvertMaxAngAxisToAlembicQuat(*particlesExt->GetParticleSpinByIndex(i),
                                       spin);
        // age =
        // (float)GetSecondsFromTimeValue(particlesExt->GetParticleAgeByIndex(i));
        id = particlesExt->GetParticleBornIndex(i);
        if (bAutomaticInstancing) {
          int nMatId = -1;
          if (ipfSystem) {
            if (groupInterface.setCurrentParticle(ticks, i)) {
              ...
              ESS_LOG_WARNING(
                  "Error: cound retrieve material ID for particle mesh " << i);
            }
          }
          ...
          GetShapeType(particlesExt, i, ticks, shapetype, shapeInstanceId,
                       shapeInstanceTime);
        }
        ...
        pos = ConvertMaxPointToAlembicPoint(
            pSimpleParticle->ParticlePosition(ticks, i));
        vel = ConvertMaxVectorToAlembicVector(
            pSimpleParticle->ParticleVelocity(ticks, i));
        // simple particles have no scale?
        // simple particles have no orientation?
        age = (float)GetSecondsFromTimeValue(
            pSimpleParticle->ParticleAge(ticks, i));
      ...
      {
        ESS_PROFILE_SCOPE(
            "AlembicPoints::SAVE - numParticlesLoop - end loop save");
        ...
        // orientation = Abc::extractQuat(orientation.toMatrix44() *
        // nodeWorldTransInv);
...

  {
    ...
    mPointsSample.setWidths(
        AbcG::OFloatGeomParam::Sample(widthSample, AbcG::kVertexScope));
  ...
  if (bLastFrame) {
    ...
    for (faceVertexHashToShapeMap::iterator it = mShapeMeshCache.begin();
         it != mShapeMeshCache.end(); it++) {
      std::stringstream pathStream;
      pathStream << "/" << it->second.name << "/" << it->second.name << "Shape";
      instanceNames[it->second.nMeshInstanceId] = pathStream.str();
    }

    // for some reason the .dims property is not written when there is exactly
    // one entry if we don't push an empty string
...
// static
void AlembicPoints::ConvertMaxEulerXYZToAlembicQuat(const Point3 &degrees,
                                                    Abc::Quatd &quat)
{
  // Get the angles as a float vector of radians - strangeley they already are
  // even though the documentation says degrees
...
// static
void AlembicPoints::ConvertMaxAngAxisToAlembicQuat(const AngAxis &angAxis,
                                                   Abc::Quatd &quat)
{
...
void AlembicPoints::ReadShapeFromOperator(IParticleGroup *particleGroup,
                                          PFSimpleOperator *pSimpleOperator,
                                          int particleId, TimeValue ticks,
                                          ShapeType &type,
                                          Abc::uint16_t &instanceId,
                                          float &animationTime)
{
  if (!pSimpleOperator) {
    return;
  ...
  if (pSimpleOperator->ClassID() == PFOperatorSimpleShape_Class_ID) {
    ...
    IParamBlock2 *pblock = pSimpleOperator->GetParamBlockByID(0);
    INode *pNode =
        pblock->GetINode(PFlow_kInstanceShape_objectMaxscript, ticks);
    ...
    std::string nodePath =
        getNodeAlembicPath(EC_MCHAR_to_UTF8(pNode->GetName()), bFlatten);
    ...
    if (syncRandom) {
      chLocalOffR = (IParticleChannelIntR *)chCont->GetPrivateInterface(
          PARTICLECHANNELLOCALOFFSETR_INTERFACE, pSimpleOperator);
    }
    ...
    //	mTimeSamplesCount++;
    //	ESS_LOG_WARNING("sampleTime: "<<(float)time<<" totalSamples:
    //"<<mTimeSamplesCount);
    //}
...
void AlembicPoints::GetShapeType(IParticleObjectExt *pExt, int particleId,
                                 TimeValue ticks, ShapeType &type,
                                 Abc::uint16_t &instanceId,
                                 float &animationTime)
{
  ...
  for (int p = particleActionList->NumActions() - 1; p >= 0; p -= 1) {
    ...
    if (pTestAction) {
      INode *childActionListNode =
          pTestAction->GetNextActionList(pActionNode, NULL);

      if (childActionListNode) {
        AlembicPoints::perActionListShapeMap_it actionListIt =
            mPerActionListShapeMap.find(childActionListNode);

        // create a cache entry if necessary
        if (actionListIt == mPerActionListShapeMap.end()) {
          mPerActionListShapeMap[childActionListNode] =
              AlembicPoints::shapeInfo();
        }
  ...
  ReadShapeFromOperator(particleGroup, pSimpleOperator, particleId, ticks, type,
                        instanceId, animationTime);

  if (type != ShapeType_NbElements) {  // write the shape to the cache

    // create cache entry for the current action list node, and then fill in the
    // shape info
    // we will fill in the parent later

    AlembicPoints::perActionListShapeMap_it actionListIt =
        mPerActionListShapeMap.find(particleActionListNode);

    // create a cache entry if necessary
    if (actionListIt == mPerActionListShapeMap.end()) {
      mPerActionListShapeMap[particleActionListNode] =
          AlembicPoints::shapeInfo();
    }
    AlembicPoints::shapeInfo &sInfo =
        mPerActionListShapeMap[particleActionListNode];
    ...
    const int MAX_DEPTH =
        10;  // just in case there is an infinite loop due a bug
    int i = 0;
    while (currActionNode && sInfo.type == ShapeType_NbElements &&
           i < MAX_DEPTH) {
      AlembicPoints::perActionListShapeMap_it actionListIt =
          mPerActionListShapeMap.find(currActionNode);
      if (actionListIt != mPerActionListShapeMap.end()) {
    ...
    if (sInfo.type != ShapeType_NbElements) {  // We have found shape, so add it
      ...
      ESS_LOG_INFO(
          "Could not determine shape type for particle with born index: "
          << nBornIndex << ". Defaulting to point.");
      type = ShapeType_Point;
    }
  }
}
...
AlembicPoints::meshInfo AlembicPoints::CacheShapeMesh(
    Mesh *pShapeMesh, BOOL bNeedDelete, Matrix3 meshTM, int nMatId,
    int particleId, TimeValue ticks, ShapeType &type, Abc::uint16_t &instanceId,
    float &animationTime)
{
  ESS_PROFILE_FUNC();
  ...
  meshDigests digests;
  {
    ...
    AbcU::MurmurHash3_x64_128(pShapeMesh->verts,
                              pShapeMesh->numVerts * sizeof(Point3),
                              sizeof(Point3), digests.Vertices.words);
    AbcU::MurmurHash3_x64_128(pShapeMesh->faces,
                              pShapeMesh->numFaces * sizeof(Face), sizeof(Face),
                              digests.Faces.words);
    if (mJob->GetOption("exportMaterialIds")) {
      ...
      AbcU::MurmurHash3_x64_128(&matIds[0], matIds.size() * sizeof(MtlID),
                                sizeof(MtlID), digests.MatIds.words);
    }
  ...
  if (it != mShapeMeshCache.end()) {
    ...
    // ESS_LOG_WARNING("Adding shape with hash("<<vertexDigest.str()<<",
    // "<<faceDigest.str()<<") \n auto assigned name "<<mi.name <<"
    // efficiency:"<<(double)mNumShapeMeshes/mTotalShapeMeshes);
  }
...
// we have to save out all mesh encountered on the current frame of this object
// immediately, because
// the pointers will no longer be valid when the object is evaluated at a new
// time
void AlembicPoints::saveCurrentFrameMeshes()
{
  ...
  for (int i = 0; i < mMeshesToSaveForCurrentFrame.size(); i++) {
    ...
    if (mi->pMesh) {
      ...
      options.SetOption("exportMaterialIds",
                        mJob->GetOption("exportMaterialIds"));
      ...
      {
        ESS_PROFILE_SCOPE(
            "AlembicPoints::saveCurrentFrameMeshes - finalPolyMesh.Save");

        Imath::M44f transform44f;
        ConvertMaxMatrixToAlembicMatrix(mi->meshTM, transform44f);
        SceneNodeMaxParticlesPtr inputSceneNode(
            new SceneNodeMaxParticles(pMesh, NULL, mi->nMatId));
      ...
      {
        ESS_PROFILE_SCOPE(
            "AlembicPoints::saveCurrentFrameMeshes - save xforms, bbox, geo, "
            "topo");
        ...
        meshSample.setFaceCounts(
            Abc::Int32ArraySample(finalPolyMesh.mFaceCountVec));
        meshSample.setFaceIndices(
            Abc::Int32ArraySample(finalPolyMesh.mFaceIndicesVec));
      }

      if (mJob->GetOption("validateMeshTopology")) {
        ESS_PROFILE_SCOPE(
            "AlembicPoints::saveCurrentFrameMeshes - validateMeshTopology");
        mJob->mMeshErrors +=
            validateAlembicMeshTopo(finalPolyMesh.mFaceCountVec,
                                    finalPolyMesh.mFaceIndicesVec, mi->name);
      }

      if (mJob->GetOption("exportNormals")) {
        ESS_PROFILE_SCOPE(
            "AlembicPoints::saveCurrentFrameMeshes - exportNormals");
        AbcG::ON3fGeomParam::Sample normalSample;
        normalSample.setScope(AbcG::kFacevaryingScope);
        normalSample.setVals(
            Abc::N3fArraySample(finalPolyMesh.mIndexedNormals.values));
        normalSample.setIndices(
            Abc::UInt32ArraySample(finalPolyMesh.mIndexedNormals.indices));
        meshSample.setNormals(normalSample);
      }

      if (mJob->GetOption("exportMaterialIds")) {
        ESS_PROFILE_SCOPE(
            "AlembicPoints::saveCurrentFrameMeshes - exportMaterialIds");
        Abc::OUInt32ArrayProperty mMatIdProperty = Abc::OUInt32ArrayProperty(
            meshSchema, ".materialids", meshSchema.getMetaData(),
            mJob->GetAnimatedTs());
        mMatIdProperty.set(
            Abc::UInt32ArraySample(finalPolyMesh.mMatIdIndexVec));

        for (facesetmap_it it = finalPolyMesh.mFaceSets.begin();
             it != finalPolyMesh.mFaceSets.end(); it++) {
          std::stringstream nameStream;
      ...
      {
        ESS_PROFILE_SCOPE(
            "AlembicPoints::saveCurrentFrameMeshes meshSchema.set");
