...
{
  return &AlembicMeshUVWModifierDesc;
}

//--- Properties block -------------------------------

static ParamBlockDesc2 AlembicMeshUVWModifierParams(
    0, _T(ALEMBIC_MESH_UVW_MODIFIER_SCRIPTNAME), 0,
    GetAlembicMeshUVWModifierClassDesc(), P_AUTO_CONSTRUCT | P_AUTO_UI, 0,

    // rollout description
    IDD_ALEMBIC_MESH_UVW_PARAMS, IDS_ALEMBIC, 0, 0, NULL,

    // params
    AlembicMeshUVWModifier::ID_PATH, _T("path"), TYPE_FILENAME, P_RESET_DEFAULT,
    IDS_PATH, p_default, "", p_ui, TYPE_EDITBOX, IDC_PATH_EDIT, p_assetTypeID,
    kExternalLink, p_end,

    AlembicMeshUVWModifier::ID_IDENTIFIER, _T("identifier"), TYPE_STRING,
    P_RESET_DEFAULT, IDS_IDENTIFIER, p_default, "", p_ui, TYPE_EDITBOX,
    IDC_IDENTIFIER_EDIT, p_end,

    AlembicMeshUVWModifier::ID_TIME, _T("time"), TYPE_FLOAT, P_ANIMATABLE,
    IDS_TIME, p_default, 0.0f, p_range, 0.0f, 1000.0f, p_ui, TYPE_SPINNER,
    EDITTYPE_FLOAT, IDC_TIME_EDIT, IDC_TIME_SPIN, 0.01f, p_end,

    AlembicMeshUVWModifier::ID_MUTED, _T("muted"), TYPE_BOOL, P_ANIMATABLE,
...
void AlembicMeshUVWModifier::EnumAuxFiles(AssetEnumCallback &nameEnum,
                                          DWORD flags)
{
...
void AlembicMeshUVWModifier::ModifyObject(TimeValue t, ModContext &mc,
                                          ObjectState *os, INode *node)
{
  ...
  this->pblock->GetValue(AlembicMeshUVWModifier::ID_IDENTIFIER, t,
  ...
  // ESS_LOG_INFO( "AlembicMeshUVWModifier::ModifyObject strPath: " << strPath
  ...
  if (!bMuted && strPath && strIdentifier) {
    ...
    // we need the path to the alembic mesh object, so we need to remove the
    // channel name part of the identifier
    ...
    options.pObjectCache =
        getObjectCacheFromArchive(szPath, strObjectIdentifier);
...
RefResult AlembicMeshUVWModifier::NotifyRefChanged(const Interval &changeInt,
...
{
#else
RefResult AlembicMeshUVWModifier::NotifyRefChanged(Interval changeInt,
                                                   RefTargetHandle hTarget,
                                                   PartID &partID,
                                                   RefMessage message)
{
      ...
      if (hTarget == pblock) {
        ...
        switch (changing_param) {
          case ID_PATH: {
            ...
            Interval v;
            pblock->GetValue(AlembicMeshUVWModifier::ID_PATH, t, strPath, v);
...
void AlembicMeshUVWModifier::BeginEditParams(IObjParam *ip, ULONG flags,
                                             Animatable *prev)
{
  this->ip = ip;
  editMod = this;
...
void AlembicMeshUVWModifier::EndEditParams(IObjParam *ip, ULONG flags,
                                           Animatable *next)
{
