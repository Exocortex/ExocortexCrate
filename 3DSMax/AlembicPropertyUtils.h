...
struct AbcProp {
  ...
  AbcProp(std::string n, std::string val, AbcA::PropertyHeader header,
          bool bConstant, int sortid)
      : name(n), displayVal(val), propHeader(header), bConstant(bConstant)
  {
    if (sortid != 0) {
      sortId = sortid;
    }
  ...
  AbcProp(std::string n, std::string val, int sortid)
      : name(n), displayVal(val), bConstant(true)
  {
    if (sortid != 0) {
      sortId = sortid;
    }
...
void addFloatController(std::stringstream& evalStream,
                        alembic_importoptions& options,
                        const std::string& modkey, std::string propName,
                        const std::string& file, const std::string& identifier,
...
class AlembicCustomAttributesEx {
 ...
 public:
  AlembicCustomAttributesEx(std::string modNamein)
      : pblock(NULL), modName(modNamein)
  {
  }

  ~AlembicCustomAttributesEx();

  bool defineCustomAttributes(INode* node, Abc::OCompoundProperty& compoundProp,
...
template <class PT>
PT readScalarProperty(Abc::ICompoundProperty propk, const std::string& propName)
{
...
float readScalarPropertyExt3(Abc::ICompoundProperty propk,
                             const SampleInfo& sampleInfo,
                             const std::string& propName,
                             const std::string& propComp)
{
  ...
  if (fProp.valid()) {
    ...
    ESS_LOG_WARNING("Float Controller Error: could not read user property "
                    << propName);
    return -1.0;
  }
}
