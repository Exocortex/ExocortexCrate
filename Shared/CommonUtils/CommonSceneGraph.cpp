...
void SceneNodeFile::setMerged(bool bMerged) { isMergedIntoAppNode = bMerged; }
bool SceneNodeFile::isAttached() { return isAttachedToAppNode; }
void SceneNodeFile::setAttached(bool bAttached)
{
  isAttachedToAppNode = bAttached;
}

bool SceneNodeFile::isMerged() { return isMergedIntoAppNode; }
bool SceneNodeAlembic::isSupported()
{
  return NodeCategory::get(pObjCache->obj) != NodeCategory::UNSUPPORTED;
}

Abc::IObject SceneNodeAlembic::getObject() { return pObjCache->obj; }
void SceneNodeAlembic::print()
{
  ESS_LOG_WARNING(
      "AlembicNodeObjectFullName: " << pObjCache->obj.getFullName());
}
...
{
  const char* classType[] = {"FILE",    "FILE_ALEMBIC", "APP",
                             "APP_MAX", "APP_MAYA",     "APP_XSI"};
  ...
  // ESS_LOG_WARNING("ExoSceneGraph Begin - ClassType:
  // "<<classType[root->getClass()]);
  ...
  while (!sceneStack.empty()) {
    ...
    if (!bOnlyPrintSelected || (bOnlyPrintSelected && eNode->selected)) {
      ...
      ESS_LOG_WARNING(
          "Level: " << sElement.level << " - Name: " << eNode->name.c_str()
                    << " - Type: " << table[eNode->type]
                    << " - ddcID: " << eNode->dccIdentifier.c_str()
                    << " - Selected: " << (eNode->selected ? "true" : "false"));
      // if(eNode->parent){
...
struct SelectChildrenStackElement {
  ...
  SelectChildrenStackElement(SceneNodePtr enode, bool selectChildren)
      : eNode(enode), bSelectChildren(selectChildren)
  {
  }
};
                ...
                bool isMaya)
{
  ...
  while (!sceneStack.empty()) {
    ...
    if (eNode->type == SceneNode::ETRANSFORM ||
        ...
        eNode->type == SceneNode::HAIR)  // removed to be able to export hair
                                         // properly in Maya!
    {
      SceneNode::SelectionT::iterator selectionIt =
          selectionMap.find(eNode->dccIdentifier);
      ...
      if (selectionIt != selectionEnd) {
        // this node's name matches one of the names from the selection map, so
        // select it
        ...
        if (eNode->type == SceneNode::ETRANSFORM && bSelectShapeNodes) {
          ...
          if (isMaya) {
            for (std::list<SceneNodePtr>::reverse_iterator it =
                     eNode->children.rbegin();
                 it != eNode->children.rend(); ++it) {
              if (::isShapeNode(
                      (*it)->type))  // MergedPolyMesh node should work here...
              {
...
int selectTransformNodes(SceneNodePtr root)
{
  ...
  while (!sceneStack.empty()) {
    ...
    if (eNode->type == SceneNode::NAMESPACE_TRANSFORM ||
...
struct FlattenStackElement {
  ...
  FlattenStackElement(SceneNodePtr node, SceneNodePtr parentNode)
      : currNode(node), currParentNode(parentNode)
  {
  }
};
...
void flattenSceneGraph(SceneNodePtr root, int nNumNodes)
{
  ...
  while (!sceneStack.empty()) {
    ...
    if (fileNode->type == SceneNode::NAMESPACE_TRANSFORM ||  // namespace
                                                             // transform (XSI
                                                             // model)
        fileNode->type ==
            SceneNode::ETRANSFORM ||  // shape node parent transform
    ...
    // here we set what the parent is going to be for the file node we pushing
    // to the stack
    //    - if parentNode is passed instead of fileNode, the subtree starting at
    //    fileNode is being skipped
...
bool SceneNode::getVisibility(double time) { return true; }