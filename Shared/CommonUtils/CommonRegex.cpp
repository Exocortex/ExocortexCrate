...
string NoReplace::replace(const string &str) const { return str; }
RegexReplace::RegexReplace(const string &_exp, const string &_form)
    : NoReplace(), exp(_exp), formatter(_form)
{
}
...
string RegexReplace::replace(const string &str) const
{
  ...
  boost::regex_replace(oi, str.begin(), str.end(), exp, formatter,
...
{
  return (exp == 0 || form == 0) ? defaultNoReplace
                                 : createReplacer(*exp, *form);
}

ReplacePtr createReplacer(const std::string &exp, const std::string &form)
{
  return (exp.length() == 0 || form.length() == 0)
             ? defaultNoReplace
             : ReplacePtr(new RegexReplace(exp, form));
}
}
...
string readEnvVar(const string &str)
{
  ...
  if (!env_value) {
    ESS_LOG_WARNING("Environment variable \""
                    << str << "\" is invalid and is not replaced");
    return str;
  }
...
string replace(const string &str)
{
  ...
  if (parts.size() & 0x1 == 0) {
    ESS_LOG_ERROR(
        "Need an even number of % in the string to delimit environment "
        "variables properly! Nothing is replaced in the string");
    return str;
  }
  ...
  for (std::vector<std::string>::iterator beg = parts.begin();
       beg != parts.end(); ++beg, isEnvVarRound = !isEnvVarRound) {
    if (isEnvVarRound) {
      if (beg->size())
        *beg = readEnvVar(*beg);
      else
